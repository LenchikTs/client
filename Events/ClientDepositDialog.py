...
from PyQt4.QtCore import Qt, QObject, QVariant, pyqtSignature
...
class CClientDepositDialog(CDialogBase, CAmbCardMixin, Ui_ClientDepositDialog):
    @pyqtSignature('')
    ...
    def __init__(self, parent, clientId, clientAge, clientSex, buttonBoxIgnore, title):
        CDialogBase.__init__(self, parent)
        self.addModels('Deposit', CClientDepositModel(self))
        ...
        self.modelDeposit.loadItems(self.currentClientId)
...
class CClientDepositModel(CInDocTableModel):
    def __init__(self, parent):
        CInDocTableModel.__init__(self, 'ClientDeposit', 'id', 'client_id', parent)
        self.addCol(CDateInDocTableCol(u'Дата', 'contractDate', 15, canBeEmpty=True)).setReadOnly(True)
        self.addCol(CClientDepositModel.CContractInDocTableCol(self)).setReadOnly(True)
        self.addCol(CFloatInDocTableCol(u'Сумма депозита', 'contractSum', 8, precision=4)).setReadOnly(True)
