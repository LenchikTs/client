...
def countDispensariable(begDate, endDate, eventTypeId, onlyPermanentAttach, onlyPayedEvents, begPayDate, endPayDate):
    stmt="""
        SELECT
            Organisation.OKFS_id AS OKFS_id,
...
def countByGroups(begDate, endDate, eventTypeId, onlyPermanentAttach, onlyPayedEvents, begPayDate, endPayDate):
    stmt="""
        ...
        WHERE
            %s
        GROUP BY
            Organisation.OKFS_id, `group`, DOD, Event.isPrimary
    """
    db = QtGui.qApp.db
    tableEvent = db.table('Event')
    cond = []
...
def countBySanatoriumAndHospital(begDate, endDate, eventTypeId, onlyPermanentAttach, onlyPayedEvents, begPayDate, endPayDate):
    stmt="""
        ...
        WHERE
            %s
        GROUP BY
            Organisation.OKFS_id, sanatorium, hospital, highTechAid
    """
    db = QtGui.qApp.db
    tableEvent = db.table('Event')
    cond = []
...
def prepareOKFS():
    mapIdToRowIndex = {}
    rows = [(u'ВСЕГО', u'0', u'X'), ]
    rows.append((u'ВСЕГО - бюджетные', u'1', u'X'))
    rows.append((u'ВСЕГО - частные',   u'2', u'X'))
    rows.append((u'ВСЕГО - смешанные', u'3', u'X'))

    db = QtGui.qApp.db
    for record in db.getRecordList('rbOKFS', 'id, code, name, ownership', order='code'):
        id = forceInt(record.value('id'))
        code = forceString(record.value('code'))
        name = forceString(record.value('name'))
        ownership = forceInt(record.value('ownership'))
        index = len(rows)
        rows.append((name, index+1, code))
        if 0<ownership<=3:
            rowIndexes = [0, ownership, index]
        else:
            rowIndexes = [0, index]
...
class CStatReportF12_D_1_08(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        self.setTitle(u'Сведения о дополнительной диспансеризации работающих граждан, Ф.№ 12-Д-1-08',
    ...
    def getSetupDialog(self, parent):
        ...
        result.setEventTypeVisible()
        result.setPayPeriodVisible(True)
        result.resize(0,0)  # Сжать до минимального размера
        return result
...
#        cursor.setCharFormat(CReportBase.ReportBody)
        ...
        tableColumns = [
            ('18%', [u'Наименование формы собственности работодателя гражданина, прошедшего диспансеризацию',
                     u'',
                     u'',
            ...
            ('10%', [u'Код формы собственности по ОКФС',
                     u'',
                     u'',
        ...
        for iRow, row in enumerate(rows):
            if row[2] == 'X' or any(reportData[iRow]):
                i = table.addRow()
                for j in xrange(3):
