...
MainRows = [
    ...
    ( u'из них: бронхопневмония, вызванная S Pneumoniac', u'11.3.1', u'J13'),
...
def selectData(begDate, endDate, eventPurposeId, eventTypeIdList, orgStructureIdList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, eventTypeDDId, params):
    stmt = u"""
SELECT
   ...
   (SELECT IF(rbDispanser.code IN (2,6), 1, 0)
    ...
    EXISTS((SELECT rbm1.regionalCode
        ...
        WHERE
            ...
            AND D1.deleted = 0 and rbm1.regionalCode in (261))) AS getProfilactic,
    ...
    EXISTS((SELECT rbm2.regionalCode
        ...
        WHERE
            D1.diagnosis_id = Diagnosis.id AND E.deleted = 0
            AND D1.deleted = 0 and rbm2.regionalCode in (211))) AS getAdultsDispans,
...
def selectObservedData(begDate, endDate, eventPurposeId, eventTypeIdList, orgStructureIdList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, eventTypeDDId, params):
    stmt = u"""
SELECT
   Diagnosis.MKB AS MKB,
    COUNT(*) AS sickCount,
...
class CStatReportF12Adults_2022(CReport):
    ...
    def build(self, params):
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'(3002) Число физических лиц зарегистрированных пациентов – всего %d , из них с диагнозом , установленным впервые в жизни %d , состоит под диспансерным наблюдением на конец отчетного года (из гр. 11, стр. 1.0) %d .'%(registeredAll, registeredFirst, consistsByEnd[0]))
        cursor.insertBlock()
