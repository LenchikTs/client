...
ORDER BY firstInPeriod DESC, observed DESC
    ...
    cond.append(
        'Diagnosis.endDate <= SUBDATE(ADDDATE(Client.birthDate, INTERVAL IF(Client.sex = 2, 55, IF(Client.sex = 1, 60, 55)) YEAR), 1)')
    if socStatusTypeId:
        subStmt = ('SELECT ClientSocStatus.id FROM ClientSocStatus WHERE '
                   + 'ClientSocStatus.deleted=0 AND ClientSocStatus.client_id=Client.id AND '
                   + 'ClientSocStatus.socStatusType_id=%d' % socStatusTypeId)
...
class CStatReportF12Seniors_2017(CReport):
    ...
    def build(self, params):
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'''(4001) Число физических лиц зарегистрированных пациентов – всего %d , из них с диагнозом , установленным впервые в жизни %d , состоит под диспансерным наблюдением на конец отчетного года (из гр. 9, стр. 1.0) %d .\n(4002) Из числа пациентов, состоящих на конец отчетного года под диспансерным наблюдением (гр. 15): состоит под диспансерным наблюдением лиц с хроническим вирусным гепатитом (В18) и циррозом печени (K74.6) одновременно %d чел.; с хроническим вирусным гепатитом (В18) и гепатоцеллюлярным раком (С22.0) одновременно %d чел.'''%(registeredAll, registeredFirst, consistsByEnd, B18K76, B18C22))
        cursor.insertBlock()
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.setCharFormat(CReportBase.ReportTitle)
        cursor.insertBlock()
