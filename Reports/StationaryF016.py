...
#        orgStructureId = params.get('orgStructureId', None)
        ...
        def unrolledHospitalBed(endDate, orgStructureIdList, profile = None, type=0):
            ...
            bedTypeProfile = db.translate('rbHospitalBedType', 'name', u'профильная', 'id')
            bedTypeRehab = db.translate('rbHospitalBedType', 'name', u'реабилитационная', 'id')
            cond = []
            cond.append(tableVHospitalBed['master_id'].inlist(orgStructureIdList))
            cond.append(tableVHospitalBed['type_id'].inlist([bedTypeProfile, bedTypeRehab]))
...
#                cond.append('''%s'''%(getPropertyAPHBP([profile], True)))
            ...
            stmt = db.selectDistinctStmt(queryTable, u'COUNT(Client.id) AS countAll, SUM(%s) AS countDeath, SUM(%s) AS countTransfer'%(getStringProperty(u'Исход госпитализации', u'(APS.value LIKE \'умер%\' OR APS.value LIKE \'смерть%\')'), getStringProperty(u'Исход госпитализации', u'(APS.value = \'переведен в другой стационар\')')), where=cond)
            query = db.query(stmt)
            if query.first():
                record = query.record()
        ...
        def getOrgStructureParent(orgStructureIdList, rowProfile, table, profile, type=0):
            if type == 1:
                isTypeOS = u'OrgStructure.type != 0'
            elif type == 2:
                isTypeOS = u'(OrgStructure.type = 0 AND (SELECT getOrgStructureIsType(OrgStructure.id)) = 1)'
        ...
        if isGroupingOS:
            getOrgStructureParent(begOrgStructureIdList, nextRow, table, profile, isType)
        else:
            getDataReport(begOrgStructureIdList, nextRow, profile, isType)
        return doc
    ...
    def averageDaysHospitalBed(self, orgStructureIdList, begDatePeriod, endDatePeriod, profile = None, isHospital = None, type = 0):
        ...
        cond = [tableOSHB['master_id'].inlist(orgStructureIdList),
                ...
                tableOS['deleted'].eq(0),
                isTypeOS,
                tableOSHB['type_id'].inlist([bedTypeProfile, bedTypeRehab]),
