...
from library.Utils      import forceInt, forceString, getInfisCodes, toVariant
...
# WTF? почему это не члены класса,
groupTitles = []
reg_num=0
reg_ind={}


def selectData(begDate, endDate,  eventTypeId, PermanentAttach, onlyPayedEvents, begPayDate, endPayDate, orgStructureId):
    stmt="""
SELECT distinct
...
class CStatReportF131Raion(CReport):
    ...
    def build(self, params):
        ...
        Raion           = forceString(params.get('Raion', None))
        YearDD          = forceString(params.get('YearForDD', ''))
        orgStructureId  = params.get('orgStructureId', None)
        reportRowSize = 4
        reportDataByGroups = []
        reportData = []
        query = selectData(begDate, endDate, eventTypeId, PermanentAttach, onlyPayedEvents, begPayDate, endPayDate, orgStructureId)
        ...
        while query.next():
            ...
            adr=getClientAddressAsString(clientId)
...
''' % (lpu, OGRN, dest, YearDD, num, prik))
        ...
        cursor.insertBlock()
        org_chief = _getChiefName(QtGui.qApp.currentOrgId())
        cursor.insertText(
u'''
