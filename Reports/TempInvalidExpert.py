...
def selectData(begDate, endDate, byPeriod, doctype, tempInvalidReasonIdList, onlyClosed, orgStructureId, personId, durationFrom, durationTo, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, MKBFilter, MKBFrom, MKBTo, insuranceOfficeMark):
    stmt="""
SELECT
    SUM(d1) AS s1,
    SUM(d2) AS s2,
...
SELECT
   ...
   BIT_OR( TempInvalid.duration >= 30 ) AS d3,
   BIT_OR( TempInvalid.duration >= 30 AND rbTempInvalidReason.code in ('04', '07')) AS d4,

   BIT_OR( rbTempInvalidResult.decision = 5 ) AS d5,
   BIT_OR( rbTempInvalidResult.decision = 5  AND rbTempInvalidReason.code in ('04', '07')) AS d6,

   BIT_OR( rbTempInvalidResult.decision = 3 ) AS d7,
   BIT_OR( rbTempInvalidResult.decision = 3  AND rbTempInvalidReason.code in ('04', '07')) AS d8,

   BIT_OR( rbTempInvalidResult.state = 1 AND rbTempInvalidResult.able = 1 ) AS d9
...
class CTempInvalidExpert(CReport):
    ...
    def build(self, params):
        ...
        query = selectData(begDate, endDate, byPeriod, doctype, tempInvalidReasonIdList, onlyClosed, orgStructureId, personId, durationFrom, durationTo, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, MKBFilter, MKBFrom, MKBTo, insuranceOfficeMark)
        if query.next():
            record = query.record()
        else:
