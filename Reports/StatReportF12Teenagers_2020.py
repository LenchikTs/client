...
def getClientCountFor2004(registeredInPeriod, begDate, endDate, eventPurposeId, eventTypeIdList, orgStructureIdList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, params):
    clientsCount = 0
    clientsDeadCount = 0
    ...
    while query.next():
        ...
        if (firstInPeriod or diseaseCharacter == '1') and getProfilactic and not getAdultsDispans:
            if not getObserved and not getRemovingObserved and not observed:
                clientsDeadCount += forceInt(sickCount)
    params['dead'] = isDead
...
class CStatReportF12Teenagers_2020(CReport):
    ...
    def build(self, params):
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'(2001) Число физических лиц зарегистрированных пациентов – всего %d , из них с диагнозом , установленным впервые в жизни %d , состоит под диспансерным наблюдением на конец отчетного года (из стр. 1.0, гр. 13) %d.'%(registeredAll, registeredFirst, consistsByEnd[0]))
        cursor.insertBlock()
        ...
        cursor.insertBlock()
        cursor.insertText(u'(2004) Число лиц с болезнями системы кровообращения, взятых под диспансерное наблюдение (стр. 10.0 гр. 8) - %d, из них умерло %d.' %
