...
class CStatReportF12Adults_2017(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        title = u'Ф.12. 4.Взрослые 18 лет и более.'
        self.setTitle(title, u'Ф.12. 4.Взрослые 18 лет и более.')
    ...
    def build(self, params):
        ...
        tableColumns = [
            ...
            ('10%', [u'Снято с диспансерного наблюдения',                 u'',                                                                          u'',                                                                    u'13'], CReportBase.AlignRight),
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'''(3002) Число физических лиц зарегистрированных пациентов – всего %d , из них с диагнозом , установленным впервые в жизни %d , состоит под диспансерным наблюдением на конец отчетного года (из гр. 15, стр. 1.0) %d .'''%(registeredAll, registeredFirst, consistsByEnd))
        cursor.insertBlock()
        cursor.insertText(u'''(3003) Из числа пациентов, состоящих на конец отчетного года под диспансерным наблюдением (гр. 15): состоит под диспансерным наблюдением лиц с хроническим вирусным гепатитом (В18) и циррозом печени (K74.6) одновременно {0} чел.; с хроническим вирусным гепатитом (В18) и гепатоцеллюлярным раком (С22.0) одновременно {1} чел. '''.format(B18K76, B18C22))
        cursor.insertBlock()
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.setCharFormat(CReportBase.ReportTitle)
        cursor.insertBlock()
