...
from library.Utils         import forceBool, forceDouble, forceInt, forceString, forceRef
...
SELECT
    ...
    (IF(rbMedicalAidType.code IN ('1', '2', '3'), (IF(Event.execDate = Event.setDate, 1, DATEDIFF(Event.execDate, Event.setDate))),
    (IF(Event.execDate = Event.setDate, 2, DATEDIFF(Event.execDate, Event.setDate) + 1)))),
    (IF(Event.execDate = Event.setDate, 1, DATEDIFF(Event.execDate, Event.setDate)))))  AS amount,
...
GROUP BY
    EventType.medicalAidType_id, %s isAdult, exposed, refused, mesCode, mesName, Event.id
"""
    db = QtGui.qApp.db
...
class CReportF14App3ksg(CReport):
    ...
    def addRowData(self, table, i, title, values, style=None):
        ...
        table.setText(i, 5, values[1], style)
