...
MainRowsToOneYear = [
    ...
    ( u'расщелина губы и неба (заячья губа и волчья пасть)', u'18.2', u'Q35-Q37'),
    ( u'хромосомные аномалии, не классифицированные в других рубриках', u'18.3', u'Q90-Q99'),
    ( u'симптомы, признаки и отклонения от нормы, выявленные при клинических и лабораторных исследованиях, не классифицированные в других рубриках', u'19.0', u'R00-R99'),
    ( u'травмы, отравления и некоторые другие последствия воздействия внешних причин', u'20.0', u'S00-T98'),
    ( u'из них: открытые укушенные раны (только с кодом внешней причины W54)', u'20.1', u'S01, S11, S21, S31, S41, S51, S61, S71, S81, S91')
];
        ...
        WHERE
          ...
          AND D1.endDate = (
            ...
            WHERE D2.diagnosis_id = Diagnosis.id
              AND D2.dispanser_id IS NOT NULL
              AND (D2.endDate IS NULL OR D2.endDate >= %s)
              AND (D2.setDate IS NULL OR D2.setDate < %s))) = 1, 1, 0)'''%(tableDiagnosis['setDate'].formatValue(begDate),
              tableDiagnosis['setDate'].formatValue(endDate.addDays(1))))
    if boolThyroidosData:
...
class CStatReportF12Children0_14_2019(CReport):
...
class CStatReportF12Children0_1_2019(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
    ...
    def build(self, params):
        ...
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.insertBlock()
        cursor.insertText(u'''(1601) Число физических лиц зарегистрированных пациентов в возрасте до 3 лет – всего %d, из них в возрасте до 1 года %d, из них (из стр. 1) с диагнозом, установленным впервые в жизни %d, из них в возрасте до 1 года %d, состоит под диспансерным наблюдением на конец отчетного года детей в возрасте до 3 лет (из гр. 18 и 19 стр. 1.0) %d, из них в возрасте до 1 года %d.'''%(resAll4, res5, res1011, res10, res1819, res18))
        cursor.insertBlock()
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.setCharFormat(CReportBase.ReportTitle)
        cursor.insertBlock()
