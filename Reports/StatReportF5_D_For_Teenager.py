...
from Reports.Report     import CReport, CReportSetupDialog, normalizeMKB
...
def selectDiagnostics(begDate, endDate, eventTypeId, onlyPermanentAttach, onlyPayedEvents, begPayDate, endPayDate, diagnosisType):
    stmt="""
        SELECT
            Event.id AS eventId,
            Event.client_id,
...
class CStatReportF5_D_For_Teenager(CReport):
    ...
    def build(self, params):
        ...
        query = selectDiagnostics(begDate, endDate, eventTypeId, onlyPermanentAttach, onlyPayedEvents, begPayDate, endPayDate, diagnosisType)
        eventClosed = 0
