...
WHERE %s
GROUP BY MKB, diseaseCharacter, firstInPeriod, getObserved, observed, rbDiagnosisType.id
    """
    db = QtGui.qApp.db
...
WHERE %s AND (Diagnosis.MKB >= 'E00' AND Diagnosis.MKB <= 'E07')
...
class CStatReportF12Adults_2015(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        title = u'Ф.12. 3.Взрослые 18 лет и старше.'
        self.setTitle(title, u'Ф.12. 3.Взрослые 18 лет и старше.')
...
#        result.setAccountAccompEnabled(False)
    ...
    def build(self, params):
        ...
        tableColumns = [
            ('30%',  [u'Наименование классов и отдельных болезней',        u'',                                                   u'1'], CReportBase.AlignLeft),
            ('5%',   [u'№ строки',                                         u'',                                                   u'2'], CReportBase.AlignLeft),
            ('13%',  [u'Код по МКБ-10 пересмотра',                         u'',                                                   u'3'], CReportBase.AlignLeft),
            ('13%',  [u'Зарегистрировано пациентов с данным заболеванием', u'всего',                                              u'4'], CReportBase.AlignRight),
            ('13%',  [u'',                                                 u'из них: с диагнозом, установленным впервые в жизни', u'5'], CReportBase.AlignRight),
            ('13%',  [u'',                                                 u'из них (из гр.5): взято под диспансерное наблюдение',u'6'], CReportBase.AlignRight),
            ('13%',  [u'Состоит на д.н. на конец периода',                 u'',                                                   u'7'], CReportBase.AlignRight)
            ]
        ...
        table.mergeCells(0, 3, 1, 3) # Всего
        table.mergeCells(0, 6, 2, 1) # Всего
        ...
        reportLine = reportMainData[0] if len(reportMainData) > 0 else []
        if len(reportLine) == 4:
            consistsByEnd = reportLine[3]
        ...
        cursor.insertBlock()
        cursor.insertText(u'''(3002) Число физических лиц зарегистрированных пациентов – всего %d , из них с диагнозом , установленным впервые в жизни %d , состоит под диспансерным наблюдением на конец отчетного года (из гр. 7, стр. 1.0) %d .'''%(registeredAll, registeredFirst, consistsByEnd))
        cursor.insertBlock()
