...
MainRows = [
          ( u'Беременность с абортивным исходом в срок с 12 до 22 недель', u'1', u'O00, O02 - O07'),
          ( u'в том числе (из стр. 1): внематочная беременность', u'2', u'O00'),
          ( u'другие анормальные продукты зачатия', u'3', u'O02'),
          ( u'самопроизвольный аборт', u'4', u'O03'),
          ( u'медицинский аборт по медицинским показаниям', u'5', u'O04'),
          ( u'другие виды аборта', u'6', u'O05'),
          ( u'аборт неуточненный', u'7', u'O06'),
          ( u'неудачная попытка аборта', u'8', u'O07')
           ]
...
def selectData(params):
    ...
    condPregnancyWeek = [tableEvent['deleted'].eq(0),
                         ...
                         tableDiagnostic['deleted'].eq(0),
                         tableEvent['pregnancyWeek'].gt(12),
...
class CSickRateAbort_2000(CReport):
    def __init__(self, parent):
        ...
        self.setTitle(u'Раздел II. Беременность с абортивным исходом в срок с 12 до 22 недель')
    ...
    def build(self, params):
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'''(2100) Осложнения, вызванные абортом, внематочной  и  молярной  беременностью  (из стр. 1 гр. 4 табл. 2000): всего %d, из них: инфекция половых путей и тазовых  органов  (O08.0)  %d, длительное или массивное кровотечение (O08.1) %d, эмболия (O08.2) %d, шок (O08.3) %d.'''%(additionalRow.get(u'O08', 0), additionalRow.get(u'O08.0', 0), additionalRow.get(u'O08.1', 0), additionalRow.get(u'O08.2', 0), additionalRow.get(u'O08.3', 0)))
        cursor.insertBlock()
        cursor.movePosition(QtGui.QTextCursor.End)
        return doc
