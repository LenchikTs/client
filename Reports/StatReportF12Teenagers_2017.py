...
MainRows = [
    ...
    ( u'детский аутизм, атипичный аутизм, синдром Ретта, дезинтегративное расстройство детского возраста', u'6.2.1', u'F84.0-3'),
    ...
    ( u'пневмонии', u'11.3', u'J20-J22'),
    ( u'из них: бронхопневмония, вызванная S.Pneumoniae', u'11.3.1', u'J13'),
    ( u'острые респираторные инфекции нижних дыхательных путей', u'11.4', u'J12-J16, J18'),
...
SELECT
  ...
  ((Diagnosis.setDate<= %(endDate)s AND Diagnosis.setDate IS NOT NULL) AND (Diagnosis.setDate>= %(begDate)s )) AS firstInPeriod,

  IF(rbDispanser.code = 1, 1, 0) AS observed,

  if(rbMedicalAidType.regionalCode in ('262'),1,0) AS getProfilactic,
  IF(Event.isPrimary = 2, 1, 0) AS isNotPrimary,
  if(rbMedicalAidType.regionalCode in ('232','252'),1,0) AS getAdultsDispans
...
WHERE Diagnostic.diagnosisType_id  IN (SELECT RBDT.id FROM rbDiagnosisType AS RBDT WHERE RBDT.code = '1' OR RBDT.code = '2')
...
-- AND Event.id=1137216
AND rbMedicalAidType.code NOT IN ('1', '2', '3', '7')
GROUP BY MKB, diseaseCharacter, sex, firstInPeriod, getObserved, getRemovingObserved, getProfilactic, isNotPrimary, observed, getAdultsDispans, rbDiagnosisType.id
    """
    db = QtGui.qApp.db
    ...
    if ageFrom <= ageTo:
        cond.append('age(Client.birthDate, Event.setDate) BETWEEN "%(ageFrom)s" and "%(ageTo)s"'% dict(ageFrom=ageFrom, ageTo=ageTo))
    if socStatusTypeId:
        ...
        WHERE
          ...
          AND D1.endDate = (
            ...
            WHERE D2.diagnosis_id = Diagnosis.id
              ...
              AND  D2.endDate >= %s
              AND  D2.endDate < %s))'''%(tableDiagnosis['setDate'].formatValue(begDate), tableDiagnosis['setDate'].formatValue(endDate.addDays(1))))
...
def selectDataClient(registeredInPeriod, begDate, endDate, eventPurposeId, eventTypeIdList, orgStructureIdList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, params, boolThyroidosData = False, fewMKB = None):
...
class CStatReportF12Teenagers_2017(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        title = u'Ф.12. 3.Дети (15-17 лет включительно).'
    ...
    def getSetupDialog(self, parent):
        ...
        result.setOrgStructureListVisible(False)
    ...
    def build(self, params):
        ...
        while query.next():
            ...
            if diseaseCharacter == '1': # острое
                ...
                if getObserved:
                    cols.append(4)
                if getProfilactic and not getAdultsDispans and getObserved:
                    cols.append(5)
                if sex == 1:
                    cols.append(7)
            elif firstInPeriod:
                if diseaseCharacter == '2':
                    cols.append(3)
                    if getObserved:
                        cols.append(4)
                    if getProfilactic and not getAdultsDispans and getObserved:
        ...
        queryFewMKB = selectDataClient(registeredInPeriod, begDate, endDate, eventPurposeId, eventTypeList, orgStructureList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, params, fewMKB="('B18.0', 'B18.1', 'B18.2', 'B18.2', 'B18.8', 'B18.9', 'C22.0', 'K74.6')")
        MKBDictC22 = {}
        MKBDictK76 = {}
        ...
        tableColumns = [
            ...
            ('6.5%', [u'',                                                 u'из заболеваний с впервые в жизни установленным диагнозом (из гр. 9):',      u'взято под диспансерное наблюдение',                                   u'9'], CReportBase.AlignRight),
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'''(2001) Число физических лиц зарегистрированных пациентов – всего %d , из них с диагнозом , установленным впервые в жизни %d , состоит под диспансерным наблюдением на конец отчетного года (из стр.1,гр.15) %d, передано под наблюдение во взрослую поликлинику____.\n '''%(registeredAll, registeredFirst, consistsByEnd))
        cursor.insertBlock()
        cursor.insertText(u'''(2003) Из числа пациентов, состоящих на конец отчетного года под диспансерным наблюдением (гр. 15): состоит под диспансерным наблюдением лиц с хроническим вирусным гепатитом (В18) и циррозом печени (K74.6) одновременно {0} чел.; с хроническим вирусным гепатитом (В18) и гепатоцеллюлярным раком (С22.0) одновременно {1} чел.'''.format(B18K76, B18C22))
        cursor.insertBlock()
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.setCharFormat(CReportBase.ReportTitle)
        cursor.insertBlock()
