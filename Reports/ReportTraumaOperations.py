...
def selectData(params):
    ...
    tableAManipulation        = db.table('Action').alias('AManipulation')
    tableATManipulation    = db.table('ActionType').alias('ATManipulation')
    ...
    cond = [tableAction['deleted'].eq(0),
            ...
            tableActionPropertyType['descr'].eq(u'Манипуляция'),
    ...
    if byPeriodIssueDate:
        havingCond = u'HAVING incidentDate BETWEEN DATE("%s") AND DATE("%s")'%(forceString(begDate.toString("yyyy-MM-dd")),
                                                                                                                                                        forceString(endDate.toString("yyyy-MM-dd")))
    else:
        cond.append(u'DATE(Action.`begDate`) BETWEEN DATE("%s") AND DATE("%s")'%(forceString(begDate.toString("yyyy-MM-dd")),
                                                                                                                                                        forceString(endDate.toString("yyyy-MM-dd"))))
    ...
    cols = [    tableClient['id'].alias('clientId'),
                    ...
                    u'''ATManipulation.name AS manipulationName''',
                    u'''(SELECT APDdp.value FROM ActionProperty AS APdp
                        LEFT JOIN ActionPropertyType AS APTdp ON APTdp.id = APdp.type_id
                        LEFT JOIN ActionProperty_String AS APDdp ON APDdp.id = APdp.id
                        WHERE APTdp.descr = 'Описание манипуляции' AND AManipulation.id = APdp.action_id LIMIT 1) AS manipulationDescr'''
                ]
    ...
    queryTable = queryTable.leftJoin(tableAManipulation, db.joinAnd([tableAManipulation['event_id'].eq(tableEvent['id']),
                                                                                                tableAManipulation['endDate'].dateEq(tableAction['endDate'])]))
    queryTable = queryTable.leftJoin(tableATManipulation, tableATManipulation['id'].eq(tableAManipulation['actionType_id']))

    queryTable = queryTable.leftJoin(tableActionPropertyType, tableActionPropertyType['actionType_id'].eq(tableATManipulation['id']))
    queryTable = queryTable.leftJoin(tableActionProperty, tableActionProperty['action_id'].eq(tableAManipulation['id']))
...
class CReportTraumaOperations(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        self.setTitle(u'Журнал "Операции"')
    ...
    def build(self, params):
        ...
        tableColumns = [
                        ...
                        ('9%', [u'Номер медицинской карты'],             CReportBase.AlignLeft),
                        ('9%', [u'Врач'],         CReportBase.AlignLeft),
                        ('8%', [u'Ф.И.О. пострадавшего'],        CReportBase.AlignLeft),
                        ('8%', [u'Дата рождения (полных лет)'],            CReportBase.AlignLeft),
                        ('5%', [u'Пол' ], CReportBase.AlignLeft),
                        ('8%', [u'Адрес' ], CReportBase.AlignLeft),
                        ('8%', [u'Телефон' ], CReportBase.AlignLeft),
                        ('8%', [u'Происшествие' ], CReportBase.AlignLeft),
                        ('8%', [u'Дата и время происшествия' ], CReportBase.AlignLeft),
                        ('8%', [u'Диагноз' ], CReportBase.AlignLeft),
                        ('8%', [u'Манипуляция' ], CReportBase.AlignLeft),
                        ('8%', [u'Описание' ], CReportBase.AlignLeft),
                        ]
        ...
        while query.next():
            ...
            manipulationName        = forceString(record.value('manipulationName'))
            manipulationDescr        = forceString(record.value('manipulationDescr'))
            i = table.addRow()
            ...
            if clientSex == 1:
                table.setText(i, 5, u'М')
            else:
                table.setText(i, 5, u'Ж')
            ...
            table.setText(i, 11, manipulationName)
            table.setText(i, 12, manipulationDescr)
        return doc
