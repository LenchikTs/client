...
WHERE Event.deleted = 0
  ...
  AND %(suspicionCond)s
  AND %(primaryCond)s
        GROUP BY Client.sex, Client.birthDate, Diagnosis.MKB, Diagnosis.id''' % {
...
SELECT
    ...
    WHERE D.event_id = Event.id
    AND D.deleted = 0
    AND rbDiagnosticResult.code = '7') AS isResultAdditionDispans,
    EXISTS(SELECT 1
           FROM
           Diagnostic AS D
           LEFT JOIN rbDispanser ON rbDispanser.id = D.dispanser_id
...
WHERE Event.deleted = 0
  ...
  AND %(suspicionCond)s
  AND %(primaryCond)s
        GROUP BY Client.sex, Client.birthDate, Event.id, Diagnosis.MKB, Diagnosis.id''' % {
    'orgStructure':orgStructure,
            'begDate':     db.formatDate(begDate),
            'endDate':     db.formatDate(endDate),
            'endYearDate': db.formatDate(lastYearDay(endDate)),
...
class CReportForm131_o_5000_2015(CReport):
...
class CReportForm131_o_5001_2015(CReportForm131_o_5000_2015):
    def __init__(self, parent):
        CReportForm131_o_5000_2015.__init__(self, parent)
...
class CReportForm131_o_6000_2015(CReportForm131_o_5000_2015):
    def __init__(self, parent):
        CReportForm131_o_5000_2015.__init__(self, parent)
        self.setTitle(u'Сведения об установленных при проведении диспансеризации предварительных диагнозах (случаев)')
