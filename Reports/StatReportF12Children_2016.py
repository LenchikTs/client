...
MainRows = [
    ...
    ( u'детский аутизм, атипичный аутизм, синдром Ретта, дезинтегративное расстройство детского возраста', u'6.2', u'F84.0 - F84.3'),
    ...
    ( u'болезни вен, лимфатических сосудов и лимфатических узлов', u'10.8', u'I80- I89'),
...
MainRowsToOneYear = [
    ...
    ( u'психические расстройства и расстройства поведения', u'6.0', u'F01, F03-F99'),
    ( u'из них: детский аутизм, атипичный аутизм, синдром Ретта, дезинтегративное расстройство детского возраста', u'6.1', u'F84.0-3'),
    ( u'болезни нервной системы', u'7.0', u'G00-G98'),
    ( u'из них: детский церебральный паралич', u'7.9.1', u'G80'),
    ( u'болезни глаза и его придаточного аппарата', u'8.0', u'H00-H59'),
    ( u'преретинопатия', u'8.6', u'H35.1'),
...
AND %s
GROUP BY MKB, diseaseCharacter, firstInPeriod, getObserved, getRemovingObserved, getProfilactic, isNotPrimary, getAdultsDispans, observed, rbDiagnosisType.id, closedEvent, dayAge
    """
    db = QtGui.qApp.db
...
LEFT JOIN rbDiseaseCharacter ON rbDiseaseCharacter.id = Diagnosis.character_id
WHERE %s
GROUP BY Diagnosis.client_id, clientAge, diseaseCharacter, firstInPeriod, observed
...
class CStatReportF12Children0_14_2016(CReport):
    ...
    def build(self, params):
        ...
        tableColumns = [
            ...
            ('6%',  [u'Состоит на д.н. на конец периода',                 u'',                                                                     u'',                                          u'15'], CReportBase.AlignRight)
            ]
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'''(1001) Число физических лиц зарегистрированных пациентов – всего - %d , из них с диагнозом , установленным впервые в жизни - %d , состоит под диспансерным наблюдением на конец отчетного года (из гр. 15, стр. 1.0) - %d .\n(1002) Из числа состоящих под диспансерным наблюдением пациентов с заболеваниями щитовидной железы (из стр. 5.1) снято с диспансерного наблюдения: всего - %d, из них в связи с выздоровлением - %d, со смертью - %d. '''%(registeredAll, registeredFirst, consistsByEnd[0], thyroidosUnhangAll, thyroidosUnhangRecovery, thyroidosUnhangDeath))
        cursor.insertBlock()
        cursor.insertText(u'''(1002) Состоит под диспансерным  наблюдением  на конец отчетного года (из стр. 1.0 гр. 15) детей в возрасте: 0 - 4 года - %d, 5 - 9 лет - %d'''%(consistsByEnd[1], consistsByEnd[2]))
        cursor.insertBlock()
        cursor.movePosition(QtGui.QTextCursor.End)
        cursor.setCharFormat(CReportBase.ReportTitle)
        cursor.insertBlock()
...
class CStatReportF12Children0_1_2016(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        title = u'Ф.12. 2. Дети первого года жизни.'
        self.setTitle(title, u'2. Дети первого года жизни.')
    ...
    def getSetupDialog(self, parent):
        ...
        result.setChkFilterDispanser(True)
        result.setTitle(self.title())
        result.edtAgeFrom.setMaximum(0)
        result.edtAgeTo.setMaximum(1)
        return result
    ...
    def build(self, params):
        ...
        tableColumns = [
            ('25%', [u'Наименование классов и отдельных болезней',        u'',                                                                     u'',                                          u'1'], CReportBase.AlignLeft),
            ('5%',  [u'№ строки',                                         u'',                                                                     u'',                                          u'2'], CReportBase.AlignLeft),
            ('14%', [u'Код по МКБ-10 пересмотра',                         u'',                                                                     u'',                                          u'3'], CReportBase.AlignLeft),
            ('7%',  [u'Зарегистрировано заболеваний',                     u'всего',                                                                u'',                                          u'4'], CReportBase.AlignRight),
            ('7%',  [u'',                                                 u'из них(из гр. 4):',                                                    u'в возрасте до 1 мес.',                      u'5'], CReportBase.AlignRight),
            ('7%',  [u'',                                                 u'',                                                                     u'взято под диспансерное наблюдение',         u'8'], CReportBase.AlignRight),
            ('7%',  [u'',                                                 u'',                                                                     u'с впервые в жизни установленным диагнозом', u'9'], CReportBase.AlignRight),
            ('7%',  [u'',                                                 u'из заболеваний с впервые в жизни установленным диагнозом (из гр. 9):', u'взято под диспансерное наблюдение',         u'10'], CReportBase.AlignRight),
            ('7%',  [u'',                                                 u'',                                                                     u'выявлено при профосмотре',                  u'11'], CReportBase.AlignRight),
            ('7%',  [u'Снято с диспансерного наблюдения',                 u'',                                                                     u'',                                          u'14'], CReportBase.AlignRight),
            ('7%',  [u'Состоит на д.н. на конец периода',                 u'',                                                                     u'',                                          u'15'], CReportBase.AlignRight)
            ]
        ...
        cursor.setCharFormat(CReportBase.ReportTitle)
        cursor.insertBlock()
        cursor.insertText(u'Дети первого года жизни.')
        cursor.insertBlock()
