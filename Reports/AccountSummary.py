...
from library.Utils      import forceDate, forceDouble, forceRef, forceString, forceInt
...
class CAccountSummary(CReportBase):
    ...
    def build(self, description, params):
        ...
        tableColumns = [
                          ...
                          ('13%',  [ u'Тариф'             ], CReportBase.AlignRight ),
                          ('13%',  [ u'НДС'             ], CReportBase.AlignRight ),
                          ('13%',  [ u'Количество'        ], CReportBase.AlignRight ),
                          ('13%',  [ u'Количество пациентов'        ], CReportBase.AlignRight ),
                          ('13%',  [ u'УЕТ'               ], CReportBase.AlignRight ),
        ...
        while query.next():
            ...
            clients = forceInt(record.value('clients'))
            ...
            if prevInsurerName != insurerName or prevInsurerId != insurerId:
                if n:
                    self.produceTotalLine(table, u'Всего по '+prevInsurerName, totalAmount, totalClients, totalUet, total)
            ...
            table.setText(i, 3, vat)
            table.setText(i, 4, amount)
            table.setText(i, 5, clients)
            table.setText(i, 6, uet)
            table.setText(i, 7, sum)
            totalAmount += amount
        ...
        if n and (not orgInsurerId):
            self.produceTotalLine(table, u'Всего по '+insurerName, totalAmount, totalClients, totalUet, total)
        self.produceTotalLine(table, u'Итого по счёту', grandTotalAmount, grandTotalClients, grandTotalUet, grandTotal)
        return doc


    def produceTotalLine(self, table, title, totalAmount, totalClients, totalUet, total):
        i = table.addRow()
        table.mergeCells(i, 0, 1, 4)
        table.setText(i, 0, title, CReportBase.TableTotal, CReportBase.AlignLeft)
        table.setText(i, 4, totalAmount, CReportBase.TableTotal)
        table.setText(i, 5, totalClients, CReportBase.TableTotal)
        table.setText(i, 6, totalUet, CReportBase.TableTotal)
        table.setText(i, 7, total, CReportBase.TableTotal)
