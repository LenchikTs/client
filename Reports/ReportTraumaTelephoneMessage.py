...
def selectData(params):
    ...
    cond = [tableAction['deleted'].eq(0),
            ...
            tableActionPropertyType['descr'].eq(u'Телефонограмма'),
    ...
    cols = [    tableClient['id'].alias('clientId'),
                    ...
                    u'''(SELECT APDdp.value FROM ActionProperty AS APdp
                        LEFT JOIN ActionPropertyType AS APTdp ON APTdp.id = APdp.type_id
                        LEFT JOIN ActionProperty_String AS APDdp ON APDdp.id = APdp.id
                        WHERE APTdp.descr = 'Номер телефонограммы' AND Action.id = APdp.action_id LIMIT 1) AS telephoneMessage'''
                ]
...
class CReportTraumaTelephoneMessage(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
        self.setTitle(u'Журнал "Телефонограмма"')
    ...
    def build(self, params):
        ...
        tableColumns = [
                        ...
                        ('9%', [u'Ф.И.О. пострадавшего'],        CReportBase.AlignLeft),
                        ('9%', [u'Дата рождения (полных лет)'],            CReportBase.AlignLeft),
                        ('5%', [u'Пол' ], CReportBase.AlignLeft),
                        ('9%', [u'Адрес' ], CReportBase.AlignLeft),
                        ('9%', [u'Телефон' ], CReportBase.AlignLeft),
                        ('9%', [u'Происшествие' ], CReportBase.AlignLeft),
                        ('9%', [u'Дата и время происшествия' ], CReportBase.AlignLeft),
                        ('9%', [u'Диагноз' ], CReportBase.AlignLeft),
                        ('9%', [u'Телефонограмма' ], CReportBase.AlignLeft),
                        ]
        ...
        while query.next():
            ...
            telephoneMessage        = forceString(record.value('telephoneMessage'))
            i = table.addRow()
            ...
            table.setText(i, 11, telephoneMessage)
        return doc
