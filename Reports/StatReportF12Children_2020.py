...
LEFT JOIN rbDiseaseCharacter ON rbDiseaseCharacter.id = Diagnosis.character_id
WHERE %s
GROUP BY Diagnosis.client_id, clientAge, diseaseCharacter, firstInPeriod, observed, Diagnosis.MKB
        ...
        WHERE
          ...
          AND D1.endDate = (
            ...
            WHERE D2.diagnosis_id = Diagnosis.id
              ...
              AND (D2.setDate IS NULL OR D2.setDate < %s))) = 1, 1, 0)'''%(tableDiagnosis['setDate'].formatValue(begDate), tableDiagnosis['setDate'].formatValue(endDate.addDays(1))))
    if boolThyroidosData:
        return db.query(stmt % (stmtAddress, db.joinAnd(cond), tableDiagnosis['setDate'].formatValue(endDate.addDays(1)), tableDiagnosis['setDate'].formatValue(endDate.addDays(1)),))
    else:
        return db.query(stmt % (db.joinAnd([tableDiagnosis['setDate'].le(endDate),
                                            tableDiagnosis['setDate'].ge(begDate)]),
                                tableDiagnosis['setDate'].formatValue(endDate.addDays(1)),
...
def getClientCountFor1004(registeredInPeriod, begDate, endDate, eventPurposeId, eventTypeIdList, orgStructureIdList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, params, selectDataFunc=selectData):
    clientsCount = 0
    clientsDeadCount = 0

    params['MKBFrom'] = 'I00'
    params['MKBTo'] = 'I99'
    query = selectDataFunc(registeredInPeriod, begDate, endDate, eventPurposeId, eventTypeIdList, orgStructureIdList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, params)
    while query.next():
        record = query.record()
        ...
        if (firstInPeriod or diseaseCharacter == '1') and getProfilactic and not getAdultsDispans:
            if not getObserved and not getRemovingObserved and not observed:
                clientsCount += forceInt(record.value('sickCount'))

    isDead = params.get('dead', False)
    params['dead'] = True
    query = selectData(registeredInPeriod, begDate, endDate, eventPurposeId, eventTypeIdList, orgStructureIdList, personId, sex, ageFrom, ageTo, socStatusClassId, socStatusTypeId, isFilterAddressOrgStructure, addrType, addressOrgStructureId, locality, params)
    while query.next():
        record = query.record()
        diseaseCharacter    = forceString(record.value('diseaseCharacter'))
        firstInPeriod       = forceBool(record.value('firstInPeriod'))
...
class CStatReportF12Children0_14_2020(CReport):
    ...
    def build(self, params):
        ...
        cursor.insertBlock()
        cursor.insertBlock()
        cursor.insertText(u'(1001) Число физических лиц зарегистрированных пациентов – всего - %d , из них с диагнозом , установленным впервые в жизни - %d , состоит под диспансерным наблюдением на конец отчетного года (из гр. 12, стр. 1.0) - %d .'%(registeredAll, registeredFirst, consistsByEnd[0]))
        cursor.insertBlock()
...
class CStatReportF12Children0_1_2020(CReport):
    def __init__(self, parent):
        CReport.__init__(self, parent)
