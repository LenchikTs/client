...
from PyQt4.QtCore import Qt, QDate, pyqtSignature, SIGNAL

from library.crbcombobox  import CRBComboBox, CRBModel, CRBSelectionModel
from library.Utils import forceString
from RefBooks.QuotaType.List import getQuotaTypeClassItemsByExists

from Quoting.Ui_RBQuotaTypeComboBoxPopup import Ui_RBQuotaTypeComboBoxPopup
...
class CQuotaTypeComboBoxPopup(QtGui.QFrame, Ui_RBQuotaTypeComboBoxPopup):
    ...
    def __init__(self, parent, tableName = ''):
        ...
        self.edtBegDate.setDate(QDate(QDate.year(QDate.currentDate()), 01, 01))
        self.edtEndDate.setDate(QDate(QDate.year(QDate.currentDate()), 12, 31))
    ...
    def setTable(self, tableName, addNone=True, filter='', order=None):
        ...
        if self.chkBegDate.isChecked():
            quotaBegDate = self.edtBegDate.date()
            quotaEndDate = self.edtEndDate.date()
            if quotaBegDate:
                cond.append(db.joinOr([tableQuotaType['begDate'].ge(quotaBegDate), tableQuotaType['begDate'].isNull()]))
            if quotaEndDate:
                cond.append(db.joinOr([tableQuotaType['endDate'].le(quotaEndDate), tableQuotaType['endDate'].isNull()]))
    ...
    def selectItemByIndex(self, index):
        row = index.row()
        self.emit(SIGNAL('quotaTypeSelected(int)'), row)
        self.hide()
    ...
    def _resize(self):
        ...
        width= max(size.width(), parent.width(), hHeaderSize, screen.width())
...
class CQuotaTypeComboBox(CRBComboBox):
    def __init__(self, parent):
        ...
        self.connect(self.popupView, SIGNAL('quotaTypeSelected(int)'), self.setValueByRow)
