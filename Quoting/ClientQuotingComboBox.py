...
from Quoting.Ui_ClientQuotingComboBoxPopup import Ui_Form
...
class CClientQuotingComboBoxPopup(QtGui.QFrame, CConstructHelperMixin, Ui_Form):
    def __init__(self, parent):
        ...
        self._clientId = None
        self._dBegDate = QDate.currentDate()
        self._dEndDate = QDate.currentDate()

        self.setupUi(self)

        self._populateClassCombobox()


    def _populateClassCombobox(self):
        for name, classCode in getQuotaTypeClassItemsByExists():
            self.cmbClass.addItem(name, QVariant(classCode))
    ...
    def updateClientQuoting(self):
        ...
        if self.chkClass.isChecked():
            classCode = self.cmbClass.itemData(self.cmbClass.currentIndex()).toPyObject()
            cond.append(tableQuotaType['class'].eq(classCode))
...
class CClientQuotingComboBox(QtGui.QComboBox):
    def __init__(self, parent, clientId, begDate=QDate(), endDate=QDate()):
        ...
        self.connect(self._popup, SIGNAL('clientQuotingClicked(int)'), self.on_clientQuotingClicked)
...
class CClientQuotingModel(CTableModel):
    def __init__(self, parent):
        ...
        CTableModel.__init__(self, parent, cols=[
            CClassCol(u'Класс', ['quotaType_id'], [u'ВТМП', u'СМП', u'Родовой сертификат'], 10),
    ...
    def data(self, index, role=Qt.DisplayRole):
        ...
        row    = index.row()
        id     = self._idList[row]
        if id == 0 and role == Qt.DisplayRole:
            return QVariant('-')
        else:
