...
u'средство для определения профиля мед. помощи по услуге/полу/возрасту/коду МКБ'
...
class CServiceDetail:
    ...
    def _setupProfiles(self, db, serviceId):
        ...
        for record in db.getRecordList(table, '*', table['master_id'].eq(serviceId), 'idx'):
            ...
            profile = CServiceMedicalAidProfile(specialityId, sex, age, mkbRegExp, profileId, kindId, typeId)
...
class CServiceMedicalAidProfile:
    def __init__(self, specialityId, sex, age, mkbRegExp, profileId, kindId, typeId):
        ...
        self.mkbRegExp = QRegExp(mkbRegExp) if mkbRegExp else None
        self.id = profileId
        self.kindId = kindId
        self.typeId = typeId
