...
from RefBooks.Tables         import rbCode, rbCodeRegional, rbEmergencyReceivedCall, rbName

from .Ui_RBEmergency import Ui_ItemEditorDialog


class CRBEmergencyReceivedCallList(CItemsListDialog):
    def __init__(self, parent):
        CItemsListDialog.__init__(self, parent, [
            CTextCol(u'Код',          [rbCode], 20),
            CTextCol(u'Региональный код', [rbCodeRegional], 20),
            CTextCol(u'Наименование', [rbName], 40),
            ], rbEmergencyReceivedCall, [rbCode, rbName])
        self.setWindowTitleEx(u'Вызов получен')

    def getItemEditor(self):
        return CRBEmergencyReceivedCallEditor(self)

#
# ##########################################################################
#

class CRBEmergencyReceivedCallEditor(CItemEditorBaseDialog, Ui_ItemEditorDialog):
    def __init__(self,  parent):
        CItemEditorBaseDialog.__init__(self, parent, rbEmergencyReceivedCall)
        self.setupUi(self)
        self.setWindowTitleEx(u'Вызов получен')
