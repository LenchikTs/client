...
class CEncryptionHandler(object):
    ...
    def __init__(self, api, userCert=None, receiverCert=None):
        assert userCert is not None, u'Должно быть задано userCert'
        assert receiverCert is not None, u'Должно быть задано receiverCert'
    ...
    def sign(self, sw):
        extraCertificate = sw._header.createAppendElement(DSIG.BASE, 'X509Certificate')
        extraCertificate.createAppendTextNode(base64.b64encode(self._getSenderCertBytes()))
    ...
    def _encrypt(self, envelope):
        ...
        try:
            ...
            try:
                logger.info(u'before decryption: %s' % xmlText)
            except:
                pass
        ...
        keyOid = self.receiverCert.keyOid()
        with api.provider(flags=api.CRYPT_VERIFYCONTEXT,
                          providerType=api.getProviderTypeByKeyOid(keyOid)
                         ) as provider:
            with provider.importPubKeyfromCert(self.receiverCert) as receiverPubKey:
...
#                    print 'api.getDhElEphemAlgIdByKeyOid(keyOid)', api.getDhElEphemAlgIdByKeyOid(keyOid)
#                    with provider.genKey(api.getDhElEphemAlgIdByKeyOid(keyOid), api.CRYPT_EXPORTABLE|api.CRYPT_PREGEN) as ephemeralKey:
                    with provider.genKey(api.getDhElEphemAlgIdByKeyOid(keyOid), api.CRYPT_EXPORTABLE) as ephemeralKey:
...
#                file(fnEncDoc,'wb').write( ciphertext )
        ...
        body.setNamespaceAttribute('ds',   DSIG.BASE)
        body.setNamespaceAttribute('xenc', ENCRYPTION.BASE)
        ...
        x509Certificate.createAppendTextNode( base64.b64encode(receiverCertBytes) )
#        x509Certificate.createAppendTextNode( base64.b64encode(self._getSenderCertBytes()) )
        ...
        cipherValue.createAppendTextNode( base64.b64encode(ktBytes) )

        cipherData = encryptedData.createAppendElement(ENCRYPTION.BASE, 'CipherData')
        cipherValue = cipherData.createAppendElement(ENCRYPTION.BASE, 'CipherValue')
        cipherValue.createAppendTextNode( base64.b64encode(ciphertext) )
    ...
    def _decrypt(self, body):
        ...
        try:
            ...
            pass
        if faultMessage:
            raise Exception(u'Сервис СФР сообщает:\n%s' % anyToUnicode(faultMessage))
...
#                        sessionKey.setParam(api.KP_PADDING, api.ISO10126_PADDING)
                        ...
                        try:
                            ...
                            try:
                                logger.info(u'after decryption: %s' % decryptedMessage)
                            except:
                                pass
