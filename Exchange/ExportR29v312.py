...
def exportR29v312(widget, accountId, accountItemIdList, type):
  #  typeAcc = type
...
class CExportWizard(QtGui.QWizard):
    def __init__(self, parent=None):
        ...
        if type == 1:
            ...
            self.setWindowTitle(u'Мастер экспорта реестра ПР_ФП')
        self.tmpDir = ''
        self.xmlFileName = ''
        self.endDate = None
        ...
        --        PreDiagnosis.MKB AS PreMKB,
            ...
            --    rbTariffCategory.federalCode AS tariffCategoryFederalCode,
                ...
                Account_Item.usedCoefficients as usedCoefficients,
                 if(`rbDispanser`.code IN (1, 2, 4), IF(`rbDispanser`.code = 6, 2,  IF(`rbDispanser`.code iN (3, 5), 6, `rbDispanser`.code)), 0) as dispanserStatus,
                Event.srcDate AS directionDate,
                Event.srcNumber AS directionNumber,
...
#TODO: Переделать хранимку updateAccount
            ...
            if self.parent.page1.type == 1:
                pass
            elif self.parent.page1.chkExportTypeBelonging.isChecked() == False:
                accountOut = CAccountDataStreamWriter(self)
                accountOut.setCodec(QTextCodec.codecForName('cp1251'))
                accountOut.writeFileHeader(accountFile, self.parent.getFullXmlFileName(), contractBegDate)
                accountOut.writeRecord(contractId, payerId, aDate, accNumber, self.parent.getFullXmlFileName(), forceString(accDate.year()), forceString(accDate.month()), accSum, self.log, contractBegDate)
...
# *****************************************************************************************
    ...
    def createXML(self):
        ...
        if self.parent.page1.chkExportTypeBelonging.isChecked() == False and self.parent.page1.type == 0:
...
class CMyXmlStreamWriter(QXmlStreamWriter, CExportHelperMixin):
 ...
 #self.getInfoFromAP(eventId, 'D_RISK')
        ...
        kindStr = None 
        typeStr = None
        usl_ok = forceString(record.value('medicalAidTypeCode'))
        if 'isZKDL' in self.serviceNote:
            profileStr = self.getInfoFromAP(eventId, 'PROFIL')
          #  kindStr = self.getInfoFromAP(eventId, 'VIDPOM')
          
        if personId:
            profile, kindStr, typeStr  = self.getProfile(forceRef(record.value('serviceId')), personId, age, forceString(record.value('sex')))
        ...
        if 'isZKDL' in self.serviceNote:
            profileStr = self.getInfoFromAP(eventId, 'PROFIL')
          #  kindStr = self.getInfoFromAP(eventId, 'VIDPOM')
  #      if 'profil28' in self.serviceNote and MKB[:3] =='U07':
 #           profileStr = 28    
        ...
        if 'isSumAmount' in self.serviceNote:
             tableAI = QtGui.qApp.db.table('Account_Item')
           
             amount = forceDouble(QtGui.qApp.db.getCount(tableAI, 
                                                    'Account_Item.amount', '(Account_Item.event_id =%s)  AND Account_Item.sum > 0 AND Account_Item.master_id = %s'
                                                    %(forceString(record.value('eventId')), accountId)))
        else:
...
#                     if ('OSS8' in self.serviceNote 
#                         and forceString(record.value('dispanserStatus'))!= '0' 
#                         and forceString(self.serviceNote[self.serviceNote.index('isVisit')+1].split(':')[1]) == '1.3'):
#                         self.OOS8 = 1
                    self.writeBenefits(self._lastClientId, self._lastEventId, self.OOS8, self.isRisk)
                    self.OOS8 = 0
                    self.isRisk = 0
...
#Ориентируемся на ___номер полиса___ и страховую компанию SMO
       ...
       #         oss += u'ОСС=1,'
            ...
            if vpolis == '3' and contractBegDate > QDate(2021, 12, 31) :
            ...
            inv = self.getInfoFromAP(eventId, 'INV')
            if inv:
                if u'.' in inv:
                    self.writeTextElement('INV', inv.split('.')[0])
 ...
 #           if ('isSelfCase' in serviceNote) and self._lastEventId == eventId:
   ...
   #             self.writeEndElement() # SLUCH
            ...
            if 'isSelfCaseForVisit' in self.serviceNote: # and serviceStr[:4] != '3.45' and serviceStr[:4] != '3.39' and serviceStr[:4] != '2.34':
...
#             self.caseNum+= 1
            if MKB:
                if 'profil28' in self.serviceNote and MKB[:3] =='U07':
                    profileStr = 28
                    kindStr = 13
                if 'profil42' in self.serviceNote and MKB[:1] =='U':
                    profileStr = 42
            else:
                log(u'Пациент %d событие %d несоответствие врача' % (clientId, eventId), True)
            #    kindStr = 13
            self.writeStartElement('Z_SL')
            self.writeTextElement('IDCASE', '%d' % self.caseNum)
            usl_ok = forceString(record.value('medicalAidTypeCode') if not typeStr else  typeStr)
            self.writeTextElement('USL_OK',  usl_ok)
            ...
            if ((miacCode[:6] !=forceString(record.value('relegateOrgMiac')) and forPom != 1) 
                or ('isNeedNapr' in self.serviceNote and forPom != 1)):
                self.writeTextElement('NPR_MO', forceString(record.value('relegateOrgMiac')),  writeEmtpyTag = False)
            directionEIRNumber, flagDirection, flagExpertise, flagOncology = Export29XMLCommon.getDirectionEIRNumber(QtGui.qApp.db, forceInt(record.value('preEventId')) if forceInt(record.value('preEventId')) >0 else eventId)
            if 'isZKDL' in self.serviceNote:
                self.writeTextElement('ORD_NO', self.getInfoFromAP(eventId, '#ODLI Specimen.container.identifier.value')) #forceString(record.value('directionNumber')))
            if flagDirection == 1:
            ...
            if ((miacCode[:6] !=forceString(record.value('relegateOrgMiac')) 
                 and forceString(record.value('relegateOrgMiac')) != '' and forPom != 1)
                or ('isNeedNapr' in self.serviceNote and forPom != 1)):
                self.writeTextElement('NPR_DATE', direcrionDate.toString(Qt.ISODate))
            lpu1Str = Export29XMLCommon.getMobileModule(QtGui.qApp.db, forceDouble(miacCode)) if eventTypeCode[:3] == '004' else ''
            ...
            #строки Таня 
         #   uslov11 = forceString(record.value('medicalAidTypeCode')) 
            #self.writeTextElement('KD_Z', forceString(duration) if 'isOneDay' not in self.serviceNote else '1') #serviceStr[:4] != '2.38' and serviceStr[:4] != '1.25' else '1')
            if 'isStacionary' in self.serviceNote  or 'isGema' in self.serviceNote :
                self.writeTextElement('KD_Z', forceString(duration) if 'isOneDay' not in self.serviceNote else '1') #serviceStr[:4] != '2.38' and serviceStr[:4] != '1.25' else '1')

            
            
            self.writeTextElement('RSLT', forceString(record.value('eventResultFederalCode')))
            self.writeTextElement('ISHOD', forceString(resultFederalCode) if forceString(record.value('eventResultFederalCode')) != '313' else u'305')
           ...
           # self.writeTextElement('SL_ID', '%d' % self.caseNum) 
        ...
        service['flagOncology'] = 1 if usl_ok == '4' else  flagOncology
        service['TNMS'] = TNMS
        service['TNMS1'] = TNMS1
        #добавила условие таня
        if 'isStacionary' in self.serviceNote  or 'isGema' in self.serviceNote :
             case['KD'] = forceString(duration) if 'isOneDay' not in self.serviceNote else '1'
     ...
     #   self.writeTextElement('DET', (u'1' if age < 18 else u'0'))
        ...
        if 'isVisit' in self.serviceNote:
            ...
            visitScene = 0
            visitPurpose = 0
            visitType = self.serviceNote[self.serviceNote.index('isVisit')+1].split(':')[1]
            if visitId:
            ...
            if ('OSS8' in self.serviceNote 
                        and forceString(record.value('dispanserStatus'))!= '0' 
                        and visitScene in ('2', '3')):
                        self.OOS8 = 1
            if (forceString(record.value('dispanserStatus')) != '0'
                 and self.isDispanObserve(forceString(record.value('eventTypeId'))) == True):
               case['P_CEL'] = '1.3'
               if MKB:
                   if MKB in ('I50.0', 'I50.1', 'I50.9', 'I10', 'I11', 'I12',
                              'I13', 'I15', 'I20.1', 'I20.8', 'I20.9', 'I25.0',
                              'I25.1', 'I25.2', 'I25.5', 'I25.6', 'I25.8', 'I25.9',
                              'I48', 'I47', 'I69.0', 'I69.1', 'I69.2', 'I69.3',
                              'I69.4', 'I67.8', 'I65.2'):
                       self.isRisk = 1
               if visitScene in ('2', '3'):
                   self.OOS8 = 1
           ...
           #     case['P_CEL'] = '1.3'
                ...
                if visitPurpose == '1.0':
                    if visitScene == '2':
                        ...
                        case['P_CEL'] = visitPurpose
                
                else:
                    case['P_CEL']  = visitPurpose
                if MKB:
                    if MKB[:1] == 'Z' and (case['P_CEL'] == '3.0'):
                        case['P_CEL'] = '3.1'
        
        if 'isDiagnosticResearch' in self.serviceNote:
             case['NHISTORY'] = forceInt(record.value('action_id'))
       
        elif forceString(record.value('eventTypeRegionalCode')) != '401':
            
            case['NHISTORY'] = ('%s' % (eventExternalId if eventExternalId != '' and contractBegDate > QDate(2015, 2, 28) else eventId))[:50]
        else:
            case['NHISTORY'] = ('%s' % forceString(QtGui.qApp.db.translate(QtGui.qApp.db.table('EmergencyCall'), 'event_id', eventId, 'numberCardCall')))
     #   transferName = Export29XMLCommon.getTransferName(QtGui.qApp.db, forceInt(record.value('preEventId')) if forceInt(record.value('preEventId')) >0 else eventId)
        ...
        if 'isDateByAction' in self.serviceNote and forceInt(record.value('action_id')) and 'isOneCase' not in self.serviceNote: # :
            (begDate, endDate) = Export29XMLCommon.getDatesForAction(QtGui.qApp.db,
                                                                ...
                                                                forceInt(record.value('action_id')),
                                                                self.serviceNote)
        elif 'isStacionary' in self.serviceNote:
            begDate = forceDate(record.value('begDateEvent'))
        if not begDate:
             log(u'Пациент %d не указана дата начала события' % clientId, True)
        else:
      ...
      #  case['KD'] =  forceString(duration) if 'isOneDay' in self.serviceNote else '1'
       # self.writeTextElement('KD', forceString(duration) if serviceStr[:4] != '2.38' and serviceStr[:4] != '1.25' else '1'  )
        
        birthDate = forceDate(record.value('birthDate'))
        (MKB, resultFederalCode, traumaCode, AccMKB, CompMKB, 
            PreMKB, TNMS, characterDiag, flagOncology1, onk, dispanserStatus) =  Export29XMLCommon.getDiagnosis(self, eventId, personId, 
                                                                                               forceRef(record.value('eventExecPersonId')),
                                                                                                contractBegDate)
        case['DS0'] = forceString(PreMKB)
        # self.writeTextElement('DS0', forceString(PreMKB))
        if 'isMainDiagnosis' in self.serviceNote and not MKB:
            MKB = PreMKB
        if 'isZKDL' in self.serviceNote:
            MKB = forceString(QtGui.qApp.db.translate(QtGui.qApp.db.table('Action'), 'id', actionId, 'MKB'))
        case['DS1'] = forceString(MKB)
        case['DS2'] = AccMKB
        service['DS'] = forceString(MKB) 
        case['DS3'] = forceString(CompMKB)
        if MKB:
           # if 'isNotRequiredDiagChar' not in self.serviceNote and (MKB[:1] == 'C' or  MKB[:1] != 'Z' or  MKB[:3] in ('D70','D45','D46','D47') or MKB[:2] == 'D0') :
           if 'isNotRequiredDiagChar' not in self.serviceNote and ( 
                                                                   MKB[:1] != 'Z') :
                case['C_ZAB'] = forceString(characterDiag)
           elif (MKB[:1] == 'C' or                                                                
                 MKB[:3] in ('D70','D45','D46','D47') or 
                 MKB[:2] == 'D0'):
               case['C_ZAB'] = forceString(characterDiag)
       ...
       # self.writeTextElement('DS2', forceString(AccMKB))
      ...
      #  if forceString(record.value('dispanserStatus')) != '0' and 'isPossibleDispanObserve' in self.serviceNote and flagOncology != 0 and self.isOncology ==1:
        ...
        if dispanserStatus != '0':
          #  self.writeTextElement('DN', forceString(record.value('dispanserStatus')))  
            case['DN'] = dispanserStatus
            service['DN'] = dispanserStatus
        else:
            service['DN']  = '0'
        
     #   if flagOncology1 == 0 and self.isOncology ==1: 
     ...
     #       Export29XMLCommon.writeOncology1(self, eventId, TNMS, TNMS1, self.getSchema(eventId), 0)
            
      #  if flagOncology1 == 1 and self.isOncology ==1:                
      #      self.writeDirection(eventId)
      #      self.writeCons(eventId)
            
            
        case['REAB'] = forceString(self.isReabialition(forceInt(record.value('event_id'))))
     ...
     #   self.isReabialition(forceInt(record.value('event_id')))
        ...
        case['PRVS'] = forceString(self.parent.getPersonSpecialityFederalCode(personId))
...
#            self.writeTextElement('PRVS', forceString(self.parent.getPersonSpecialityFederalCode(personId)))
     ...
     #   case['IDDOKT'] = personSNILS
      ...
      #  self.writeTextElement('ED_COL', ('%6.2f' % amount).strip())
        if 'isOneSum' in self.serviceNote and 'isStacionary' not in self.serviceNote:
            summ = forceDouble(QtGui.qApp.db.getSum(QtGui.qApp.db.table('Account_Item'), 
                                                    'sum', '(event_id = %s or event_id = %s) and master_id = %d'
                                                    %(eventId, forceString(record.value('preEventId')), accountId)))
          #  if forceInt(record.value('ContractTariffType')) == CTariff.ttEventByMES or forceInt(record.value('ContractTariffType')) == CTariff.ttEventByMESLen:
              #  self.writeTextElement('TARIF',('%10.2f' % round(summ,2)).strip())
           #     case['TARIF'] = ('%10.2f' % round(summ,2)).strip()
        else:
            summ = Export29XMLCommon.doubleToStrRounded(forceDouble(record.value('sum')))
          #  if forceInt(record.value('ContractTariffType')) == CTariff.ttEventByMES or forceInt(record.value('ContractTariffType')) == CTariff.ttEventByMESLen:
               # self.writeTextElement('TARIF',('%10.2f' % round(summ,2)).strip())
        
        case['TARIF'] = ('%10.2f' % round(summ,2)).strip() # ('%10.2f' % round(forceDouble(record.value('price')),2)).strip()

      #  self.writeTextElement('SUM_M',('%10.2f' % round(summ,2)).strip())
        case['SUM_M'] = ('%10.2f' % round(summ,2)).strip()
      ...
      #  if forceInt(record.value('preEventId')):
        ...
        (MKB, resultFederalCode, traumaCode, AccMKB, CompMKB, PreMKB, 
         TNMS, characterDiag, flagOncology1, onk, dispanserStatus) =  Export29XMLCommon.getDiagnosis(self, eventId, personId, 
                                                                                                     forceRef(record.value('eventExecPersonId')), 
                                                                                                     contractBegDate)
    #    if 'isMainDiagnosis' in self.serviceNote and not MKB:
    #            MKB = PreMKB
    #    service['DS'] = MKB
        ...
        service['MED_DEV'] = self.getActions(eventId, 'MED_DEV')
        service['age'] = age
        service['sex'] = forceString(record.value('sex'))
        if not personId:
      ...
      #  service
        if not personSNILS:
            ...
            case['IDDOKT'] = personSNILS[0:3] + '-' +personSNILS[3:6]+'-' +personSNILS[6:9]+'-' +personSNILS[9:11]
        eventTypeCode = forceString(record.value('eventTypeCode'))
        lpu1Str = Export29XMLCommon.getMobileModule(QtGui.qApp.db, forceDouble(miacCode)) if eventTypeCode[:3] == '004' else ''
        service['LPU_1'] = lpu1Str    
        case['LPU_1'] = lpu1Str   
...
#                 self.writeTextElement('PROFIL_K', bedProfile)
                 service['PROFIL_K'] = bedProfile
       # if 'isSchemaRequired' in self.serviceNote:
        service['CRIT'] = self.getSchema(eventId)
        if MKB == 'U07.1' or MKB == 'U07.2':
            case['WEI'] = self.getInfoFromAP(eventId, 'WEI', 'MedicalSupplie', 'ActionProperty_Double')
            case['LEK_PR'] = self.getActions(eventId, 'MedicalSupplie')
   #         if case['LEK_PR'] == []:
   #             case['LEK_PR'] = self.getActions(eventId, '1-6', 1)
       # if schema != {}:
       #     if '1' in schema.keys():
       #         service['DKK1'] = schema['1']
       #     elif '2' in schema.keys():
       #         service['DKK2'] = schema['2']

        if 'hasOperation' in self.serviceNote:
            service['VID_VME'] = self.getOperationService(serviceId)
        
#         if (uet > 0) or 'isDateByAction' in self.serviceNote:
#             (begDate, endDate) = Export29XMLCommon.getDatesForAction(QtGui.qApp.db,
#                                                                     forceInt(record.value('event_id')),
#                                                                     forceInt(record.value('action_id')),
#                                                                     self.serviceNote)
...
#             self.writeServices(service, log, contractBegDate)
     #  if 'isOneSum' not in self.serviceNote:    
      #      self.writeEndElement() #SL
        ...
        if 'isOneSum' in self.serviceNote :
            ...
            self.summ = summ if self.summ < summ else self.summ 
           # self.summ = summ
            self.medicalAidKind = forceString(record.value('medicalAidUnitFederalCode'))
            
            if 'isStacionary' in self.serviceNote:
                self.writeCases( case, service, log, contractBegDate, 0)
                self.flagMain = 1
            elif self.flagMain == 1:
                self.writeCases( case, service, log, contractBegDate, 2)

            else:
                self.writeCases( case, service, log, contractBegDate, 1)
    ...
    def writeServices(self, service, log, contractBegDate):
        ...
        if 'hasVirtual' in service['note']:
            ...
            for vt in visits:
                ...
                serv['PRVS'] = forceString(self.parent.getPersonSpecialityFederalCode(vt['personId']))
                ...
                serv['PROFIL'], kindStr, typeStr  = self.getProfile(service['id'], vt['personId'], service['age'], service['sex'])
                if not serv['PROFIL']:
                    serv['PROFIL'] = service['PROFIL']
                if 'profil28' in service['note'] and service['DS'][:3] == 'U07':
                    serv['PROFIL'] =28
                if 'profil42' in service['note'] and service['DS'][:1] == 'U':
                    serv['PROFIL'] =42
                self.writeService(serv, contractBegDate)
                service['NPL'] = ''
        if  'hasAmountVirtual' in service['note']:
    ...
    def writeService(self, service, contractBegDate):
        ...
        if contractBegDate < QDate(2021, 12, 31):
            order = [ 'UID_MIS', 'N_KSG', 'CRIT', 'LPU', 'LPU_1', 'PODR', 
                     'ADDR_CODE', 'PROFIL',  'VID_VME', 'DET', 'DATE_IN', 
                     'DATE_OUT','DS', 'CODE_USL', 'KOL_USL', 'TARIF', 
                  'SUMV_USL', 'PRVS', 'CODE_MD', 'NAPR', 'ONK_USL', 
                  'NPL', 'COMENTU', 'REFERRAL_NO']
        else:
            order = [ 'UID_MIS', 'N_KSG', 'CRIT', 'LPU', 'LPU_1', 'PODR', 
                     'ADDR_CODE', 'PROFIL',  'VID_VME', 'DET', 'DATE_IN', 
                     'DATE_OUT','DS', 'CODE_USL', 'KOL_USL', 'TARIF', 
                  'SUMV_USL', 'MED_DEV', 'MR_USL_N', 'NAPR', 'ONK_USL', 
                  'NPL', 'COMENTU', 'REFERRAL_NO']
            ...
            if tag in service.keys():
                if tag == 'COMENTU':
                        ...
                        for coefficient in com.values():
                            ...
                            for item, coeff in coefficient.items(): 
                                if forceString(item) != u'__all__':
                                    ...
                                    if item == '30' or item == '31' or item == '102' or item == '103':
                   ...
                   for el in service['CRIT']:
                    ...
                    if service['MED_DEV'] !=[]:
                        for action in service['MED_DEV']:
                            self.writeStartElement('MED_DEV')
                            self.writeTextElement('DATE_MED', 
                                                  forceDate(action.value('endDate')).toString(Qt.ISODate) )
                            self.writeTextElement('CODE_MEDDEV', 
                                                  forceString(QtGui.qApp.db.translate('ActionType', 
                                                                                      'id', 
                                                                                      action.value('actionType_id'), 
                                                                                      'code')) )
                            self.writeTextElement('NUMBER_SER', self.getInfoFromAPbyActionId(action.value('id'), 
                                            u'NUMBER_SER', 
                                            'MED_DEV') )
                            self.writeEndElement()
                else:
    ...
    def writeCases(self, case, service, log, contractBegDate, flag):
        order = ['LPU_1', 'PODR', 'ADDR_CODE', 'PROFIL', 'PROFIL_K', 
                 'DET', 'P_CEL', 'NHISTORY', 'P_PER', 'DATE_1',
                  'DATE_2', 'WEI', 'KD', 'DS0', 'DS1', 'DS2', 'DS3', 'C_ZAB', 
                  'DS_ONK','DN', 'CODE_MES1', 'CODE_MES2', 'NAPR','CONS','ONK_SL',
                  'KSG_KPG', 'REAB', 'PRVS', 'VERS_SPEC', 'IDDOKT', 'ED_COL', 'TARIF',
                   'SUM_M', 'LEK_PR', 'USL',  'COMENTSL' ]
        if flag == 1:
            self.writeServices(service, log, contractBegDate)
         #   self.writeEndElement()
        else:
            ...
            for tag in order:
                if tag == 'DS2':
                    if 'DS2' in case and case['DS2'] != [] and case['DS2'] != None:
                        for ds in case['DS2']:
                            self.writeTextElement(tag, forceString(ds), writeEmtpyTag = False)
                elif tag == 'LEK_PR'  and 'LEK_PR' in case.keys():
                    if case['LEK_PR'] !=[]:
                        self.writeLP(case['LEK_PR'], log, contractBegDate)
                elif tag == 'USL':
                    self.writeServices(service, log, contractBegDate)
                elif tag == 'NAPR' and case['flagOncology'] == 1 and self.isOncology ==1:
                    self.writeDirection(case['eventId'], case['LPU'])
                elif tag == 'CONS' and case['flagOncology'] in (0, 1) and self.isOncology == 1:
                    self.writeCons(case['eventId'])
                elif tag == 'ONK_SL' and case['flagOncology'] == 0 and self.isOncology ==1:
    ...
    def writeBenefits(self, clientId, eventId, isOSS, isRisk=None):
            ...
            if isRisk == 1:
                isRisk = self.getInfoFromAP(eventId, 'D_RISK')
            if codes != [] or  osses != [] or isOSS == 1 or trombs != [] or isRisk:
                self.writeStartElement('DSVED')
                if isOSS == 1:
                    self.writeTextElement('OSS', '8')
                for oss in osses:
                ...
                if isRisk:
                    self.writeTextElement('D_RISK', isRisk.split('.')[0])
                self.writeEndElement() # DSVED
                
    def writeLP(self, actions, log, contractBegDate):
     #   actions = self.getActions(eventId, 'MedicalSupplie')
        actionsAEP = []  
        for action in actions:
            id  = forceString(action.value('id'))
            db = QtGui.qApp.db
            table = db.table('ActionExecutionPlan_Item')
            tableA = db.table('Action')
            actionsPlans = []
            amount = 0
            # проверка лек препаратам
            code_sh = self.getInfoFromAPbyActionId(action.value('id'), 
                                        u'CODE_SH', 
                                        'MedicalSupplie', 
                                        'ActionProperty_rbNomenclatureClass',   
                                        'rbNomenclatureClass',
                                        'code')
            recordAEP = db.getRecordEx(table, '*', table['action_id'].eq(id)) 
        #    if 1 !=1:
        #    self.wirteL(action, code_sh)   
            if recordAEP and id not in actionsAEP:
                flagAEP = 1
                actionsPlanId = db.getMin(table, 'action_id', table['master_id'].eq(forceString(recordAEP.value('master_id'))))
                amount = db.getCount(table, 'action_id', db.joinAnd([table['master_id'].eq(forceString(recordAEP.value('master_id'))),
                                                                 table['date'].eq(forceDate(recordAEP.value('date')))]))
                table = table.leftJoin(tableA, tableA['id'].eq(table['action_id']))
                actionsAEP = db.getRecordListGroupBy(table, '*', db.joinAnd([table['master_id'].eq(forceString(recordAEP.value('master_id')))
                                                                         #  , table['date'].ne(forceDate(recordAEP.value('date')))
                                                                           , "action_id is not null AND Action.status = 2"])
                                                                           , 'date') 
#                 aeps = db.getIdList(table, 'id', db.joinAnd([table['id'].eq(forceString(recordAEP.value('master_id')))
#                                                                            , table['date'].ne(forceDate(recordAEP.value('date')))
#                                                                            , "action_id is not null"
#                                                                           ]))
    
                action =  db.getRecordEx(tableA, '*', tableA['id'].eq(actionsPlanId), 
                                         db.joinAnd([tableA['status'].eq('2'),
                                                 tableA['deleted'].eq('0')] ))  
                                                 
                for a in actionsAEP:
                    amount = db.getCount(table, 'action_id', db.joinAnd([table['master_id'].eq(forceString(a.value('master_id'))),
                                                                 table['date'].eq(forceDate(a.value('date')))]))
                    date = forceDate(a.value('date')).toString(Qt.ISODate)
                    self.wirteL(action, code_sh, amount, date)
            else:
                self.wirteL(action, code_sh)
                
    def wirteL(self, action, code_sh, amount=0, date = None):
        id  = forceString(action.value('id'))
        if code_sh:
            self.writeStartElement('LEK_PR')
            self.writeTextElement('DATE_INJ', date if date else forceDate(action.value('endDate')).toString(Qt.ISODate)  )

            self.writeTextElement('CODE_SH', code_sh)
            lekPR = self.getLP(id)
            self.writeTextElement('REGNUM', lekPR)
            self.writeTextElement('COD_MARK', self.getInfoFromAPbyActionId(action.value('id'), 
                                                                 u'COD_MARK', 
                                                                 'MedicalSupplie'))

            
            if code_sh[:1] in ('1','2'):
                self.writeStartElement('LEK_DOSE')
                self.writeTextElement('ED_IZM', self.getUnit(id))  #self.getInfoFromAPbyActionId(action.value('id'), 
                                                  #                   u'ED_IZM', 
                                                  #                   'MedicalSupplie', 
                                                   #                  'ActionProperty_rbUnit',
                                                    #                 'rbUnit',              
                                                     #                'code'))
                self.writeTextElement('DOSE_INJ', forceString(self.getInfoFromAPbyActionId(action.value('id'), 
                                                                     u'DOSE_INJ', 
                                                                     'MedicalSupplie', 
                                                                     'ActionProperty_Double')))
                self.writeTextElement('METHOD_INJ', self.getInfoFromAPbyActionId(action.value('id'), 
                                                                     u'METHOD_INJ', 
                                                                     'MedicalSupplie', 
                                                                     'ActionProperty_rbNomenclatureUsingType',
                                                                     'rbNomenclatureUsingType',
                                                                     'code'))
                self.writeTextElement('COL_INJ', forceString(action.value('amount')) if amount == 0 else forceString(amount))
                self.writeEndElement() # LEK_DOSE
    ...
    def writeCons(self, eventId):
        stmt = """SELECT ActionType.flatCode as flatCode, ActionPropertyType.descr as descr, ActionPropertyType.shortName as name, ActionProperty.id,  
                        ...
                        WHERE flatCode IN ('ControlListOnko', 'Consilium') AnD Action.deleted =0 
                        AND ActionProperty.deleted =0  and Action.event_id = %d 
                        and ActionPropertyType.shortName IN ('kosilium', 'kosiliumDate') AND Action.status != 3 """ %eventId
    ...
    def writeDirection(self, eventId, lpu):
        ...
        stmt3 = """select a1.id as actionId, a1.begDate as directionDate, at2.code as code,  at1.flatCode as serviceCode, Organisation.miacCode as NAPR_MO
                    ...
                    left join ActionType at1 on at1.id=a1.actionType_id 
                     ...
                     where at1.flatCode in ('consultationDirection', 'inspectionDirection', 'researchDirection2018',
                                             'consultationTaktikInspectionDirection', 'consultationDirection2018', 
                                             'inspectionDirection2018', 'consultationTaktikInspectionDirection2018') and 
        ...
        while query.next():
            ...
            if forceString(query.record().value('NAPR_MO')) != '' and lpu != forceString(query.record().value('NAPR_MO')): #and lpu != forceString(query.record().value('NAPR_MO')):
...
#            self.writeTextElement('SUMMAV', 'dummy')
            if u'F-' in forceString(accNumber):
                str = forceString(accNumber)
                self.writeTextElement('IFIN', accNumber.split('F-')[1])
            elif      u'НСЗ' in forceString(accNumber):
            ...
            elif    u'МБТ-НМП' in forceString(accNumber):
                self.writeTextElement('IFIN', u'МБТ-НМП')
            ...
            elif    u'КОВ1310' in forceString(accNumber):
                self.writeTextElement('IFIN', u'КОВ1310')
            ...
            elif    u'-1310' in forceString(accNumber):
                self.writeTextElement('IFIN', u'1310') 
            else:
    ...
    def getUnit(self, actionId):    
        db = QtGui.qApp.db
        stmt = """SELECT 
                    rbUnit_Identification.value as code
                    FROM Action
                    LEFT JOIn ActionProperty ON ActionProperty.action_id = Action.id 
                    LEFT JOIN ActionPropertyType ON ActionPropertyType.id = ActionProperty.type_id 
                    LEFT JOIN ActionProperty_rbUnit ON ActionProperty_rbUnit.id = ActionProperty.id
                    LEFT JOIN rbUnit ON ActionProperty_rbUnit.value = rbUnit.id
                    LEFT JOIN rbUnit_Identification ON rbUnit_Identification.master_id = rbUnit.id
                    LEFT JOIN rbAccountingSystem ON rbUnit_Identification.system_id = rbAccountingSystem.id
                    LEFT JOIN ActionType ON ActionType.id = Action.actionType_id 
                    WHERE -- ActionType.isNomenclatureExpense = 1 -- AND Action.note = '1'  
                    -- AND
                    Action.status != 3 AND ActionProperty.deleted =0 AND Action.deleted =0
                    AND rbAccountingSystem.urn = 'urn:oid:1.2.643.5.1.13.13.11.1358' 
                    AND Action.begDate > rbUnit_Identification.checkDate and Action.id = %s""" 
        query = db.query(stmt % actionId)
        if query.next():
            record = query.record()
            return forceString(record.value(0))
        else:
    ...
    def getLP(self, actionId):    
        ...
        stmt = """SELECT 
                    ...
                    AND Action.begDate > rbNomenclature_Identification.checkDate and Action.id = %s""" 
        query = db.query(stmt % actionId)
        if query.next():
            record = query.record()
            return forceString(record.value(0))
    ...
    def isDispanObserve(self, eventTypeId):
        exportId = forceString(QtGui.qApp.db.translate('rbAccountingSystem', 'code', 'DN', 'id'))
        stmt = """SELECT `EventType_Identification`.value as code
                            FROM `EventType_Identification`
                            WHERE `EventType_Identification`.`system_id`=%s AND 
                                    EventType_Identification.master_id = %s""" %(exportId, eventTypeId)
        query =  QtGui.qApp.db.query(stmt)
        if query.next():
            if forceString(query.record().value('code'))=='1.3':
                return True
        else:
            return False
    ...
    def getActions(self, eventId, code, type=0):
        if type == 1:
            s = "ActionType.code IN ('%s')"%code
        else:
            s = "ActionType.flatCode IN ('%s') "%code
        stmt = """SELECT *
                    FROM Action
                    LEFT JOIN ActionType ON ActionType.id = actionType_id
                    WHERE %s
                    AND `Action`.event_id =%d AND Action.deleted = 0 AND Action.status = 2
                     """%( s, eventId)
        query = QtGui.qApp.db.query(stmt)
        codeM = []
        if query:
            while query.next():
                codeM.append(query.record())
        return codeM   
    
    def getEndDateForActionByAT(self, eventId, type):  
        stmt = """SELECT `Action`.endValue as endDate
                     FROM `Action`
                     LEFT JOIN `ActionType` ON `Action`.`actionType_id` = `ActionType`.`id`
                     WHERE `Action`.`event_id`=%d AND ActionType.flatCode = '%s'
                             AND  `Action`.`deleted` =0 %s""" %(eventId, type)
        query =  QtGui.qApp.db.query(stmt)
        if query.next():
            return  forceDate(query.record().value('endDate'))            
            
    def getInfoFromAP(self, eventId, descr, 
                      type = None, tableAP= None, 
                      addTable = None, ch='value'):
        table = QtGui.qApp.db.table('Action')
        tableAPTV = QtGui.qApp.db.table(tableAP if tableAP else 'ActionProperty_String')
        tableAT = QtGui.qApp.db.table('ActionType')
        tableAP = QtGui.qApp.db.table('`ActionProperty`')
        tableAPT = QtGui.qApp.db.table('`ActionPropertyType`')
        
        table = table.leftJoin(tableAT, table['actionType_id'].eq(tableAT['id']))
        table = table.leftJoin(tableAP, tableAP['action_id'].eq(table['id']))
        table = table.leftJoin(tableAPT, tableAP['type_id'].eq(tableAPT['id']))
        table = table.leftJoin(tableAPTV, tableAPTV['id'].eq(tableAP['id']))
        cond = [table['event_id'].eq(eventId)]
        cond.append(table['deleted'].eq(0))
        cond.append(tableAPT['descr'].eq(descr))
        if type:
            cond.append(tableAT['flatCode'].eq(type))
        if addTable:
            addTable = QtGui.qApp.db.table(addTable)
            table = table.leftJoin(addTable, addTable['id'].eq(tableAPTV['value']))
            stmt = QtGui.qApp.db.selectStmt(table, addTable[ch], where=cond)
        else:
            stmt = QtGui.qApp.db.selectStmt(table, tableAPTV[ch], where=cond)
        query = QtGui.qApp.db.query(stmt)
        if query.next():
            if tableAPTV.tableName == 'ActionProperty_Double':
                return  forceDouble(query.record().value(ch))
            else:
                return  forceString(query.record().value(ch))

    def getInfoFromAPbyActionId(self, actionId, descr, 
                      type = None, tableAP= None, 
                      addTable = None, ch='value', isId = 0):
        table = QtGui.qApp.db.table('Action')
        tableAPTV = QtGui.qApp.db.table(tableAP if tableAP else 'ActionProperty_String')
        tableAT = QtGui.qApp.db.table('ActionType')
        tableAP = QtGui.qApp.db.table('`ActionProperty`')
        ...
        if addTable:
            if isId == 1:
                tableIdentification = QtGui.qApp.db.table(addTable + '_Identification')
                tableSystem = QtGui.qApp.db.table('`rbAccountingSystem`')
            addTable = QtGui.qApp.db.table(addTable)
            table = table.leftJoin(addTable, addTable['id'].eq(tableAPTV['value']))
            if isId == 1:
                table = table.leftJoin(tableIdentification, tableIdentification['master_id'].eq(addTable['id']))
                table = table.leftJoin(tableSystem, tableSystem['id'].eq(tableIdentification['system_id']))
                stmt = QtGui.qApp.db.selectStmt(table, tableIdentification['value'], where=cond)
            else:
                stmt = QtGui.qApp.db.selectStmt(table, addTable[ch], where=cond)
        else:
            stmt = QtGui.qApp.db.selectStmt(table, tableAPTV[ch], where=cond)
    ...
    def getSchema(self, eventId):
        str = u"""(ActionType.code LIKE 'sh%' 
                or ActionType.code LIKE 'it%' 
                or ActionType.code LIKE 'rb%' 
                or ActionType.code LIKE 'mt%' 
                or ActionType.code LIKE 'pn%'
                or ActionType.code LIKE 'pbt%'
                or ActionType.code LIKE 'olt%'
                or ActionType.code LIKE 'nif%'
                or ActionType.code LIKE 'if%'
                or ActionType.code LIKE 'bt%'
                or ActionType.code LIKE 'flt%'
                or ActionType.code LIKE 'plt%'
                or ActionType.code LIKE 'lgh%'
                or ActionType.code LIKE 'stt%'
                or ActionType.code LIKE 'ivf%'
                or ActionType.code LIKE 'gem%'
                or ActionType.code LIKE 'derm%'
                or ActionType.code LIKE 'amt%'
                or ActionType.code LIKE 'fr%'
                or ActionType.code LIKE 'gibp%'
                or ActionType.code LIKE 'irs%'
                or ActionType.code LIKE 'mgi%'
                or ActionType.code LIKE 'rbpt%'
                or ActionType.code LIKE 'rbs%'
                )"""
...
#                 return (forceString(query1.record().value('cCode')))
   ...
   #     TNMS = TNMS.replace(u'MМ', u'М')
    ...
    def getProfile(self, serviceId, personId, age, sex):
...
#                print "Shit happens"
        ...
        stmt = """SELECT rbMedicalAidProfile.regionalCode, rbMedicalAidKind.code, rbMedicalAidType.regionalCode, 
                rbService_Profile.sex, rbService_Profile.age
        ...
        if query:
            while query.next():
                ...
                if record:
                    if  forceInt(record.value(4)) > 0:
                        if forceInt(record.value(4)) == forceInt(sex):
...
class CAccountDataStreamWriter(QXmlStreamWriter):
    ...
    def writeRecord(self, contractId, payerId, accDate, accNumber, fileName, year, month, accSum, log, contractBegDate):
        db = QtGui.qApp.db
    ...
    def writeHeader(self, fileName, accDate):
        self.writeStartElement('ZGLV')
        if accDate < QDate(2022, 1, 1):
            self.writeTextElement('VERSION', '1.0')
        else:    
            self.writeTextElement('VERSION', '1.1' if accDate < QDate(2022, 1, 2) else '1.2')
      #  self.writeTextElement('DATA', accDate.toString(Qt.ISODate))
        self.writeEndElement()
