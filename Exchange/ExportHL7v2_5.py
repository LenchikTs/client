...
from PyQt4.QtCore       import Qt, QDate, QDateTime, QFile, QXmlStreamWriter, pyqtSignature, SIGNAL

from library.DialogBase import CConstructHelperMixin
from library.Utils      import anyToUnicode, forceBool, forceDate, forceDateTime, forceInt, forceRef, forceString, formatSex, toVariant
...
from Registry.Utils     import getClientAddressEx, getClientInfo
from Reports.StatReport1NPUtil import havePermanentAttach

from Exchange.Ui_ExportHL7v2_5_Wizard_1 import Ui_ExportHL7v2_5_Wizard_1
from Exchange.Ui_ExportHL7v2_5_Wizard_2 import Ui_ExportHL7v2_5_Wizard_2


def ExportHL7v2_5(parent):
    dirName = forceString(QtGui.qApp.preferences.appPrefs.get('ExportHL7v2_5DirName', ''))
    exportAll = forceBool(QtGui.qApp.preferences.appPrefs.get('ExportHL7v2_5ExportAll', False))
    compressRAR = forceBool(QtGui.qApp.preferences.appPrefs.get('ExportHL7v2_5CompressRAR', False))
    exportOnlyOwn = forceBool(QtGui.qApp.preferences.appPrefs.get('ExportHL7v2_5ExportOnlyOwn', True))
    dlg = CExportHL7v2_5(dirName, exportAll, compressRAR,  exportOnlyOwn)
    dlg.exec_()
    QtGui.qApp.preferences.appPrefs['ExportHL7v2_5ExportAll'] = toVariant(dlg.exportAll)
    QtGui.qApp.preferences.appPrefs['ExportHL7v2_5DirName'] = toVariant(dlg.dirName)
    QtGui.qApp.preferences.appPrefs['ExportHL7v2_5CompressRAR'] = toVariant(dlg.compressRAR)
    QtGui.qApp.preferences.appPrefs['ExportHL7v2_5ExportOnlyOwn'] = toVariant(dlg.exportOnlyOwn)
...
class CMyXmlStreamWriter(QXmlStreamWriter):
    ...
    def writeEventType(self,  eventTypeString):
        ...
        self.writeTextElement(u'n:TS.1', QDateTime.currentDateTime().toString('yyyyMMddhhmmss'))
...
#                typeName = forceString(record.value('complainTypeName'))
                complain = '' # getActionPropertyValue(id, typeName) # FIXME
        ...
        try:
            ...
            while (query.next()):
                ...
                fileName = '%s/hl7-%d.%s' % (dirName, \
                    QDateTime.currentDateTime().toTime_t(),  fileExt)
