...
from PyQt4.QtCore import Qt, QDateTime, QFile, QXmlStreamWriter, pyqtSignature, SIGNAL
...
from library.Utils import anyToUnicode, forceBool, forceDate, forceInt, forceRef, forceString, formatSNILS, get_date, toVariant

from Events.Action import CActionPropertyType
from Exchange.DBFFormats import get_131_Fields_new
from Exchange.Utils import compressFileInRar, getDiags, get_dop, getOtherDiags, get_specs, getWorkHurt, tbl
from Orgs.Utils import getOrganisationInfo, getOrganisationShortName
from Registry.Utils import getAddress, getAttachRecord, getCityName, getClientAddress, getClientDocument, getClientInfo, getClientPolicy, getClientWork, getStreetName
from Reports.ReportBase import CReportBase, createTable
from Reports.ReportView import CReportViewDialog

from Exchange.Ui_Export131_Wizard_1 import Ui_Export131_Wizard_1
from Exchange.Ui_Export131_Wizard_2 import Ui_Export131_Wizard_2
...
class CMyWizardPage1(QtGui.QWizardPage, Ui_Export131_Wizard_1):
    def __init__(self, parent):
        ...
        self.done=False
        EventTypes_str=forceString(QtGui.qApp.preferences.appPrefs.get('EventTypes', '1, 12, 25, 27, 26'))
    ...
    def export(self):
        ...
        try:
            ...
            dbf131=Dbf(dbfFileName, new=True, encoding='cp866')
...
#                ClientAttaches=Client['attaches']
                ...
                if KLADRCode:
                    socr=forceString(db.translate('kladr.KLADR', 'CODE', KLADRCode, 'SOCR'))
                ...
                if documentRecord:
                    ...
                    documentTypeRecord = db.getRecord(tableDocumentType, 'name', documentType_id)
...
class Export131(QtGui.QWizard):
    def __init__(self, parent=None):
        ...
        self.rarname=''
        self.ext=''
        self.date1=forceDate(QtGui.qApp.preferences.appPrefs.get('Export131begDate', None))
        self.date2=forceDate(QtGui.qApp.preferences.appPrefs.get('Export131endDate', None))
        ...
        self.rec_num=0
        self.tmp_dir = unicode(tempfile.mkdtemp('','samson_dbf_'), locale.getpreferredencoding())
        self.page(0).checkFinished.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Export131Finished', 'False')))
        self.page(0).checkPayed.setChecked(forceBool(   QtGui.qApp.preferences.appPrefs.get('Export131Payed', 'False')))
        self.page(0).checkRAR.setChecked(forceBool(     QtGui.qApp.preferences.appPrefs.get('Export131RAR', 'False')))
        self.page(0).checkXML.setChecked(forceBool(     QtGui.qApp.preferences.appPrefs.get('Export131XML', 'False')))
        self.page(0).chkUseDefaultAnalysisValue.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Export131UseDefaultAnalysisValue', 'False')))
...
class CMyXmlStreamWriter(QXmlStreamWriter):
    def __init__(self,  parent, OGRN):
        ...
        self.tableDocumentType = self.db.table('rbDocumentType')
        ...
        if recordList:
            for record in recordList:
                ...
                if typeId and name and typeName:
                    typeRecord = self.db.getRecord('ActionPropertyType', '*', typeId)
                    actionPropertyType = CActionPropertyType(typeRecord)

                    val =  forceString(actionPropertyType.getValue(id))
                    if val:
                        valueDict[name] = val
                    elif evaluation:
...
#                    TIP_DD=u'921Б'
        ...
        if KLADRCode:
            socr = forceString(self.db.translate('kladr.KLADR', 'CODE', KLADRCode, 'SOCR'))
