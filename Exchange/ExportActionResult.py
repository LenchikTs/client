...
from PyQt4.QtCore import Qt, QDate, QFile, QFileInfo, QXmlStreamWriter, pyqtSignature, SIGNAL
...
from library.Utils import anyToUnicode, exceptionToUnicode, forceBool, forceInt, forceString, toVariant


from Accounting.Utils import CContractTreeModel, getContractDescr
from Events.ActionStatus import CActionStatus
from Events.Utils import getWorkEventTypeFilter
from Exchange.Utils import compressFileInRar
from Registry.Utils import getClientInfo,  getClientPhones

from Exchange.Ui_ExportActionResult_Wizard_1 import Ui_ExportActionResult_Wizard_1
from Exchange.Ui_ExportActionResult_Wizard_2 import Ui_ExportActionResult_Wizard_2


def ExportActionResult(parent = None):
    appPrefs = QtGui.qApp.preferences.appPrefs
    fileName = forceString(appPrefs.get('ExportActionResultFileName', ''))
    compressRAR = forceBool(appPrefs.get('ExportActionResultCompressRAR', 'False'))
    dlg = CExportActionResult(fileName, compressRAR,  parent)
    dlg.page(1).addDateToFileName = forceBool(appPrefs.get('ExportActionResultAddDate', 'False'))
    dlg.page(1).addContractNumberToFileName = forceBool(appPrefs.get('ExportActionResultAddContractNumber', 'False'))
    dlg.page(1).verboseLog = forceBool(appPrefs.get('ExportActionResultVerboseLog', 'False'))
    dlg.exec_()
    appPrefs['ExportActionResultFileName'] = toVariant(dlg.fileName)
    appPrefs['ExportActionResultCompressRAR'] = toVariant(dlg.compressRAR)
    appPrefs['ExportActionResultAddDate'] = toVariant(dlg.page(1).addDateToFileName)
    appPrefs['ExportActionResultAddContractNuumber'] = toVariant(dlg.page(1).addContractNumberToFileName)
    appPrefs['ExportActionResultVerboseLog'] = toVariant(dlg.page(1).verboseLog)
...
#                self.writeEndElement()
        ...
        if self.parent.parent.includeClientContactInfo:
            ...
            for (typeName, contact, notes) in clientContact:
...
class CExportActionResultWizardPage1(QtGui.QWizardPage, Ui_ExportActionResult_Wizard_1, CConstructHelperMixin):
    ...
    def updateFilterAction(self):
        ...
        if self.chkActionDate.isChecked():
            cond.append(self.tableAction['begDate'].ge(self.edtActionBegDate.date()))
            cond.append(self.tableAction['endDate'].le(self.edtActionEndDate.date()))

        if self.chkActionStatus.isChecked():
            cond.append(self.tableAction['status'].eq(self.cmbActionStatus.value()))

        if self.chkActionSetPerson.isChecked():
            cond.append(self.tableAction['setPerson_id'].eq(self.cmbActionSetPerson.value()))

        if self.chkActionClass.isChecked():
            cond.append(self.tableActionType['class'].eq(self.cmbActionClass.currentIndex()))

        if self.chkActionType.isChecked():
            cond.append(self.tableAction['actionType_id'].eq(self.cmbActionType.value()))
    ...
    def on_selectionModelTree_currentChanged(self, current, previous):
        ...
        if self.parent.selectedContractIdList != []:
            contractDescr = getContractDescr(self.parent.selectedContractIdList[0])

            if not self.chkEventSetDate.isChecked():
                self.edtEventSetBegDate.setDate(contractDescr.begDate)
                self.edtEventSetEndDate.setDate(contractDescr.endDate)

            if not self.chkEventExecDate.isChecked():
                self.edtEventExecBegDate.setDate(contractDescr.begDate)
                self.edtEventExecEndDate.setDate(contractDescr.endDate)

            if not self.chkActionDate.isChecked():
                self.edtActionBegDate.setDate(contractDescr.begDate)
                self.edtActionEndDate.setDate(contractDescr.endDate)
    ...
    def on_btnExport_clicked(self):
        ...
        if self.addContractNumberToFileName:
            ...
            fileName = fileInfo.path()+'/'+fileInfo.baseName() + '_' + \
                        contractDescr.number + '.' +fileInfo.suffix()
    ...
    def on_btnSend_clicked(self):
        if self.writtenFileName:
            ...
            if filtersPage.chkActionStatus.isChecked():
                data['actStatus'] = CActionStatus.text(filtersPage.cmbActionStatus.value())
            else:
            ...
            contractNumberList = []
            for x in self.parent.selectedContractIdList:
                contractDescr = getContractDescr(x)
                contractNumberList.append(forceString(contractDescr.number))
