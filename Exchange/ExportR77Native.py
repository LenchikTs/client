...
from PyQt4.QtCore import Qt, QDate, QDir, QFile, QIODevice, QString, QTextStream, pyqtSignature, SIGNAL

from library.dbfpy.dbf import Dbf
from library.Utils     import forceBool, forceDate, forceDouble, forceInt, forceRef, forceString, forceStringEx, formatShortName, pyDate, toVariant, trim

from Orgs.Utils import getOrganisationInfo

from Exchange.Ui_ExportR77NativePage1 import Ui_ExportPage1
...
def getAccountInfo(accountId):
    ...
    if accountRecord:
        ...
        payerId = forceRef(accountRecord.value('payerId'))
    else:
        date = None
        number = ''
        orgCode = ''
    ...
    return date, number, fileName,  dirName,  financeCode,  contractNum, \
        contractDate,  orgName,  orgAddr,  payerId
...
class CExportR77NativeWizard(QtGui.QWizard):
    ...
    def setAccountId(self, accountId):
        ...
        date,  number, fileName,  dirName, financeCode, contractNum, \
            contractDate, orgName, orgAddr, payerId = getAccountInfo(accountId)
...
class CExportPage1(QtGui.QWizardPage, Ui_ExportPage1):
    def __init__(self, parent):
        ...
        self.chkGroupByMKB.setChecked(
            forceBool(QtGui.qApp.preferences.appPrefs.get(
...
#        accDate = self.accDate if self.accDate else QDate.currentDate()

        if self.financeCode == '3': #ДМС
            policyType_id = forceRef(db.translate('rbPolicyType', \
                'code', '3',  'id'))
            policyFilter = 'P.policyType_id = %d' % policyType_id
        else:
            policyFilter = ''
        ...
        stmt = """SELECT Client.id AS clientId,
                ActualPolicy.serial AS actualPolicySerial,
                ActualPolicy.number AS actualPolicyNumber,
                BackupPolicy.serial AS backupPolicySerial,
                BackupPolicy.number AS backupPolicyNumber,
                ...
                IF(Action.person_id IS NOT NULL,actionPerson.regionalCode,eventPerson.regionalCode) AS personCode,
                IF(Action.person_id IS NOT NULL,vrbActionPerson.name, vrbEventPerson.name) AS personName,
                Account_Item.serviceDate AS execDate,
                IF(Action.MKB != '', Action.MKB, Diagnosis.MKB) AS MKB,
                IF(Action.MKB != '', ActionMKB.DiagName, MKB.DiagName) AS diagName,
                ...
                rbAttachType.name AS attachName
            FROM Account_Item
            ...
            LEFT JOIN ClientPolicy AS ActualPolicy ON
                      ActualPolicy.id = (SELECT MAX(AP.id)
                                         FROM   ClientPolicy AS AP
                                         WHERE  AP.client_id = Client.id
                                            AND Account_Item.serviceDate BETWEEN AP.begDate AND AP.endDate
                                            AND AP.insurer_id = %d
                                            %s)
            LEFT JOIN ClientPolicy AS BackupPolicy ON
                      BackupPolicy.id = (SELECT MAX(BP.id)
                                         FROM   ClientPolicy AS BP
                                         WHERE  BP.client_id = Client.id
                                            AND Account_Item.serviceDate BETWEEN BP.begDate AND BP.endDate
                                            %s)
            LEFT JOIN ClientPolicy ON
                      ClientPolicy.id = (SELECT MAX(CP.id)
                                         FROM   ClientPolicy AS CP
                                         WHERE  CP.client_id = Client.id %s)
            ...
            LEFT JOIN MKB AS ActionMKB ON ActionMKB.DiagID = Action.MKB
            ...
            LEFT JOIN ClientAttach ON ClientAttach.client_id = Client.id AND ClientAttach.id = (
              SELECT MAX(CA.id) FROM ClientAttach AS CA
            ...
            LEFT JOIN vrbPerson AS vrbActionPerson ON actionPerson.id = vrbActionPerson.id
            LEFT JOIN vrbPerson AS vrbEventPerson ON eventPerson.id = vrbEventPerson.id
            WHERE
        ...
        """ % (self.payerId, 'AND A%s' % policyFilter if policyFilter else policyFilter,
                                    'AND B%s' % policyFilter if policyFilter else policyFilter,
                                    'AND C%s' % policyFilter if policyFilter else policyFilter,
                                    accSysFilter,
                                    self.accDate.toString(Qt.ISODate),
                                    self.accDate.toString(Qt.ISODate),
    ...
    def process(self, dbf, txt, appendixStream, record):
        ...
        policySerial  = forceString(record.value('actualPolicySerial'))
        policyNumber = forceString(record.value('actualPolicyNumber'))

        if not policySerial and not policyNumber:
            policySerial  = forceString(record.value('backupPolicySerial'))
            policyNumber = forceString(record.value('backupPolicyNumber'))

        if not policySerial and not policyNumber:
            policySerial  = forceString(record.value('policySerial'))
            policyNumber = forceString(record.value('policyNumber'))

        dbfRecord['SPOLIS'] = policySerial    # Серия полиса
        dbfRecord['NPOLIS'] = policyNumber   # Номер полиса
        ...
        dbfRecord['KHYS'] = forceInt(record.value('personCode')) % 10**3 #  Врач
...
"""
    ...
    def writeText2Header(self,  txt2Stream):
        ...
        dateStr = forceString(self.accDate.toString('MM.yy'))
        header = (u'%s'% orgInfo.get('title')).center(76)+ '\n\n'
        header += (u'Перечень медицинских услуг').center(76)+ '\n'
...
Пациент %s %s %s
полис серия %10s N %12s место работы %24s
N истории болезни %s
с об'емом медицинской помощи %s
""" % (
            identifier,
            forceString(record.value('lastName')).upper(),
            forceString(record.value('firstName')).upper(),
            forceString(record.value('patrName')).upper(),
            forceString(policySerial),
            forceString(policyNumber),
            workName,
            identifier,
            forceString(record.value('attachName'))
        )
        txtStream << header
...
class CExportPage2(QtGui.QWizardPage, Ui_ExportPage2):
    def __init__(self, parent):
        ...
        exportDir = forceString(QtGui.qApp.preferences.appPrefs.get('R77NativeExportDir', homePath))
