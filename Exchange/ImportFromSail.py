...
from library.Utils import anyToUnicode, exceptionToUnicode, forceInt, forceRef, forceString, forceStringEx, toVariant

from Exchange.Cimport import Cimport
#from Exchange.DBFFormats import get_SailOrgStructure_fields, get_SailPerson_fields
from Exchange.Utils  import getId

from Exchange.Ui_ImportFromSailDialog import Ui_ImportFromSailDialog


def ImportFromSail():
    dlg = CImportFromSail()
    dlg.edtFileNameOrgStructure.setText(forceString(QtGui.qApp.preferences.appPrefs.get('ImportOSFromSail', '')))
    dlg.edtFileNamePerson.setText(forceString(QtGui.qApp.preferences.appPrefs.get('ImportPFromSail', '')))
    dlg.exec_()
    ...
    def importPerson(self):
        ...
        try:
            if self.grpImportPerson.isChecked():
                ...
                dbfSailPerson = Dbf(dbfFileName, readOnly=True, encoding='cp1251')
...
#                dbfFields = dbfSailPerson.fieldNames
                for i, row in enumerate(dbfSailPerson):
                    ...
                    getId(tablePerson, personFields, personFields2)
            db.commit()
        except:
            db.rollback()
            QtGui.qApp.logCurrentException()
