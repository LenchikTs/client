...
class CXmlStreamReader(QXmlStreamReader):
    ...
    def readData(self):
        ...
        while not self.atEnd():
            ...
            if self.hasError() or self.isParentAborted():
                break
    ...
    def readUnknownElement(self, report=True):
        """Читает неизвестный элемент, и сообщает об этом, если report==True."""
    ...
    def isParentAborted(self):
        return ((hasattr(self._parent,'aborted') and self._parent.aborted)
                or (hasattr(self._parent, 'abort') and self._parent.abort))
