...
def exportR08EmergencyV200(widget, accountId, accountItemIdList, _):
...
class CR08ExportWizard(COrder79ExportWizard):
    ...
    def __init__(self, parent=None):
        title = u'Мастер экспорта реестра услуг для Калмыкии, СМП V173'
        prefix = 'R08EmergencyV173'
...
class CR08ExportPage1(CExportPage1):
    ...
    def exportInt(self):
        ...
        params['isReexposed'] = self.chkReexposed.isChecked()
        params['onkologyInfo'] = self.getOnkologyInfo()
        self.setProcessParams(params)
        CExportPage1.exportInt(self)


    def getTotalSum(self):
        return self.processParams()['accSum']
    ...
    def prepareStmt(self, params):
        ...
        select = u"""FirstEvent.id AS eventId,
            ...
            rbDiseaseCharacter.code AS SL_C_ZAB,
            ...
            (SELECT SI.value FROM rbService_Identification SI
                ...
                AND SI.deleted = 0
                AND SI.system_id = '{sysIdTfoms08V001}'
        ...
        """.format(sysIdTfoms08V001=self._getAccSysIdByUrn('urn:tfoms08:V001'))
...
class R08XmlStreamWriter(XmlStreamWriter):
    ...
    def writeService(self, record, params):
        ...
        serviceType = forceInt(onkologyInfo.get('ONK_USL_USL_TIP', 0))
        if serviceType in (1, 3, 4):
            params['USL_IDSERV'] += 1
    ...
    def writeCompleteEvent(self, record, params):
        ...
        patrName = forceString(record.value('PERS_OT'))
        noPatrName = not patrName or patrName.upper() in (
            u'НЕТ', u'НЕТ ОТЧЕСТВА')
        ...
        if noPatrName:
            local_params['Z_SL_OS_SLUCH'] = '2'
        elif  params['isJustBorn'] and not params['isAnyBirthDoc']:
           local_params['Z_SL_OS_SLUCH'] = '1'
        else:
            local_params['Z_SL_OS_SLUCH'] = ''
...
if __name__ == '__main__':
    ...
    testAccountExport(exportR08EmergencyV200,
                      configFileName=u'75_48_s11.ini',
                      #accNum=1836,
                      eventIdList=[160284]
                      )
