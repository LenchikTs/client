...
from library.Utils import calcAgeTuple, forceDate, forceInt, forceRef, forceString, forceStringEx, nameCase, toVariant, trim, unformatSNILS

from Events.Utils import checkEventPosibility, countUsageDiagnosis, createDiagnosisRecord, createVisits, findDiagnosisRecord, getEventPeriod, modifyDiagnosisRecord, updateDiagnosisRecord
from Exchange.Cimport import CDBFimport, Cimport
from Exchange.Utils import currentYearBeg, currentYearEnd, dbfCheckNames, get_dop, getId, get_specs, isNotNull, isNull, tbl

from Exchange.Ui_Import131 import Ui_Dialog



def Import131(widget):
    dlg = CImport131(widget)
    dlg.edtFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get('Import131FileName',  '')))
    dlg.exec_()
...
class CImport131(QtGui.QDialog, Ui_Dialog, CDBFimport):
    ...
    def on_btnSelectFile_clicked(self):
        ...
        if fileName != '':
            ...
            dbf131 = Dbf(dbfFileName, readOnly=True, encoding='cp866')
            self.labelNum.setText(u'всего записей в источнике: %d' % len(dbf131))
    ...
    def processPerson(self, personName, specialityId, personCode, personDate, personSl, numDiags, dbfFieldPrefix):
        if not str(personDate).replace('#','').strip():
            personDate=None
...
#            if self.newFormat: self.nbad+=1; return
        ...
        self.get_ClientAddressId(self.get_field('NAS_P'), self.get_field('UL'), self.get_field('DOM'), self.get_field('KOR'), self.get_field('KV'), self.get_field('ADRES_TYPE'), ADRES)
