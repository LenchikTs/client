...
from PyQt4.QtCore import Qt, QDir, QFile, QStringList, QTemporaryFile, QXmlStreamReader, pyqtSignature, SIGNAL
...
from Exchange.Ui_ImportNSI_Wizard_1 import Ui_ImportNSI_Wizard_1
from Exchange.Ui_ImportNSI_Wizard_2 import Ui_ImportNSI_Wizard_2
from Exchange.Ui_ImportNSI_Wizard_3 import Ui_ImportNSI_Wizard_3


def ImportNSI():
    dlg = CImportNSI(forceString(QtGui.qApp.preferences.appPrefs.get('ImportNSIDirName', '')),
        fullLog = forceBool(QtGui.qApp.preferences.appPrefs.get('ImportNSIFullLog', 'False')),
        importAll = forceBool(QtGui.qApp.preferences.appPrefs.get('ImportNSIImportAll', 'False')))
    dlg.exec_()
    QtGui.qApp.preferences.appPrefs['ImportNSIDirName'] = toVariant(dlg.dirName)
    QtGui.qApp.preferences.appPrefs['ImportNSIFullLog'] = toVariant(dlg.fullLog)
    QtGui.qApp.preferences.appPrefs['ImportNSIImportAll'] = toVariant(dlg.importAll)
...
#            print u'Неизвестный элемент: %s' % self.name().toString()
...
class CImportNSIWizardPage2(QtGui.QWizardPage, Ui_ImportNSI_Wizard_2):
        ...
        try:
            ...
            for x in self.parent.listFiles:
                ...
                if info:
                    ...
                    eventDateItem.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                    eventNameItem.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                    eventOIDItem.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                    eventVerItem.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
                    eventAuthorItem.setFlags(Qt.ItemIsSelectable | Qt.ItemIsEnabled)
    ...
    def getNSIMetaInfo(self,  fileName):
        ...
        if is_zipfile(fileName):
            ...
            sharedStringsFile.open(QFile.ReadOnly | QFile.Text) # fixit
            ...
            metadataSheet.open(QFile.ReadOnly | QFile.Text) # fixit
...
#                metadata = SheetParser.sheetData
                ...
                dataSheet.open(QFile.ReadOnly | QFile.Text) # fixit
...
#                author = sharedString[ref] if ref else ""
                ...
                if self.aborted:
                    ...
                    self.logBrowser.append(u'! Ошибка: файл %s, %s' % (fileName, CSheetParser.errorString()))
...
def extractToTempFile(zipFile,  fileToExtract):
    ...
    outFile = QTemporaryFile()

    if not outFile.open(QFile.WriteOnly):
