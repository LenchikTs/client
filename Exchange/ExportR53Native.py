...
from PyQt4.QtCore import Qt, QDate, QDir, QFile, QString, QTextCodec, QXmlStreamReader, QXmlStreamWriter, pyqtSignature, SIGNAL

from library.dbfpy.dbf import Dbf

from library.Utils import anyToUnicode, exceptionToUnicode, firstMonthDay, forceBool, forceDate, forceDouble, forceInt, forceRef, forceString, forceStringEx, formatSNILS, lastMonthDay, pyDate, toVariant, trim
from Exchange.Utils import compressFileInZip
from Exchange.Export import CExportHelperMixin

from Exchange.Ui_ExportR53NativePage1 import Ui_ExportPage1
    ...
    def getFullDbfFileName(self):
        ...
        if not self.dbfFileName:
            ...
            if self.page1.cmbExportType.currentIndex() == CExportPage1.exportTypeDMS:
                ...
                year = forceString(exposeDate.year() if exposeDate else date.year())
...
# *****************************************************************************************
    ...
    def exportInt(self):
        ...
        if exportType == self.exportTypeTFOMS:
            begDate = self.edtBegDate.date()
            endDate = self.edtEndDate.date()
            if begDate and endDate and endDate<begDate:
...
#        strContractNumber = forceString(QtGui.qApp.db.translate('Contract', 'id', contractId , 'number')) if contractId else u'б/н'
        ...
        if self.idList:
            if exportType in (self.exportTypeTFOMS, self.exportTypeInternal,
                ...
                if exportType == self.exportTypeServiceInfo:
                    out.writeFileHeader(file, aDate)
                else:
...
# *****************************************************************************************
    ...
    def process(self, dbf, record, codeLPU, accNumber,  accDate, lpuOKATO, payerOKATO):
        ...
        dbfRecord['STAT_P'] = forceInt(representativeInfo.get('regionalRelationTypeCode'))
        if age < 14:
        ...
        dbfRecord['PRVS'] = forceString(self.getPersonSpecialityRegionalCode(forceRef(record.value('execPersonId'))))
...
class CExportPage2(QtGui.QWizardPage, Ui_ExportPage2):
    def __init__(self, parent):
        ...
        exportDir = forceString(QtGui.qApp.preferences.appPrefs.get('R53ExportDir', homePath))
...
#    <SN_DOC>Серия и номер документа, удостоверяющего личность</SN_DOC>
        ...
        self.writeTextElement('DATE_DOC', forceDate(record.value('documentDate')).toString(Qt.ISODate))
        ...
        if age < 1 and documentType != '3' and info != {}:
            ...
            relationTypeCode= info.get('regionalRelationTypeCode', '0')
            # Дата рождения родителя (представителя) пациента
            representativeBirthDate  = info.get('birthDate').toString(Qt.ISODate)
...
#    <S_ALL>Выставленная сумма к оплате (Стоимость оказанной медицинской помощи)</S_ALL>
    ...
    def writeHeader(self, senderName, senderOGRN, insurerName, insurerOGRN,  begDate,
        ...
        self.writeTextElement('DATE_S', accDate.toString(Qt.ISODate))
        self.writeEndElement()
...
#            self.writeTextElement('DR_P', forceString(record.value('birthPlace')), writeEmtpyTag=False)
        ...
        self.writeTextElement('DATE_2', forceDate(record.value('endDate')).toString(Qt.ISODate))
        # Предварительный основной диагноз врача ответственного за событие
...
#        self.writeTextElement('CODE_MES2', '')
        ...
        self.writeTextElement('PRVS', forceString(self.parent.getPersonSpecialityFederalCode(forceRef(record.value('execPersonId')))))
...
class CServiceInfoXmlStreamWriter(QXmlStreamWriter):
    def __init__(self, parent):
        QXmlStreamWriter.__init__(self)
        self.parent = parent
        self.setAutoFormatting(True)
