...
from PyQt4.QtCore import Qt, SIGNAL, pyqtSignature, QString, QFile, QXmlStreamReader
...
class CMyXmlStreamReader(QXmlStreamReader):
    ...
    fieldsUslGroup = ('CODE', 'NAME', 'USL_KAT', 'DATEBEG', 'DATEEND')
    fieldsUsl = ('USL_OK', 'RAZDEL', 'CODE',  'NAME', 'DATEBEG', 'DATEEND')
    ...
    coordUsl = {'code':'CODE', 'name':'NAME', 'begDate':'DATEBEG', 'endDate':'DATEEND', 'medicalAidType_id':'USL_OK', 'group_id': 'RAZDEL'}
    coordDop = {'code':'DOP_CODE', 'name':'DOP_NAME', 'begDate':'DATEBEG', 'endDate':'DATEEND', 'adultUetDoctor': 'KOL_USL', 'infis':'DOP_CODE'}
    coordPG = {'code':'P_CODE', 'name':'P_NAME', 'sex':'sex', 'age':'age'}
    ...
    def retranslateNamePacientGroup(self, name):
        sex = age = ''
        name = unicode(name)
        name = self.transformPG(name, ' ', '')
        firstFields = name.split('.')
        if name in self.sexDict.keys():
            sex = self.sexDict[name]
            age = ''
        elif len(firstFields) == 2 and str(firstFields[0]) in self.sexDict.keys():
        ...
        if age != '':
            age = self.retransformPG(age, '[0-9][^a-z][0-9][^a-z]')
        ...
        while (not self.atEnd()):
            ...
            if self.isStartElement():
                if (self.name() == "PG"):
                    ...
                    idx = self.lookupServiceElement(table, result['P_CODE'], result['P_NAME'])
            ...
            if self.isStartElement():
                ...
                #    stmt = 'Select FROM rbService LEFT JOIN rbServiceGroup ON rbServiceGroup.id = rbService.group id WHERE'
                    ...
                    for dop in dopCode:
                        ...
                        result['RAZDEL'] =  '9'
                        name = forceString(dop.value('name')) + ' -' +result['DOP_NAME'][:500]
        ...
        while (not self.atEnd()):
            ...
            if self.isStartElement():
                if (self.name() == "USL"):
                    ...
                    self.log(u'Элемент: %s (%s)' %(name, code))
                    id = None
                    idx = self.lookupServiceElement(self.table, code, name)
                    for id in idx:
                        if id:
    ...
    def getServiceDifference(self, table, oldRecordId, newRecordDict, dict, flag = None, code = None, name = None):
        ...
        for fieldName in dict.keys():
                ...
                oldFieldName = toVariant(fieldName)
                if fieldName=='group_id':
                        oldValue = toVariant(self.db.translate('`rbServiceGroup`', 'id', oldValue, 'code'))
                if flag:
                    if (newRecordDict['DET'] == '1') and fieldName == 'adultUetDoctor':
                        fieldName='childUetDoctor'
                        oldValue = oldRecord.value(fieldName)
                        newValue = toVariant(newRecordDict.get('KOL_USL'))
                    elif code and fieldName in ('code', 'infis'):
                        newValue =code
                    elif name and fieldName == 'name':
                        newValue =name
                    else:
                ...
                if not variantEq(oldValue, newValue): # or  not variantEq(toVariant(fieldName), oldFieldName):
                    ...
                    parts.append(u'%s было %s стало %s' % (self.trans[fieldName], forceString(oldValue), forceString(newValue)))
...
#        self.lookupAid('Type', forceString(newRecordDict.get('USL_OK')))
            ...
            if self.isStartElement():
                ...
                if result != [{}]:
                    for element in  result:
                        code = element['CODE']
                        name = element['NAME']
                        stmt = "SELECT `name` FROM `rbServiceGroup` WHERE `code` = '%s' and `name` = '%s'"
                        query = self.db.query(stmt%(code,name[:64]))
                        if query.next():
                            pass
                        else:
    ...
    def readRefElement(self):
        code = forceString(self.attributes().value('USL_OK').toString())
...
class CImportRbService(QtGui.QDialog, Ui_ImportRbServiceR29_1):
