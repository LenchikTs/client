    ...
    def resolved(self, klass):
        ...
        if resolved is not None:
            if isinstance(resolved, klass):
                return resolved
            logger.warning("Referenced resource {0} is not a {1} but a {2}".format(refid, klass, resolved.__class__))
            return None
            ...
            for contained in owning_resource.contained:
                if contained.id == refid:
                    ...
                    if isinstance(contained, klass):
                        return contained
                    logger.warning("Contained resource {0} is not a {1} but a {2}".format(refid, klass, contained.__class__))
                    return None
                ...
                for entry in bundle.entry:
                    if entry.fullUrl == fullUrl:
                        ...
                        if isinstance(found, klass):
                            return found
                        logger.warning("Bundled resource {0} is not a {1} but a {2}".format(refid, klass, found.__class__))
                        return None
        ...
        if server is None:
            logger.warning("Not implemented: resolving absolute reference to resource {0}"
