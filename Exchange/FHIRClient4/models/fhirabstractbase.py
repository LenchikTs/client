...
class FHIRAbstractBase(object):
    ...
    def with_json(cls, jsonobj):
        ...
        raise TypeError("`with_json()` on {0} only takes dict or list of dict, but you provided {1}"
    ...
    def _with_json_dict(cls, jsondict):
        ...
        if not isinstance(jsondict, dict):
            raise TypeError("Can only use `_with_json_dict()` on {0} with a dictionary, got {1}"
    ...
    def update_with_json(self, jsondict):
        ...
        if not isinstance(jsondict, dict):
            raise FHIRValidationError("Non-dict type {0} fed to `update_with_json` on {1}"
            ...
            if value is not None:
                ...
                if is_list:
                    if not isinstance(value, list):
                        err = TypeError("Wrong type {0} for list property \"{1}\" on {2}, expecting a list of {3}"
                ...
                if testval is not None and not self._matches_type(testval, typ):
                    err = TypeError("Wrong type {0} for property \"{1}\" on {2}, expecting {3}"
        ...
        if len(nonoptionals) > 0:
            for miss in nonoptionals - found:
                errs.append(KeyError("Non-optional property \"{0}\" on {1} is missing"
                    .format(miss, self)))
        
        # were there superfluous dictionary keys?
        if len(set(jsondict.keys()) - valid) > 0:
            for supflu in set(jsondict.keys()) - valid:
                errs.append(AttributeError("Superfluous entry \"{0}\" in data for {1}"
                    .format(supflu, self)))
    ...
    def as_json(self):
        ...
        for name, jsname, typ, is_list, of_many, not_optional in self.elementProperties():
            ...
            if is_list:
                if not isinstance(value, list):
                   err = TypeError("Expecting property \"{0}\" on {1} to be list, but is {2}"
                       .format(name, type(self), type(value)))
                elif len(value) > 0:
                    if not self._matches_type(value[0], typ):
                        err = TypeError("Expecting property \"{0}\" on {1} to be {2}, but is {3}"
        ...
        if len(nonoptionals - found) > 0:
            for nonop in nonoptionals - found:
                errs.append(KeyError("Property \"{0}\" on {1} is not optional, you must provide a value for it"
                    .format(nonop, self)))
