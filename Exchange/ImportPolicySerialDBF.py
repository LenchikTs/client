...
from Exchange.Ui_ImportPolicySerialDBF import Ui_Dialog


def ImportPolicySerialDBF(widget):
    dlg=CImportPolicySerialDBF(widget)
    dlg.edtFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get('ImportPolicySerialDBFFileName',  '')))
    dlg.exec_()
...
class CImportPolicySerialDBF(QtGui.QDialog, Ui_Dialog, CDBFimport):
    def __init__(self, parent):
        ...
        self.parent = parent #wWTF? скрывать Qt-шный parent - это плохая затея
...
#        db = QtGui.qApp.db
        ...
        dbfPolicySerial = Dbf(dbfFileName, readOnly=True, encoding='cp866')
    ...
    def processTariff(self,  row):
        ...
        if typeCode not in ('',' ', u'т', u'п', u'д'):
            self.log.append(u'<b><font color=orange>ОШИБКА:</b> '\
