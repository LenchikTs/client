...
from PyQt4.QtCore import QDate, QDir, pyqtSignature, SIGNAL

from KLADR.KLADRModel  import getStreetName, getCityName
from library.dbfpy.dbf import Dbf
from library.Utils     import calcAgeInYears, checkHouse, forceBool, forceDate, forceDouble, forceInt, forceRef, forceString, forceStringEx, formatSex, nameCase, pyDate, toVariant, trim

from Registry.Utils import getAttachRecord

from Exchange.Ui_ExportLOFOMSPage1 import Ui_ExportLOFOMSPage1
from Exchange.Ui_ExportLOFOMSPage2 import Ui_ExportLOFOMSPage2
from Exchange.Ui_ExportLOFOMSPage3 import Ui_ExportLOFOMSPage3
...
class CExportWizard(QtGui.QWizard):
    ...
    def setAccountId(self, accountId):
        ...
        self.page3.setTitle(u'Укажите директорий для сохранения обменных файлов "*.dbf"')
...
class CExportLOFOMSPage1(QtGui.QWizardPage, Ui_ExportLOFOMSPage1):
    def __init__(self, parent):
        ...
        actionTypeCytological = forceRef(QtGui.qApp.preferences.appPrefs.get('ExportLOFOMSActionTypeCytological', None))
        actionTypeFluorography = forceRef(QtGui.qApp.preferences.appPrefs.get('ExportLOFOMSActionTypeFluorography', None))
        if actionTypeCytological is None:
            actionTypeCytological = self.lookupActionTypeByName(u'цитологическ')
        if actionTypeFluorography is None:
    ...
    def lookupActionTypeByName(self, name):
        ...
        record = db.getRecordEx(tableActionType, 'id', tableActionType['name'].contain(name))
        if record:
    ...
    def validatePage(self):
        return self.cmbActionTypeCytological.value() is not None and \
                    self.cmbActionTypeFluorography.value() is not None
...
#'12', #'Прием с выездом на село'
    ...
    def __init__(self, parent):
        ...
        self.tariffCache = {}
        self.parent = parent #wtf? скрывать Qt-шный parent - это плохая затея
        self.ignoreErrors = forceBool(QtGui.qApp.preferences.appPrefs.get('ExportLOFOMSIgnoreErrors', False))
        self.chkVerboseLog.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('ExportLOFOMSVerboseLog', 'False')))
    ...
    def exportInt(self):
        ...
        if currentOrgINN.startswith('47'):  # областное мед.учреждение
...
#        eventExecPersonCode = forceInt(record.value('personCode'))
        ...
        if docType == '13':
            if regKLADRCode.startswith('78'):
                seriaLet = u'СП'
            elif regKLADRCode.startswith('47'):
                seriaLet = u'ЛО'
            else:
                seriaLet = u' '
        else:
...
#            dbfRecord['MS_Type'] = '0'
        ...
        if age == 0:
            clientCategory = '012' # Ребенок до года
        elif children == '1':
            clientCategory = '011' # Подросток
        else:
...
class CExportLOFOMSPage3(QtGui.QWizardPage, Ui_ExportLOFOMSPage3):
    def __init__(self, parent):
        ...
        exportDir = forceString(QtGui.qApp.preferences.appPrefs.get('LOFOMSExportDir', homePath))
    ...
    def on_btnSelectDir_clicked(self):
        dir = QtGui.QFileDialog.getExistingDirectory(self,
                u'Выберите директорий для сохранения файла выгрузки в ЛОФОМС',
