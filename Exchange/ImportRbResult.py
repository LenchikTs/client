...
from PyQt4.QtCore import Qt, QDir, QFile, QVariant, QXmlStreamReader, pyqtSignature, SIGNAL

from library.Utils import anyToUnicode, exceptionToUnicode, forceBool, forceInt, forceRef, forceString, toVariant

from Exchange.ExportRbResult import rbResultFields
...
def ImportRbResult(parent):
    dlg = CImportRbResult(forceString(QtGui.qApp.preferences.appPrefs.get('ImportRbResultFileName', '')),
        fullLog = forceBool(QtGui.qApp.preferences.appPrefs.get('ImportRbResultFullLog', 'False')),
        importAll = forceBool(QtGui.qApp.preferences.appPrefs.get('ImportRbResultImportAll', 'False')),
        parent=parent)
    dlg.exec_()
...
class CMyXmlStreamReader(QXmlStreamReader):
    ...
    def readResult(self,  filterList,  makeList):
        ...
        if id:
            ...
            if not self._parent._parent.page(0).rbSkip.isChecked():
                if not self.isCoincidentResult(result,  id):
                    ...
                    if self._parent._parent.page(0).rbAskUser.isChecked():
                        self.log(u' Запрос действия пользователя: ')
                        if QtGui.QMessageBox.question(self._parent, u'Записи различаются',
                    ...
                    elif self._parent._parent.page(0).rbUpdate.isChecked():
                        self.log(u' - обновляем')
                        self.updateRbResult(result,  id)
            else:
                self.log(u' - пропускаем')
        ...
        self.nprocessed += 1
        self._parent.statusLabel.setText(
...
class CImportRbResultWizardPage1(QtGui.QWizardPage, Ui_ImportRbResult_Wizard_1):
    ...
    def doPreImport(self):
        inFile = QFile(self._parent.fileName)
        if not inFile.open(QFile.ReadOnly | QFile.Text):
            QtGui.QMessageBox.warning(self, u'Импорт справочника "Результаты События"',
                                      ...
                                      (self._parent.fileName, inFile.errorString()))
            return False
        else:
            self.progressBar.reset()
            self.progressBar.setValue(0)
            ...
            if (myXmlStreamReader.readFile(inFile,  None,  True)):
                ...
                self._parent.elementsList = myXmlStreamReader.elementsList
                self.statusLabel.setText(u'Найдено %d элементов для импорта' % \
                                                    len(self._parent.elementsList))
            else:
                self.progressBar.setText(u'Прервано')
                if self.aborted:
                    ...
                    self.logBrowser.append(u'! Ошибка: файл %s, %s' % \
                        (self._parent.fileName, myXmlStreamReader.errorString()))
                return False
    ...
    def on_btnSelectFile_clicked(self):
        self._parent.fileName = QtGui.QFileDialog.getOpenFileName(
            self, u'Укажите файл с данными', self.edtFileName.text(), u'Файлы XML (*.xml)')
        if self._parent.fileName != '':
            self.edtFileName.setText(QDir.toNativeSeparators(self._parent.fileName))
...
class CImportRbResultWizardPage2(QtGui.QWizardPage, Ui_ImportRbResult_Wizard_2):
    ...
    def isComplete(self):
        return self._parent.importAll or self._parent.selectedItems != []


    def initializePage(self):
        self.tblEvents.setRowCount(len(self._parent.elementsList))
        ...
        for x in self._parent.elementsList:
    ...
    def on_chkImportAll_toggled(self,  checked):
        self._parent.importAll = checked
        ...
        if checked:
            ...
            self.statusLabel.setText(u'Выбрано %d элементов для импорта' % \
                                        len(self._parent.selectedItems))
    ...
    def on_tblEvents_itemSelectionChanged(self):
        ...
        for i in rows:
            ...
            self._parent.selectedItems.append((code, name,  purpose))
...
class CImportRbResultWizardPage3(QtGui.QWizardPage, Ui_ImportRbResult_Wizard_3):
    ...
    def doImport(self):
        ...
        if not inFile.open(QFile.ReadOnly | QFile.Text):
            ...
            self.progressBar.setFormat(u'%v байт')
            self.progressBar.setMaximum(max(inFile.size(), 1))
            myXmlStreamReader = CMyXmlStreamReader(self,  self._parent.fullLog)
            if (myXmlStreamReader.readFile(inFile,  self._parent.selectedItems)):
                self.progressBar.setText(u'Готово')
                ...
                if self.aborted:
                    ...
                    self.logBrowser.append(u'! Ошибка: файл %s, %s' % (self._parent.fileName, myXmlStreamReader.errorString()))
