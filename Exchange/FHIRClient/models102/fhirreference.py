    ...
    def resolved(self, klass):
        ...
        if resolved is not None:
            if isinstance(resolved, klass):
                return resolved
            logging.warning("Referenced resource {0} is not a {1} but a {2}".format(refid, klass, resolved.__class__))
            return None
            ...
            for contained in owning_resource.contained:
                if contained.id == refid:
                    ...
                    if isinstance(contained, klass):
                        return contained
                    logging.warning("Contained resource {0} is not a {1} but a {2}".format(refid, klass, contained.__class__))
                    return None
        ...
        if '://' not in self.reference:
            ...
            logging.warning("Reference owner {0} does not have a server, cannot resolve relative reference {1}"
                .format(self._owner, self.reference))
            return None
        
        # absolute resource
        logging.warning("Not implemented: resolving absolute reference to resource {0}"
