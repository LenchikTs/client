...
class FHIRAbstractBase(object):
    ...
    def _with_json_dict(cls, jsondict):
        if not isinstance(jsondict, dict):
            raise Exception("Cannot use this method with anything but a JSON dictionary, got {0}"
                .format(jsondict))
        return cls(jsondict)
    ...
    def update_with_json(self, jsondict):
        ...
        if len(nonoptionals - found) > 0:
            for miss in nonoptionals - found:
                logging.warning("Non-optional property '{0}' on {1} is missing from JSON"
                    .format(miss, self))
        ...
        if superfluous:
            for supflu in superfluous:
                logging.warning("Superfluous entry '{0}' in JSON for {1}"
                    .format(supflu, self))
    ...
    def as_json(self):
        ...
        if len(nonoptionals - found) > 0:
            for nonop in nonoptionals - found:
                logging.warning("Element '{0}' is not optional, you should provide a value for it on {1}"
                    .format(nonop, self))
        return js
