...
set Organisation.modifyDatetime = now(),
    ...
    Organisation.smoCode = tmpSMO.FED_CODE,
    Organisation.tfomsExtCode = tmpSMO.id,
    ...
    Organisation.area = '7800000000000',
...
and (Organisation.area = ''
    or Organisation.smoCode != tmpSMO.FED_CODE
    or Organisation.tfomsExtCode != tmpSMO.id
...
-- поле ID_HISTORY не используется - поэтому используем его для пометки о добавлении
update tmpSMO
set ID_HISTORY = 5
where not exists (
    select fullName from Organisation
    where tmpSMO.INN = Organisation.INN and tmpSMO.CODE = Organisation.infisCode);
...
select 0 as id,
       ...
       0 as deleted,
        ...
        tmpSMO.id as tfomsExtCode,
        '' as miacCode,
        tmpSMO.FED_CODE as smoCode,
        '' as usishCode,
       ...
       PHONE as phone,
       '' as accountant,
       '7800000000000' as area,
       0 as compulsoryServiceStop,
       0 as voluntaryServiceStop,
