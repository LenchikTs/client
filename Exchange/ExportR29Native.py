...
from library.Utils import exceptionToUnicode, forceBool, forceDate, forceDouble, forceInt, forceRef, forceString, forceStringEx, formatSNILS, toVariant, trim

from Events.Action import CAction
from Exchange.Export import CExportHelperMixin
from Exchange.Utils import compressFileIn7Zip

from Exchange.Ui_ExportR29NativePage1 import Ui_ExportPage1
...
class CExportR29NativeWizard(QtGui.QWizard):
    ...
    def setAccountId(self, accountId):
        ...
        self.page2.setTitle(u'Укажите директорий для сохранения обменного файла')
...
class CExportPage1(QtGui.QWizardPage, Ui_ExportPage1, CExportHelperMixin):
    ...
    '1122': u'ДД9', #терапевт
    '1126': u'ДД10', #хирург
    '1112': u'ДД11', #офтальмолог
    '1109': u'ДД12', #невролог
    '1101': u'ДД13', #акушер-гинеколог
    }

    dsSpecialityMap = {
    '1134': u'ДС8', #педиатр
    '1109': u'ДС9', #невролог
    '1112': u'ДС10', #офтальмолог
    '1135': u'ДС11', #детский хирург
    '1111': u'ДС12', #оториноларинголог
    '1101': u'ДС13', #акушер-гинеколог
    '140102': u'ДС14', #детский стоматолог
    '1123': u'ДС15', #ортопед-травматолог
    '1115': u'ДС16', #психиатр
    '113502': u'ДС17', #детский уролог-андролог
    '112702': u'ДС18', #детский эндокринолог1
    '113402': u'ДС18', #детский эндокринолог2
    }
    ...
    def __init__(self, parent):
        ...
        self.chkVerboseLog.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('ExportR29NativeVerboseLog', False)))
        self.chkExportDD.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('ExportR29NativeExportDD', False)))
        self.chkExportDS.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('ExportR29NativeExportDS', False)))
        self.chkEventSplit.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('ExportR29EventSplit', False)))
        ddEventTypesStr=forceString(QtGui.qApp.preferences.appPrefs.get('ExportR29NativeEventTypeDD', '1,12,25,27,26'))
        dsEventTypesStr=forceString(QtGui.qApp.preferences.appPrefs.get('ExportR29NativeEventTypeDS', '2,28'))
        self.currentOrgCode = ''
    ...
    def prepareActionTypeGroups(self):
        ...
        if not self.actionTypeGroup3:
            record = self.db.getRecordEx('ActionType', 'id', 'code=\'1-3\' and class=1')
            if record:
                self.actionTypeGroup3 = forceInt(record.value(0)) # id функциональной диагностики
    ...
    def prepareForDD(self):
        ...
        ddAnalysisTypes = [
            ...
            (u'ДД27', '07', self.actionTypeGroup3), # ЭКГ
...
# *****************************************************************************************
    ...
    def createQuery(self):
        ...
        stmt = """SELECT
                ...
                Employment.code AS employmentCode,
            ...
            LEFT JOIN ClientPolicy ON ClientPolicy.client_id = Client.id AND
                      ClientPolicy.id = (SELECT MAX(CP.id)
                                         ...
                                         WHERE  CP.client_id = Client.id
                                            AND (Event.setDate BETWEEN CP.begDate AND IF(CP.endDate IS NOT NULL, CP.endDate, '9000-1-1')
                                             OR Event.execDate BETWEEN CP.begDate AND IF(CP.endDate IS NOT NULL, CP.endDate, '9000-1-1'))
                                            AND %s)
    ...
    def process(self, txt,  record):
        ...
        if representativeInfo:
            result[u'СПП'] = representativeInfo['regionalRelationTypeCode']
...
#Выполнять нижеследующие проверки только если отсутствует полис
#Исправить sql-запрос
#Ну, с б-гмом
        if not forceString(record.value('policyNumber')):
        ...
        category = forceString(record.value('employmentCode'))

        catRes = ''

        if category == '5' or category == '8':
            catRes = u'Р'
...
#        medicalAidTypeCode = forceString(record.value('medicalAidTypeCode'))[:2]
        ...
        #Если соц. статус - работающий или пенсионер-раб, то ВМП == 14
        #В противном случае - как раньше
...
#        medicalAidProfileCode = forceString(record.value('medicalAidProfileCode'))
        ...
        # 10 Арабские цифры по формату дд/мм/гггг. Разделитель "/"
        # Обязательное поле
        result[u'ДНЛ'] = forceDate(record.value('begDate')).toString('dd/MM/yyyy')
        # Дата окончания лечения
        #10 Арабские цифры по формату дд/мм/гггг. Разделитель "/"
        # Обязательное поле
        result[u'ДКЛ'] = forceDate(record.value('endDate')).toString('dd/MM/yyyy')
...
#            result[u'ЧТЕ'] = u'%3.2f' % (forceDouble(record.value('amount'))*forceDouble(record.value('uet')))
            ...
            if actionId:
                ...
                if bedId:
                    ...
                    if bedProfileId:
                        ...
                        if bedServiceId:
                            result[u'КМП'] = forceString(self.db.translate('rbService', 'id', bedServiceId, 'infis'))[:5]
        ...
        if result[u'КВД'] and (result[u'КВД'] not in ('1','2')):
            result[u'ОСС'] += '2'
    ...
    def writeTextHeader(self,  txtStream):
        ...
        txtStream << u'ЗАГ:%s,%s,%d,1,%s,1\n' % \
            (lpuCode.zfill(8), orgCode.zfill(8), self.registryType, date.toString('dd/MM/yyyy'))
...
# *****************************************************************************************
    ...
    def getSOCR(self, code):
        ...
        if code != '':
            ...
            result = forceString(QtGui.qApp.db.translate('kladr.KLADR', 'CODE', code.ljust(13, '0'), 'SOCR'))
...
class CExportPage2(QtGui.QWizardPage, Ui_ExportPage2):
    def __init__(self, parent):
        ...
        exportDir = forceString(QtGui.qApp.preferences.appPrefs.get('R29NativeExportDir', homePath))
