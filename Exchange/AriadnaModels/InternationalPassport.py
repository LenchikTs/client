...
class InternationalPassport(AbstractObject):
    def __init__(self, jsondict=None):
        """ Initialize all valid properties."""
        self.documentType = 'INTERNATIONAL_PASSPORT'  # String Обязательно Тип документа
        ...
        super(InternationalPassport, self).__init__(jsondict)

    def elementProperties(self):
        """ Returns a list of tuples, one tuple for each property that should
        be serialized, as: ("name", "json_name", type, is_list, "of_many", not_optional)
        """
        js = super(InternationalPassport, self).elementProperties()
