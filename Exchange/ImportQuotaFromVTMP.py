...
from PyQt4.QtCore import QDate, QDateTime, QVariant, pyqtSignature

from library.Utils import forceDate, forceInt, forceString, forceStringEx, nameCase, toVariant

from Exchange.Cimport import Cimport
from Exchange.Utils  import check_rus, check_rus_lat, getId, tbl
from Registry.ClientEditDialog import CClientEditDialog
from Users.Rights import urAdmin, urRegTabWriteRegistry, urRegTabReadRegistry

from Exchange.Ui_ImportQuotaFromVTMP import Ui_ImportQuotaFromVTMPDialog
...
def ImportQuotaFromVTMP(parent):
    ...
    dlg.edtFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get('ImportQuotaFromVTMPFileName', '')))
    dlg.exec_()
...
#        self.progressBar.setValue(n-1)
    ...
    def getClientDocumentId(self, info, clientId):
        fullDocument = info['clientDocument'].replace(u'№', ' ') if info['clientDocument'] else None
        if fullDocument:
            serial = ''
            number = ''
            i = 0
