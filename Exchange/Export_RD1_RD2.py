...
from Exchange.Ui_ExportRD2_Wizard_1 import Ui_ExportRD2_Wizard_1
from Exchange.Ui_ExportRD2_Wizard_2 import Ui_ExportRD2_Wizard_2
from Exchange.Ui_PervDoc import Ui_PervDocDialog
...
class CMyWizardPage1(QtGui.QWizardPage, Ui_ExportRD2_Wizard_1):
    def __init__(self, parent, pervDoc, period=None, weekNumber=None):
        ...
        if self.parent.useXML:
            ...
            self.cmbExportFormat.addItem(u'РД1 2011', toVariant('RD7'))
...
WHERE Account.deleted = 0 AND Account.id=%d''' % accountId
    ...
    def export(self):
        ...
        try:
            ...
            self.dbf=Dbf(dbfFileName, new=True, encoding='cp866')
            dbfRD2=self.dbf
            dbfFields=[]
                        ...
                        q_from+=u'''
                            ...
                            LEFT JOIN ClientPolicy ON ClientPolicy.client_id = Event.client_id
                                    AND ClientPolicy.id = (
                                        ...
                                        WHERE CP.deleted = 0 AND CP.client_id = Event.client_id AND (CP.policyType_id IS NULL OR rbPolicyType.isCompulsory))
...
#                            set_field('COMMENT', '[%s]' % Account_ItemId)
            ...
            if self.checkRAR.isChecked():
                self.parent.ext='.rar'
                prg=QtGui.qApp.execProgram('rar a -ep "%s" "%s"'% (rarFileName, dbfFileName))
                if not prg[0]:
                    self.parent.rarname=''
...
class Export_RD1_RD2(QtGui.QWizard):
    def __init__(self, AccountItemsIdList, accountIdList, parent=None, pervDoc=False, period=None, weekNumber=None, useXML=False, XMLDialog=None):
        ...
        self.rec_num=0
        self.tmp_dir=unicode(tempfile.mkdtemp('','samson_mail_'), locale.getpreferredencoding())
        self.dbf=None
        self.dbf_recs=None
...
#        text0+=u'получатель: '+self.recipient_name+'\n'
        ...
        table.setText(i, 3, QDateTime.currentDateTime().toString('dd.MM.yyyy HH:mm:ss'))
...
def exposeByEvents(progressDialog, eventIdList, contractDescr, accountId, accountItemIdList):
    totalAmount = 0.0
    totalSum    = 0.0

    def exposeEvent(contractDescr, accountId, eventId):
        db = QtGui.qApp.db
    ...
    def exposeEventAsCoupleVisit(contractDescr, accountId, eventId):
        db = QtGui.qApp.db
        event = db.getRecord('Event', 'id, eventType_id, payStatus', eventId)
        eventTypeId = forceRef(event.value('eventType_id'))
        totalAmount = 0.0
    ...
    for eventId in eventIdList:
        progressDialog.step()
        amount, sum = exposeEvent(contractDescr, accountId, eventId)
        totalAmount += amount
        totalSum += sum
        amount, sum = exposeEventAsCoupleVisit(contractDescr, accountId, eventId)
        totalAmount += amount
        totalSum += sum
    return totalAmount, totalSum


def exposeByVisitsByActionServices(progressDialog, visitIdList, contractDescr, accountId, accountItemIdList):
    totalAmount = 0.0
    totalSum    = 0.0

    def exposeVisitByActionServices(contractDescr, accountId, serviceId, visitId):
        db = QtGui.qApp.db
...
#        serviceId = forceRef(visit.value('service_id'))
        ...
        for serviceId, visitIdList in mapServiceIdToVisitIdList.items():
            for visitId in visitIdList:
                progressDialog.step()
                amount, sum = exposeVisitByActionServices(contractDescr, accountId, serviceId, visitId)
                totalAmount += amount
                totalSum += sum
    return totalAmount, totalSum


def exposeByVisits(progressDialog, visitIdList, contractDescr, accountId, accountItemIdList):
    totalAmount = 0.0
    totalSum    = 0.0

    def exposeVisit(contractDescr, accountId, visitId):
        db = QtGui.qApp.db
        visit = db.getRecord('Visit', 'id, event_id, date, service_id, payStatus', visitId)
        serviceId = forceRef(visit.value('service_id'))
        totalAmount = 0.0
    ...
    for visitId in visitIdList:
        progressDialog.step()
        amount, sum = exposeVisit(contractDescr, accountId, visitId)
        totalAmount += amount
        totalSum += sum
    return totalAmount, totalSum

def exposeByActions(progressDialog, actionIdList, contractDescr, accountId, accountItemIdList):
    totalAmount = 0.0
    totalSum    = 0.0

    def exposeAction(contractDescr, accountId, actionId):
        db = QtGui.qApp.db
    ...
    for actionId in actionIdList:
        progressDialog.step()
        amount, sum = exposeAction(contractDescr, accountId, actionId)
        totalAmount += amount
        totalSum += sum
    return totalAmount, totalSum

def reExpose(progressDialog, idList, contractDescr, orgStructureId, date, accountId, accountRecord, accountItemIdList):
    db = QtGui.qApp.db

    if idList and not accountId:
        accountId, accountRecord = createAccount(
            contractDescr, QtGui.qApp.currentOrgId, orgStructureId, date, reexpose=True)
    ...
    for oldId in idList:
        ...
        updateDocsPayStatus(oldAccountItem, contractDescr.payStatusMask, CPayStatus.exposedBits)
...
def getAccountList(parent, dialog, contractIdList):
    ...
    try:
        ...
        for contractId in contractIdList:
            ...
            eventIdList = selectEvents(contractDescr, None, date)
            mapServiceIdToVisitIdList = selectVisitsByActionServices(contractDescr, None, date)
            visitIdList = selectVisits(contractDescr, None, date)
            actionIdList = selectActions(contractDescr, None, date)
            if perevystavl:
                reexposableIdList = selectReexposableAccountItems(contractDescr,  date)
            else:
                reexposableIdList = []
            ...
            accountId, accountRecord = createAccount (contractDescr, QtGui.qApp.currentOrgId, None, date)
            accountIdList.append(accountId)
            totalAmount = 0.0
            totalSum = 0.0
            amount, s = exposeByEvents(progressDialog, eventIdList, contractDescr, accountId, accountItemIdList)
            totalAmount += amount
            totalSum += s
            amount, s = exposeByVisitsByActionServices(progressDialog, mapServiceIdToVisitIdList, contractDescr, accountId, accountItemIdList)
            totalAmount += amount
            totalSum    += s
            amount, s = exposeByVisits(progressDialog, visitIdList, contractDescr, accountId, accountItemIdList)
            totalAmount += amount
            totalSum += s
            amount, s = exposeByActions(progressDialog, actionIdList, contractDescr, accountId, accountItemIdList)
            totalAmount += amount
            totalSum += s
            if perevystavl:
#                reexposableIdList = selectReexposableAccountItems(contractDescr)
                accountItemIdList+=reexposableIdList
                accountId, accountRecord, amount, s = reExpose(
                    progressDialog, reexposableIdList, contractDescr, QtGui.qApp.currentOrgId, date, accountId, accountRecord, accountItemIdList)
                accountIdList.append(accountId)
                totalAmount += amount
                totalSum += s
...
class CPervDocDialog(CDialogBase, Ui_PervDocDialog):
    ...
    def on_selectionModelContracts_currentChanged(self, current, previous):
        ...
        if contractIdList:
            ...
            contractDescr = getContractDescr(contractId)
            self.edtBegDate.setDate(contractDescr.begDate)
            self.edtEndDate.setDate(contractDescr.endDate)
