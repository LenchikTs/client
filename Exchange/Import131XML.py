...
from library.Utils import calcAgeTuple, forceBool, forceDate, forceInt, forceRef, forceString, forceStringEx, nameCase, toVariant, trim, unformatSNILS

from Events.Utils import checkEventPosibility, createVisits, getEventPeriod
from Exchange.Cimport import CXMLimport
from Exchange.Import131 import TIP_DD_b, TIP_DD_r, TIP_DD_v, get_DiagnosisId2
from Exchange.Utils import check_rus_lat, currentYearBeg, currentYearEnd, get_dop, getId, isNotNull, isNull, tbl

from Exchange.Ui_Import131XML import Ui_Dialog


def Import131XML(widget):
    dlg = CImport131XML(widget)
    dlg.edtFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get('Import131XMLFileName',  '')))
    dlg.checkFed.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLFed',  True)))
    dlg.checkShortLPU.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLShortLPU',  True)))
    dlg.checkShortWork.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLShortWork',  True)))
    dlg.checkAttachOGRN.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLAttachOGRN', False)))
    dlg.checkDate.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLcurrentYearOnly', True)))
    dlg.checkTIP_DD_R.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLcardTypeR', False)))
    dlg.checkTIP_DD_V.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLcardTypeV', False)))
    dlg.checkNoOwn.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLskipOwn', True)))
    dlg.checkOGRN2.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get('Import131XMLuseAsOwnOGRN', False)))
    dlg.edtOGRN2.setText(forceString(QtGui.qApp.preferences.appPrefs.get('Import131XMLownOGRN',  '')))
    dlg.exec_()
...
#        n=0
    ...
    def processPerson(self, personName, specialityId, personCode, personDate, personSl, numDiags, dbfFieldPrefix):
        if not str(personDate).replace('#',''):
            personDate=None
        if isinstance(personDate, basestring):
        ...
        for iDiag in range(1, numDiags+1):
            ...
            if len(diag)==4:
                ...
                diag=diag[0:5]
            if diag and not diag.startswith('Z'):
...
#            if diag.startswith('Z') and characterId:
#                pass
...
#        return dop
    ...
    def getEventId(self, noAdd=False):
        ...
        if self.eventType and self.datemax:
            ...
            nextEventDate=QDate(self.datemax).addMonths(getEventPeriod(period, singleInPeriod))
            org_id = self.eventOrgId
            if org_id:
                canCreateEvent, EventId = checkEventPosibility(
                    self.clientId, eventTypeId, None, QDate(self.datemin))
...
#        clientPolicy_id=None
        ...
        if len(polis)==2:
            self.get_clientPolicy_id(self.clientId, polis[0], polis[1], insurerId)
        else:
...
#                if (lpuId!=self.my_org_id or TIP_DD not in TIP_DD_b) and attachType:
        ...
        self.DZ_MKB=None
        if self.dbf_DZ_MKB:
            self.DZ_MKB=self.get_field('DZ_MKB').upper()
            if self.DZ_MKB.startswith('Z'):
                self.DZ_MKB=None
        self.DiagList=[]
        self.dateList=[]
...
#                self.dateList.append(DP)
    ...
    def get_MKB_char(self, diag):
        ...
        MKBchar=forceInt(self.db.translate('MKB', 'DiagID', diag, 'characters'))
        ...
        if factors:
            ...
            for factor in factors:
                if factor:
                    ...
                    if factorType_id:
                        Hurt_FactorFields=[
                            ('master_id', Hurt_id), ('factorType_id', factorType_id)]
                        getId(self.tableClientWork_Hurt_Factor, Hurt_FactorFields)
    ...
    def getClientId(self, lastNameField, firstNameField, patrNameField, sexField, birthDateField):
        ...
        if sex == u'М':
            ...
            bad=True
            self.err2log(u'пол не указан или указан неверно')
