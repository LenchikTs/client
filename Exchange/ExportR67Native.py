...
from PyQt4.QtCore import QDate, QDir, QFile, QIODevice, QString, QTextStream, pyqtSignature, SIGNAL

from library.AmountToWords import amountToWords
from library.dbfpy.dbf import Dbf
from library.Utils     import forceBool, forceDate, forceDouble, forceInt, forceRef, forceString, forceStringEx, pyDate, toVariant, trim
...
from Exchange.Ui_ExportR67NativePage1 import Ui_ExportPage1
...
class CExportPage1(QtGui.QWizardPage, Ui_ExportPage1):
    ...
    def __init__(self, parent):
        ...
        self.chkExportFormatAliens.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get(
            'ExportR67ExportFormatAliens', 'False')))
        self.chkSkipZeroSum.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get(
            'ExportR67SkipZeroSum', 'True')))
        self.chkNumerateRecId.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get(
    ...
    def prepareToExport(self, aliens):
        ...
        query = self.createQuery(self._parent.contractId)
...
# *****************************************************************************************
    ...
    def createTxt(self):
        txt = QFile(os.path.join(self._parent.getTmpDir(), self.getTxtFileName()))
    ...
    def createPDbf(self):
        ...
        dbfName = os.path.join(self._parent.getTmpDir(), 'P'+self.getDbfBaseName())
    ...
    def createSDbf(self):
        ...
        dbfName = os.path.join(self._parent.getTmpDir(), 'S'+self.getDbfBaseName())
        dbf = Dbf(dbfName, new=True, encoding='cp866')
        dbf.addField(
            # Номер соответствующей записи из реестра пролеченных пациентов в основной базе ЛПУ
            ('RECID','C', 6),
...
# *****************************************************************************************
    ...
    def createCDbf(self):
        ...
        dbfName = os.path.join(self._parent.getTmpDir(), 'C'+self.getDbfBaseName())
        dbf = Dbf(dbfName, new=True, encoding='cp866')
        dbf.addField(
    ...
    def createIDbf(self):
        ...
        dbfName = os.path.join(self._parent.getTmpDir(), 'I'+self.getDbfBaseName())
        dbf = Dbf(dbfName, new=True, encoding='cp866')
        dbf.addField(
...
# *****************************************************************************************
    ...
    def createQuery(self, contractId):
        ...
        stmt = """SELECT
                ...
                IF (Contract_Tariff.tariffType IN ('%d','%d','%d'),1,0) AS isMes
            FROM Account_Item
            ...
            AND %s
        """ % (CTariff.ttVisitsByMES, CTariff.ttEventByMES, CTariff.ttEventByMESLen,
...
# *****************************************************************************************
    ...
    def createServiceQuery(self):
        ...
        stmt = """SELECT
                ...
                IF(Contract_Tariff.tariffType IN ('%d','%d','%d'),1,0) AS isMES1,
                ...
                SUM(IF (HospitalAction.id IS NOT NULL AND rbMedicalAidType.code != '7' AND Contract_Tariff.tariffType NOT IN ('%d','%d','%d'),Account_Item.`sum`,0)) AS hospSum,
                SUM(IF (HospitalAction.id IS NOT NULL AND rbMedicalAidType.code = '7' AND Contract_Tariff.tariffType NOT IN ('%d','%d','%d'),Account_Item.`sum`,0)) AS dayHospSum,
                SUM(IF (HospitalAction.id IS NOT NULL AND rbMedicalAidType.code != '7' AND Contract_Tariff.tariffType IN ('%d','%d','%d'),Account_Item.`sum`,0)) AS hospMesSum,
                SUM(IF (HospitalAction.id IS NOT NULL AND rbMedicalAidType.code = '7' AND Contract_Tariff.tariffType IN ('%d','%d','%d'),Account_Item.`sum`,0)) AS dayHospMesSum,
                SUM(IF (HospitalAction.id IS NOT NULL,0,Account_Item.`sum`)) AS ambSum,
                SUM(IF (HospitalAction.id IS NOT NULL AND Contract_Tariff.tariffType NOT IN ('%d','%d','%d') AND Account_Item.`sum` != 0,
                ...
                SUM(IF (HospitalAction.id IS NOT NULL AND Contract_Tariff.tariffType IN ('%d','%d','%d'),Account_Item.`amount`,0)) AS mesBedDays,
                COUNT(DISTINCT IF (Contract_Tariff.tariffType IN ('%d','%d','%d'),Event.id,NULL)) AS mesCount,
                SUM(IF (Contract_Tariff.tariffType IN ('%d','%d','%d'),Account_Item.`sum`,NULL)) AS mesSum,
                SUM(IF (Contract_Tariff.tariffType IN ('%d','%d','%d'),
                    ...
                    SUM(IF (HospitalAction.id IS NOT NULL AND rbMedicalAidType.code != '7' AND Contract_Tariff.tariffType NOT IN ('%d','%d','%d'),LEAST(IF(Contract_Tariff.federalLimitation = 0,
                            Account_Item.amount,
                            LEAST(Contract_Tariff.federalLimitation, Account_Item.amount)) * Contract_Tariff.federalPrice,
                ...
                SUM(IF (HospitalAction.id IS NOT NULL AND rbMedicalAidType.code != '7' AND Contract_Tariff.tariffType IN ('%d','%d','%d'),
    ...
    def processAliens(self, dbf, record, codeLPU, contractNumber, contractDate,  lpuCategory,  tariffFactor, lpuOKPO):
        ...
        if not(clientId in self.exportedClients):
            ...
            if info != {}:
                ...
                r = info.get('regionalRelationTypeCode', 0)
...
# *****************************************************************************************
    ...
    def processServices(self,  txtStream,  record):
        if self._parent.page1.chkSkipZeroSum.isChecked() and forceDouble(record.value('sum')) == 0:
            return
        ...
        fmt = u'%6.6s│%60.60s│%19.19s│%16.16s│%6.6s│%11.11s\n'
...
""" % (orgName, self.accDate.toString('dd/MM/yyyy'), self.accDate.toString(u'MMMM yyyy').toLower(),
    ...
    def writeTextFooter(self,  txtStream):
        self.writeOrgStructureFooter(txtStream)
        delimeter = u'+-------------------+------+------+------+------+------+----------+-----------+-----+------+----------+-----------+------+-----------+\n'
        fmt = u'|%19.19s|%6d|%6d|%6d|%6d|%6d|%10.2f|%11.2f|%5.2f|%6d|%10.2f|%11.2f|%6d|%11.2f|\n'
        totalClientCount = 0
...
#        totalServiceCount = 0
        ...
        orgChief = forceString(QtGui.qApp.db.translate('Organisation', \
            'id',  QtGui.qApp.currentOrgId(), 'chiefFreeInput'))
        ...
        for (key, val) in self.orgStuctStat.iteritems():
            if key != 'total':
                ...
                if sum != 0:
                    self.appendixStream << fmt % (key.ljust(19), clientCount,
...
""" % (orgName, self.accDate.toString(u'MMMM yyyy').toLower(),
        ...
        txtStream << fmt % (u'Итого'.ljust(19), totalClientCount,
...
class CExportPage2(QtGui.QWizardPage, Ui_ExportPage2):
    def __init__(self, parent):
        ...
        exportDir = forceString(QtGui.qApp.preferences.appPrefs.get('R67ExportDir', homePath))
    ...
    def validatePage(self):
        ...
        prefixes = ('I', 'C') if self._parent.page1.chkExportFormatAliens.isChecked() else ('P', 'S')
        for src in prefixes:
            srcFullName = os.path.join(forceStringEx(self._parent.getTmpDir()),
        ...
        srcFullName = os.path.join(forceStringEx(self._parent.getTmpDir()),
                                                self._parent.page1.getTxtFileName())
        dst = os.path.join(forceStringEx(self.edtDir.text()),
                                                self._parent.page1.getTxtFileName())
        success, result = QtGui.qApp.call(self, shutil.move, (srcFullName, dst))
