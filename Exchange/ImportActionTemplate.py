...
from library.Utils      import anyToUnicode, exceptionToUnicode, forceBool, forceRef, forceString, formatSex, toVariant

from Events.Action import CAction, CActionPropertyType

from Events.ActionPropertiesTable import CActionPropertiesTableModel
from Exchange.ExportActionTemplate import actionTemplateFields,  actionFields, actionRefFields, actionPropertyTypeFields,  unitFields,   actionTypeFields,  actionPropertyTemplateFields, ownerFields
from Exchange.Utils import tbl

from Exchange.Ui_ImportActionProperty import Ui_ImportActionProperty
from Exchange.Ui_ImportActionTemplate_Wizard_1 import Ui_ImportActionTemplate_Wizard_1
from Exchange.Ui_ImportActionTemplate_Wizard_2 import Ui_ImportActionTemplate_Wizard_2
from Exchange.Ui_ImportActionTemplate_Wizard_3 import Ui_ImportActionTemplate_Wizard_3


actionTemplateRefFields = ('group_id',  'owner_id', 'speciality_id')

def ImportActionTemplate():
    dlg = CImportActionTemplate(forceString(QtGui.qApp.preferences.appPrefs.get('ImportActionTemplateFileName', '')),
        fullLog = forceBool(QtGui.qApp.preferences.appPrefs.get('ImportActionTemplateFullLog', 'False')),
        importAll = forceBool(QtGui.qApp.preferences.appPrefs.get('ImportActionTemplateImportAll', 'False')))
    dlg.exec_()
    QtGui.qApp.preferences.appPrefs['ImportActionTemplateFileName'] = toVariant(dlg.fileName)
    QtGui.qApp.preferences.appPrefs['ImportActionTemplateFullLog']  = toVariant(dlg.fullLog)
    QtGui.qApp.preferences.appPrefs['ImportActionTemplateImportAll']= toVariant(dlg.importAll)
...
#        purpose = None
        ...
        try:
            ...
            self.log(u'!<b><font color=red>ОШИБКА<\font><\b>'
                        u' работы с БД: "%s"'  % exceptionToUnicode(e),  True)
        ...
        if actionType:
            ...
            if typeId:
                ...
                self.log(u'<b><font color=red>ОШИБКА<\font><\b>:'
                 ...
                 (actionType['name'], actionType['typeName']),  True)
    ...
    def isCoincidentActionProperty(self, property,  id):
        ...
        if id:
            ...
            extValue =  forceString(actionPropertyType.getValue(id))
    ...
    def addActionProperty(self,  property,  actionId):
        ...
        if typeId and type:
            ...
            typeRecord = self.db.getRecord('ActionPropertyType', '*', property.get('type_id',  None))
            actionPropertyType = CActionPropertyType(typeRecord)
            ...
            if typeName != '':
                ...
                t = tbl(actionPropertyType.getValueTableName())
...
class CActionPropertyReplaceDialog(CDialogBase, Ui_ImportActionProperty):
    ...
    def setActionId(self):
        self.setAction(CAction.getActionById(self.actionId))
