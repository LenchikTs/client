...
class CFssSignInfo:
    ...
    _moLnActorUriFmt  = '%(baseUri)s/mo/%(ogrn)s/%(lnCode)s'
    ...
    def getTreatPeriodElementIdAndActorUri(cls, lnCode, zeroBasedIndex):
        if not 0<=zeroBasedIndex<=2:
            raise Exception(u'Л.Н. не может иметь элемент %s с индексом %r' % ('TREAT_PERIOD', zeroBasedIndex))
    ...
    def getTreatFullPeriodElementIdAndActorUri(cls, lnCode, zeroBasedIndex):
        if not 0<=zeroBasedIndex<=2:
            raise Exception(u'Л.Н. не может иметь элемент %s с индексом %r' % ('TREAT_FULL_PERIOD', zeroBasedIndex))
        args = {  'baseUri': cls._baseUri, 'lnCode': lnCode, 'num': 3+zeroBasedIndex }
