...
where 1 -- length(code) = 9 -- это условие нужно??????????????????
...
update tmpC, rbService_Contents
left join rbService sGroup on sGroup.id = rbService_Contents.master_id
left join rbService sService on sService.id = rbService_Contents.service_id
set tmpC.rbServiceGroup_id = rbService_Contents.id,
...
insert into rbService_Contents(master_id, service_id, required)
select  sGroup.id as master_id,
...
delete from rbService_Contents
where master_id = 0
or service_id = 0;
...
--where rbService.id = rbService_Contents.master_id
--)
--and not exists
--(select * from tmpC where tmpC.rbServiceGroup_id = rbService_Contents.id
