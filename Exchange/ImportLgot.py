...
from PyQt4.QtCore import Qt, QDate, QDir, QFileInfo, pyqtSignature

from library.dbfpy.dbf import Dbf
from library.DbfViewDialog import CDbfViewDialog
from library.Utils import forceBool, forceDate, forceInt, forceRef, forceString, forceStringEx, nameCase, toVariant, unformatSNILS

from Exchange.Cimport import CDBFimport, Cimport

from Exchange.Utils import check_rus_lat, dbfCheckNames, get_dom_korp_kv, getId, tbl
from Registry.Utils import selectLatestRecord

from Exchange.Ui_ImportLgot import Ui_Dialog


def ImportLgot(widget):
    dlg = CImportLgot(widget)
    dlg.edtFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get('ImportLgotFileName',  '')))
    dlg.edtPersonalDataFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get(
        'ImportBenefitPersonalDataFileName',  '')))
    dlg.edtDocumentDataFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get(
        'ImportBenefitDocumentDataFileName',  '')))
    dlg.chkFederalBenefit.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get(
        'ImportBenefitTypeFederal', True)))
    dlg.chkRegionalBenefit.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get(
        'ImportBenefitTypeRegional', False)))
    dlg.chkAddMissingClients.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get(
        'ImportBenefitAddMissingClients', False)))
    dlg.chkAddDocumentType.setChecked(forceBool(QtGui.qApp.preferences.appPrefs.get(
...
class CImportLgot(QtGui.QDialog, Ui_Dialog, CDBFimport):
    ...
    def on_btnSelectDocumentDataFile_clicked(self):
        ...
        if fileName != '':
            self.edtDocumentDataFileName.setText(QDir.toNativeSeparators(fileName))
            self.checkName()
            self.updateLabelNum()
    ...
    def on_btnSelectPersonalDataFile_clicked(self):
        ...
        if fileName != '':
            self.edtPersonalDataFileName.setText(QDir.toNativeSeparators(fileName))
            self.checkName()
    ...
    def updateLabelNum(self):
        ...
        if fileName:
            dbfLgot = Dbf(fileName, readOnly=True, encoding='cp866')
    ...
    def startImportEMSRN(self):
        ...
        dbfLgot = Dbf(dbfFileName, readOnly=True, encoding='cp866')
    ...
    def startImportFSS(self):
        ...
        dbfPersonalData = Dbf(dbfPersonalDataFileName, readOnly=True, encoding='cp866')

        dbfDocumentDataFileName = forceStringEx(self.edtDocumentDataFileName.text())
        dbfDocumentData = Dbf(dbfDocumentDataFileName, readOnly=True, encoding='cp866')
    ...
    def processDocumentDataRow(self,  row, federalBenefitInfo=None):
        ...
        if not socStatusTypeId:
            self.log.append(u'<b><font color=red>ОШИБКА</b>: для записи %d' \
                ...
                u' отсутствует в БД.' % (npp, socStatusTypeCode))
            return
...
#            return
        ...
        begDate = QDate.fromString(row['DATE_BL'], 'yyyy/MM/dd')
        endDate = QDate.fromString(row['DATE_EL'], 'yyyy/MM/dd')

        if not begDate:
            self.log.append(u'<b><font color=red>ОШИБКА</b>: для записи %d' \
                u' отсутствует дата начала действия НСУ.' % npp)
            return
    ...
    def processRow(self):
        ...
        NUMB_P=forceString(row['NUMB_P']).strip()
        number=NUMB_P.lstrip('0').zfill(6)
...
#                self.tableClientDocument['date'].eq(toVariant(DATE_VP))
            ...
            if ClientDocument:
                clientId=forceRef(ClientDocument.value('client_id'))
        ...
        if SNILS:
            getId(self.tableClient, [('id', clientId)], [('SNILS', SNILS)])
...
#        row=self.row
    ...
    def processPaspDoc(self, clientId, serial, number, documentType_id):
        ...
        getId(self.tableClientDocument, clientDocumentFields, clientDocumentFields2)
...
def formatSerial(SER_P):
    if SER_P.isdigit() and len(SER_P) == 4:
