...
from PyQt4.QtCore import Qt, QDateTime, QFile, QObject, QXmlStreamWriter, pyqtSignature, SIGNAL

from library.DialogBase import CConstructHelperMixin
from library.exception  import CException
from library.TableModel import CTableModel, CTextCol
from library.Utils      import anyToUnicode, forceBool, forceDate, forceInt, forceRef, forceString, toVariant

from Exchange.ExportEvents import checkPropertyList
from Exchange.Utils import compressFileInRar
from KLADR.KLADRModel import getCityName, getStreetName

#getStreetName
from RefBooks.Person.List import getPersonDocument,  getPersonAddress, selectLatestRecord
from Registry.Utils import getAddress

from Exchange.Ui_ExportHL7_Wizard_1 import Ui_ExportHL7_Wizard_1
from Exchange.Ui_ExportHL7_Wizard_2 import Ui_ExportHL7_Wizard_2


def ExportHL7():
    dirName = forceString(QtGui.qApp.preferences.appPrefs.get('ExportHL7DirName', ''))
    exportAll = forceBool(QtGui.qApp.preferences.appPrefs.get('ExportHL7ExportAll', False))
    compressRAR = forceBool(QtGui.qApp.preferences.appPrefs.get('ExportHL7CompressRAR', False))
    exportOnlyOwn = forceBool(QtGui.qApp.preferences.appPrefs.get('ExportHL7ExportOnlyOwn', True))
    dlg = CExportHL7(dirName, exportAll, compressRAR,  exportOnlyOwn)
    dlg.exec_()
    QtGui.qApp.preferences.appPrefs['ExportHL7ExportAll'] = toVariant(dlg.exportAll)
    QtGui.qApp.preferences.appPrefs['ExportHL7DirName'] = toVariant(dlg.dirName)
    QtGui.qApp.preferences.appPrefs['ExportHL7CompressRAR'] = toVariant(dlg.compressRAR)
    QtGui.qApp.preferences.appPrefs['ExportHL7ExportOnlyOwn'] = toVariant(dlg.exportOnlyOwn)
    ...
    def doExport(self):
        ...
        try:
            ...
            while (query.next()):
                ...
                if QFile.exists(fileName) and eventType == 2:
                    i = 2
