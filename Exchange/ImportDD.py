...
from PyQt4.QtCore import QDate, QString, pyqtSignature


from library.dbfpy.dbf import Dbf
from library.Utils import forceInt, forceString, toVariant

from Cimport import CDBFimport, Cimport
from Exchange.DBFFormats import get_DD_fields
from Exchange.Utils  import checkPatientData, dbfCheckNames, findAndUpdateOrCreateRecord, getId, getOKVED, isNotNull, isNull

from Exchange.Ui_ImportDD import Ui_Dialog


def ImportDD():
    dlg = CImportDD()
    dlg.edtFileName.setText(forceString(QtGui.qApp.preferences.appPrefs.get('ImportDDFileName', '')))
    dlg.exec_()
...
class CImportDD(QtGui.QDialog, Ui_Dialog, CDBFimport):
    ...
    def startImport(self):
        ...
        dbfDD       = Dbf(dbfFileName, readOnly=True, encoding='cp866')
        ...
        for row in dbfDD:
            ...
            self._getClientPolicyId(clientId, row['POLIS_SERI'], row['POLIS_NUMB'], insurerId, policyType_id, dependentGetId)
...
#            UNSTRUCT_A += '; '+forceString(row['TYPE_INF']) #test
            ...
            dependentGetId(self.tableClientAddress, clientAddressFields, clientAddressFields2)
...
#                    ('OKVED', OKVED)
                ...
                getId(self.tableClientWork, ClientWorkFields, ClientWorkFields2)
            #id LPU
            Organisation_id=None
