...
u"""Экспорт реестра в формате XML. Республика Калмыкия, стационар (Д1,4) V200"""
...
def exportR08HospitalV200(widget, accountId, accountItemIdList, _):
...
class CR08XmlStreamWriterCommon(XmlStreamWriter, CExportHelperMixin):
    ...
    def writeHeader(self, params):
        ...
        self.writeElement('SUMMAV', u'{0:.2f}'.format(self._parent.getTotalSum()))
    ...
    def writeRecord(self, record, params):
        ...
        if (clientId != self._lastClientId or
            ...
            params['isJustBorn'] = execDate.isValid() and params[
...
class CR08XmlStreamWriterD4(CR08XmlStreamWriterCommon):
    ...
    def writeHeader(self, params):
        ...
        self.writeElement('DSCHET', params['accDate'].toString(Qt.ISODate))
        self.writeElement('PLAT', params['payerCode'])
        self.writeElement('SUMMAV', u'{0:.2f}'.format(self._parent.getD4TotalSum()))
        self.writeEndElement() # SCHET
...
class CR08XmlStreamWriter(CR08XmlStreamWriterCommon):
    ...
    def writeRecord(self, record, params):
        ...
        if ((not onkInfo or not onkInfo.get('isOnkology')) and
                self._parent.registryType == CR08ExportPage1.registryTypeD1):
            CR08XmlStreamWriterCommon.writeRecord(self, record, params)
...
if __name__ == '__main__':
    ...
    testAccountExport(exportR08HospitalV200, u'08-ОСН-45/ОНКОмай(доп счет апрель)',
                      u's11_ROD.ini', [316695])
