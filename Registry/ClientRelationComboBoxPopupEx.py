...
from library.Utils      import forceDate, forceInt, forceString, forceRef, getPref, setPref, addDots
...
__all__ = [ 'CClientRelationComboBoxPopupEx',
          ]


class CClientRelationComboBoxPopupEx(QtGui.QFrame, Ui_ClientRelationComboBoxPopup):
...
#       но enter не работает...
    ...
    def getClientRelationIdList(self, lastName, firstName, patrName, birthDate, docTypeId, serial, number, forceCode, clientId):
        ...
        if lastName:
            condFilter.append(tableClient['lastName'].like(addDots(lastName)))
        if firstName:
            condFilter.append(tableClient['firstName'].like(addDots(firstName)))
        if patrName:
            condFilter.append(tableClient['patrName'].like(addDots(patrName)))
        if birthDate:
            condFilter.append(tableClient['birthDate'].eq(birthDate))
        if docTypeId:
            condFilter.append(tableDocument['documentType_id'].eq(docTypeId))
        if serial:
            condFilter.append(tableDocument['serial'].eq(serial))
        if number:
            condFilter.append(tableDocument['number'].eq(number))
        if condFilter and not clientId:
