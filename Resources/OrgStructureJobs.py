...
from PyQt4.QtCore import Qt, SIGNAL, QByteArray, QLocale, QMimeData, QModelIndex, QObject, QTime, QVariant
...
from library.InDocTable  import CRecordListModel, CInDocTableView, CIntInDocTableCol, CRBInDocTableCol, CTimeInDocTableCol
from library.Utils       import forceInt, forceRef



class COrgStructureJobsModel(CRecordListModel):
    def __init__(self, parent):
        ...
        self.addCol(CIntInDocTableCol(u'План',      'quantity', 5, low=0, high=999)).setToolTip(u'Плановое количество').setSortable()
    ...
    def setData(self, index, value, role=Qt.EditRole):
        ...
        if role == Qt.EditRole:
            ...
            if column == 0:
                row = index.row()
                jobTypeId = forceRef(self.value(row, 'jobType_id'))
                capacity = self.__getJobTypeDefaultCapacity(jobTypeId)
                self.setValue(row, 'capacity', capacity)
        return result
    ...
    def loadItems(self, orgStructureId, period, customLength):
        self.period = period
        self.customLength = customLength

        db = QtGui.qApp.db
        table = db.table(COrgStructureJobTemplate.tableName)
        records = db.getRecordList(table, '*', [table['master_id'].eq(orgStructureId),
                                               ],
                                        'day, begTime'
                                  )
        groupByDay = {}
        for record in records:
            item = COrgStructureJobTemplate(record)
    ...
    def getEmptyItem(self, day):
        result = COrgStructureJobTemplate()
        result.day = day
        result.capacity = 1
        return result


    def saveItems(self, orgStructureId):
        idList = []
        ...
        db = QtGui.qApp.db
        table = db.table(COrgStructureJobTemplate.tableName)
        db.deleteRecord(table, [table['master_id'].eq(orgStructureId),
    ...
    def __getJobTypeDefaultCapacity(self, jobTypeId):
        result = self.__mapJobTypeIdToCapacity.get(jobTypeId)
        if result is None:
            db = QtGui.qApp.db
            result = forceInt(db.translate('rbJobType', 'id',  jobTypeId, 'capacity'))
            self.__mapJobTypeIdToCapacity[jobTypeId] = result
        return result


class COrgSrtuctureJobsView(CInDocTableView):
    mimeType = 'application/x-s11/OrgStructureJobTemplateList'
