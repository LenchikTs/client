...
from PyQt4.QtCore import pyqtSignature, SIGNAL, QDate
...
#        self.setIsDirty(False)
        ...
        self._datetimeTaken    = QDate()

        self.actUncheckItems = QtGui.QAction(u'Снять отметки', self)
        self.connect(self.actUncheckItems, SIGNAL('triggered()'), self.on_uncheckItems)
    ...
    def saveProbes(self, takenTissueJournalId, resetTmpFilter=True):
        self.modelSamplePreparation.saveItems(takenTissueJournalId, resetTmpFilter)
    ...
    def automaticalSaveProbes(self, tissueJournalIdList):
        ...
        if dlg.exec_():
            ...
            for tissueJournalId in tissueJournalIdList:
                ...
                self.emit(SIGNAL('samplePreparationPassed()'))
                ...
                self.saveProbes(tissueJournalId, resetTmpFilter=True)
