...
class CModelPatientComboBox(CROComboBox):
    ...
    def updateText(self):
        db = QtGui.qApp.db
        tableRBModelPatient = db.table('rbPatientModel')
        record = db.getRecordEx(tableRBModelPatient, [tableRBModelPatient['code'], tableRBModelPatient['name']], [tableRBModelPatient['id'].eq(self.code)])
        text = ''
        if record:
