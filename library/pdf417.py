...
# text code mode change commands
PL = 25 # latch to PUNCTUATION
LL = 27 # latch to LOWER
AS = 27 # shift to ALPHA
ML = 28 # latch to MIXED
AL = 28 # latch to ALPHA
PS = 29 # shift to PUNCTUATION
PAL = 29 # latch to ALPHA from PUNCTUATION
...
charCodes = {'\t' :(12, 12),'\n' : ( 8, 15), '\r': (12, 11), ' ' : ( 7, 26), '!' : ( 8, 10),
              '"' : ( 8, 20), '#' : ( 4, 15), '$' : (12, 18), '%' : ( 4, 21), '&' : ( 4, 10),
              "'" : ( 8, 28), '(' : ( 8, 23), ')' : ( 8, 24), '*' : (12, 22), '+' : ( 4, 20),
              ',' : (12, 13), '-' : (12, 16), '.' : (12, 17), '/' : (12, 19), '0' : ( 4,  0),
              '1' : ( 4,  1), '2' : ( 4,  2), '3' : ( 4,  3), '4' : ( 4,  4), '5' : ( 4,  5),
              '6' : ( 4,  6), '7' : ( 4,  7), '8' : ( 4,  8), '9' : ( 4,  9), ':' : (12, 14),
              ';' : ( 8,  0), '<' : ( 8,  1), '=' : ( 4, 23), '>' : ( 8,  2), '?' : ( 8, 25),
              '@' : ( 8,  3), 'A' : ( 1,  0), 'B' : ( 1,  1), 'C' : ( 1,  2), 'D' : ( 1,  3),
              'E' : ( 1,  4), 'F' : ( 1,  5), 'G' : ( 1,  6), 'H' : ( 1,  7), 'I' : ( 1,  8),
              'J' : ( 1,  9), 'K' : ( 1, 10), 'L' : ( 1, 11), 'M' : ( 1, 12), 'N' : ( 1, 13),
              'O' : ( 1, 14), 'P' : ( 1, 15), 'Q' : ( 1, 16), 'R' : ( 1, 17), 'S' : ( 1, 18),
              'T' : ( 1, 19), 'U' : ( 1, 20), 'V' : ( 1, 21), 'W' : ( 1, 22), 'X' : ( 1, 23),
              'Y' : ( 1, 24), 'Z' : ( 1, 25), '[' : ( 8,  4), '\\': ( 8,  5), ']' : ( 8,  6),
              '^' : ( 4, 24), '_' : ( 8,  7), '`' : ( 8,  8), 'a' : ( 2,  0), 'b' : ( 2,  1),
              'c' : ( 2,  2), 'd' : ( 2,  3), 'e' : ( 2,  4), 'f' : ( 2,  5), 'g' : ( 2,  6),
              'h' : ( 2,  7), 'i' : ( 2,  8), 'j' : ( 2,  9), 'k' : ( 2, 10), 'l' : ( 2, 11),
              'm' : ( 2, 12), 'n' : ( 2, 13), 'o' : ( 2, 14), 'p' : ( 2, 15), 'q' : ( 2, 16),
              'r' : ( 2, 17), 's' : ( 2, 18), 't' : ( 2, 19), 'u' : ( 2, 20), 'v' : ( 2, 21),
              'w' : ( 2, 22), 'x' : ( 2, 23), 'y' : ( 2, 24), 'z' : ( 2, 25), '{' : ( 8, 26),
              '|' : ( 8, 21), '}' : ( 8, 27), '~' : ( 8,  9)}
...
def breakString(data):
    u'функция разбивает строку на сегменты, каждый сегмент - пара из типа кодирования и цепочки байтов'

    def byteClass(byte):
        code = ord(byte)
        if 48 <= code <= 57:
            return BCDIG # digits
        elif code in (9,10,13) or 32<=code<=126:
            return BCTXT # text
        else:
            return BCBIN # any chars
    ...
    for i, byte in enumerate(data):
        newBc = byteClass(byte)
        if newBc != bc:
            if bc:
                result.append([bc, data[start:i]])
            bc = newBc
            start = i
    if bc:
        result.append([bc, data[start:]])

    # группировка по Grand Zebu (http://grandzebu.net/informatique/codbar-en/pdf417.htm)
    ...
    for i in xrange(resultLen):
        ...
        if curr[0] == BCDIG:
            if i == 0: # It's the first block
                if resultLen>1: # And there is other blocks behind
                    next = result[i+1]
                    if next[i] == BCTXT:
                        ...
                        if len(curr[1])==1:
                            curr[0] = BCBIN
            elif i == resultLen-1: # It's the last one
                prev = result[i-1]
                if prev[0] == BCTXT:
                    ...
                    if len(curr[1])==1:
                        curr[0] = BCBIN
            else: # It's not the first block and It's the last one
                prev = result[i-1]
                next = result[i+1]
                if prev[0] == BCTXT and next[0] == BCTXT:
                    # Framed by "text" type blocks
                    if len(curr[1])<8:
                        curr[0] = BCTXT
                elif prev[0] == BCBIN and next[0] == BCBIN:
                    # Framed by "byte" type blocks
                    if len(curr[1])<4:
                        curr[0] = BCBIN
                if prev[0] == BCTXT and next[0] == BCBIN:
    ...
    for i in xrange(resultLen):
        curr = result[i]
        if curr[0] == BCTXT and i>0:
            # It's not the first (if first, never interesting to change)
            if i == resultLen-1: # It's the last one
                prev = result[i-1]
                if prev[0] == BCBIN:
                    # It's preceded by a "byte" type block
                    if len(curr[1]) == 1:
                        curr[0] = BCBIN
            else: # It's not the last one
                prev = result[i-1]
                next = result[i+1]
                if prev[0] == BCBIN and next[0] == BCBIN:
                    # Framed by "byte" type blocks
                    if len(curr[1]) < 5:
                        curr[0] = BCBIN
                elif prev[0] == BCBIN or next[0] == BCBIN:
    ...
    #
    resultLen = len(result)
    for i in xrange(1, resultLen-1):
        curr = result[i]
        if curr[0] == BCBIN and len(curr[0])==0 and result[i-1][0] == BCTXT and result[i+1][0] == BCTXT:
            curr[0] = BCTXT
    return regroup(result)


def txtCompaction(codeWords, data):
    u'текстовое уплотнение'
    def codeChar(char):
        if char:
            charInfo = charCodes.get(char, None)
            if charInfo:
                return charInfo
            else:
                return (ISBYTE, ord(char))
        else:
            return 0, 0

    mode = ALPHA # Default table
    encodedData = []
    for i, char in enumerate(data):
        charModes, charCode = codeChar(char)
        if charModes & mode:
            encodedData.append(charCode)
        elif charModes ==ISBYTE:
            if len(encodedData)%2:
                encodedData.append( PAL if mode & PUNCTUATION else PS )
                mode = ALPHA if mode & PUNCTUATION else mode
            encodedData.append(BYTESHIFT)
            encodedData.append(charCode)
        else:
            if mode == PUNCTUATION:
                encodedData.append(PAL)
                mode = ALPHA

            if mode == ALPHA:
                if charModes & LOWER:
                    encodedData.append(LL)
                    encodedData.append(charCode)
                    mode = LOWER
                elif charModes & MIXED:
                    encodedData.append(ML)
                    encodedData.append(charCode)
                    mode = MIXED
                elif codeChar(data[i+1:i+2])[0] & codeChar(data[i+2:i+3])[0] & PUNCTUATION:
                    encodedData.append(ML)
                    encodedData.append(PL)
                    encodedData.append(charCode)
                    mode = PUNCTUATION
                else:
                    encodedData.append(PS)
                    encodedData.append(charCode)
            elif mode == LOWER:
                if charModes & ALPHA:
                    if codeChar(data[i+1:i+2])[0] & codeChar(data[i+2:i+3])[0] & ALPHA:
                        encodedData.append(ML)
                        encodedData.append(AL)
                        mode = ALPHA
                    else:
                        encodedData.append(AS)
                    encodedData.append(charCode)
                elif charModes & MIXED:
                    encodedData.append(ML)
                    encodedData.append(charCode)
    ...
    if len(encodedData) % 2:
        encodedData.append(PS)
    for i in xrange(0, len(encodedData), 2):
        v0 = encodedData[i]
        v1 = encodedData[i+1]
        if v0 >= 30:
            codeWords.append(v0)
            ...
            codeWords.append(v0*30+v1)


def digCompaction(codeWords, data):
    u'цифровое уплотнение'
    for i in xrange(0,len(data),44):
        num = int('1'+data[i:i+44])
        out = []
        while num:
            out.append(num%900)
            num //= 900
        codeWords += reversed(out)



def binCompaction(codeWords, data):
    u'байтовое уплотнение'
    for i in xrange(0,len(data),6):
        ...
        if len(section) == 6:
            ...
            for char in section:
                codeWords.append(ord(char))


def assemble(segments):
    u'функция превращает цепочку сегментов в список кодовых слов'
    codeWords = []
    for segment in segments:
        if segment[0] == BCTXT:
            if codeWords:
                codeWords.append(BCTXT)
            txtCompaction(codeWords, segment[1])
        elif segment[0] == BCDIG:
            codeWords.append(BCDIG)
            digCompaction(codeWords, segment[1])
        else: # segment[0] == BCBIN:
            codeWords.append(BCBIN if len(segment[1])%6 else BCBIN6)
            binCompaction(codeWords, segment[1])
    return codeWords


def maxPossibleErrorLevel(remain):
    level = 8
    size = 512
    while level > 0:
        if remain >= size:
...
def getMaxSquare(codeColumns, codeRows):
    if codeColumns > 21:
        codeColumns = 29
        codeRows = 32
    else:
        codeColumns = 16
        codeRows = 58
    return MAX_DATA_CODEWORDS + 2, codeColumns, codeRows


def calculateErrorCorrection(codeWords, errorLevel):
    if errorLevel < 0 or errorLevel > 8:
        errorLevel = 0
    A = ERROR_LEVEL[errorLevel]
    lenECWords = 2 << errorLevel
    lenCodeWords = len(codeWords)
    codeWords.extend( [0]*lenECWords )
    lastE = lenECWords - 1
    for k in xrange(lenCodeWords):
        t1 = codeWords[k] + codeWords[lenCodeWords]
        for e in xrange(lenECWords):
            t2 = (t1 * A[lastE - e]) % MOD
            t3 = MOD - t2
            codeWords[lenCodeWords + e] = ((0 if e == lastE else codeWords[lenCodeWords + e + 1]) + t3) % MOD
    for e in xrange(lenECWords):
        codeWords[lenCodeWords + e] = (MOD - codeWords[lenCodeWords + e]) % MOD


def pdf417basecode(data, errorLevel=-1, codeColumns=-1, codeRows=-1, aspectRatio=0.5, scanLineHeight=3):
    segments = breakString(data)
    codeWords = assemble(segments)
    codeWords.insert(0, len(codeWords)+1)
    lenCodeWords = len(codeWords)
    if lenCodeWords>MAX_DATA_CODEWORDS:
        raise Exception('The text is too big')

    if errorLevel<0:
        if lenCodeWords < 41:
            errorLevel = 2
        elif lenCodeWords < 161:
            errorLevel = 3
        elif lenCodeWords < 321:
            errorLevel = 4
        else:
            errorLevel = 5
    errorLevel = min(errorLevel, maxPossibleErrorLevel(MAX_DATA_CODEWORDS + 2 - lenCodeWords))

    fixedColumns= codeColumns>=1
    fixedRows   = codeRows>=3
    skipRowColAdjust = False
    codeColumns = max(1, min(codeColumns, 30))
    codeRows    = max(3, min(codeColumns, 90))
    lenErr = 2 << errorLevel
    tot = lenCodeWords + lenErr
    if fixedColumns and fixedRows:
        tot = codeColumns * codeRows
        if tot > MAX_DATA_CODEWORDS + 2:
            tot, codeColumns, codeRows = getMaxSquare(codeColumns, codeRows)
        if tot < lenCodeWords + lenErr:
            tot = lenCodeWords + lenErr
        else:
            skipRowColAdjust = True
    elif not fixedColumns and not fixedRows:
        fixedColumns = True
        codeRows = max(0.001, min(codeColumns, 1000))
        b = 73 * aspectRatio - 4
        c = (-b + sqrt(b * b + 4 * 17 * aspectRatio * (lenCodeWords + lenErr) * scanLineHeight)) / (2 * 17 * aspectRatio)
        codeColumns = int(c + 0.5)
        codeColumns = max(1, min(codeColumns, 30))
    if not skipRowColAdjust:
        if fixedColumns:
    ...
    codeWords.extend([BCTXT]*pad)
    lenCodeWords = len(codeWords)
    codeWords[0] = lenCodeWords
    calculateErrorCorrection(codeWords, errorLevel)
    lenCodeWords = tot
    return codeWords, codeColumns, codeRows, errorLevel
...
class CImageOut:
    ...
    def __init__(self, codeWords, codeColumns, codeRows, errorLevel, scanLineHeight):
        width = self.leftMargin + START_CODE_SIZE * (codeColumns + 3) + STOP_SIZE + self.rightMargin
        height = self.topMargin + codeRows*scanLineHeight + self.bottomMargin
        self.image = QtGui.QImage(width, height, QtGui.QImage.Format_Mono)
        self.image.fill(1)
        self.x = self.leftMargin
        self.y = self.topMargin
        self.scanLineHeight = scanLineHeight
        self.paintCode(codeWords, codeColumns, codeRows, errorLevel)


    def nl(self):
        self.x  = self.leftMargin
        self.y += self.scanLineHeight


    def outCodeWord(self, word, bits=17):
        for i in xrange(bits-1,-1,-1):
            if word & (1<<i):
                for j in xrange(self.scanLineHeight):
                    self.image.setPixel(self.x, self.y+j, 0)
            self.x += 1


    def outStartPattern(self):
        self.outCodeWord(START_PATTERN, 17)


    def outStopPattern(self):
        self.outCodeWord(STOP_PATTERN, 18)
        self.nl()

    def paintCode(self, codeWords, codeColumns, codeRows, errorLevel):
        codePtr = 0
        for row in xrange(codeRows):
            rowMod = row % 3
            ...
            self.outCodeWord(cluster[edge])

            for column in xrange(codeColumns):
                self.outCodeWord(cluster[codeWords[codePtr]])
                codePtr += 1
...
def pdf417image(data, errorLevel=-1, codeColumns=-1, codeRows=-1, aspectRatio=0.5, scanLineHeight=3):
    if type(data) != str:
        data = str(data)
    scanLineHeight = min(max(scanLineHeight, 1), 5)
    codeWords, codeColumns, codeRows, errorLevel = pdf417basecode(data, errorLevel, codeColumns, codeRows, aspectRatio, scanLineHeight)
    io = CImageOut(codeWords, codeColumns, codeRows, errorLevel, scanLineHeight)
    return io.image
