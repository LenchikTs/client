...
class CSimpleProgressDialog(Ui_Dialog, QtGui.QDialog):
    ReadyToWork = 1
    WorkInProgress = 2
    WorkCanceled = 3
    WorkDone = 4
...
#                enabledButtons = ()
                enabledButtons = QtGui.QDialogButtonBox.NoButton
...
#                button.setEnabled(self.buttonBox.standardButton(button) in enabledButtons)

            self.buttonBox.setStandardButtons(enabledButtons)
            if self.okButtonText and (enabledButtons & QtGui.QDialogButtonBox.Ok):
                self.buttonBox.button(QtGui.QDialogButtonBox.Ok).setText(self.okButtonText)
            self.state = state
...
#            self.repaint()
    ...
    def onStop(self):
        if self.autoClose:
            QTimer.singleShot(300, self.close) # 0.3 sec
