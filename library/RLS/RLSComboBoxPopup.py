...
from library.Utils import forceBool, forceDate, forceInt, forceRef, forceString, getPref, setPref
from library.TableModel import CDateCol, CTableModel, CTextCol

from Ui_RLSComboBoxPopup import Ui_RLSComboBoxPopup
    ...
    def eventFilter(self, watched, event):
        if watched == self.tblRLS:
            if event.type() == QEvent.KeyPress and event.key() in (Qt.Key_Return, Qt.Key_Enter, Qt.Key_Select):
                ...
                self.tblRLS.emit(SIGNAL('doubleClicked(QModelIndex)'), index)
                return True
    ...
    def getRLSIdList(self, tradeName, inn, pharmGroupId, ATCid, date, showDisabled, forceCode):
        ...
        if tradeName:
            cond.append(db.joinOr([
                                    table['tradeName'].like(tradeName),
                                    table['tradeNameLat'].like(tradeName),
                                  ]))
        if inn:
            cond.append(db.joinOr([
                                    table['INPName'].like(inn),
                                    table['INPNameLat'].like(inn),
                                  ]))
        if pharmGroupId:
        ...
        if date:
            subCond = db.joinAnd([ db.joinOr([table['regDate'].isNull(), table['regDate'].le(date)]),
                                   db.joinOr([table['annDate'].isNull(), table['annDate'].gt(date)]) ])
            if forceCode:
                subCond = db.joinOr([subCond, table['code'].eq(forceCode)])
            cond.append(subCond)
        if not showDisabled:
            ...
            if forceCode:
                subCond = db.joinOr([subCond, table['code'].eq(forceCode)])
            cond.append(subCond)
        idList = db.getIdList(queryTable, table['id'].name(),
                              where=cond,
...
#        pass
        ...
        if code:
            ...
            if record:
                ...
                if self.tableModel.date:
                    subCond = db.joinAnd([ db.joinOr([table['regDate'].isNull(), table['regDate'].le(self.tableModel.date)]),
                                           db.joinOr([table['annDate'].isNull(), table['annDate'].gt(self.tableModel.date)]) ])
                    if self.code:
                        subCond = db.joinOr([subCond, table['code'].eq(self.code)])
                    cond.append(subCond)
                subCond = table['disabledForPrescription'].eq(0)
                if self.code:
                    subCond = db.joinOr([subCond, table['code'].eq(self.code)])
    ...
    def selectRLSCode(self, code):
        self.code = code
        self.emit(SIGNAL('RLSCodeSelected(int)'), code)
        self.close()
