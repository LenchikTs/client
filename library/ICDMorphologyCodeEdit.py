...
from library.Ui_ICDMorphologyPopup import Ui_ICDMorphologyPopup
...
class CICDMorphologyCodeEdit(QtGui.QLineEdit):
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() not in (Qt.Key_Return, Qt.Key_Backspace) and not self.existsValue(unicode(self.text())):
...
class CICDMorphologyCodeEditEx(CROComboBox):
    ...
    def __init__(self, parent=None):
        ...
        self._searchString = ''
        self.setSizeAdjustPolicy(CROComboBox.AdjustToMinimumContentsLength)
        self.preferredWidth = 600
        self._popup = CICDMorphologyPopup(self)
        self.connect(self._popup, SIGNAL('morphologyMKBSelected(QString)'), self.setText)
    ...
    def sizeHint(self):
        ...
        size = option.fontMetrics.boundingRect('____/_').size()
    ...
    def showPopup(self):
        if not self.isReadOnly():
            ...
            width = max(size.width()+20, selfWidth, self.preferredWidth)
            size.setWidth(width)
...
class CICDMorphologyPopup(QtGui.QFrame, Ui_ICDMorphologyPopup):
    ...
    def selectItemByCode(self, code):
        self.emit(SIGNAL('morphologyMKBSelected(QString)'), code)
        self.hide()
...
#        self._selectionModel.setCurrentIndex(index, QtGui.QItemSelectionModel.Select)
    ...
    def on_btnSearch_clicked(self):
        ...
        cond  = [table['name'].contain(word) for word in words]
