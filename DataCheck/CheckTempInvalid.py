...
from DataCheck.Ui_TempInvalid import Ui_TempInvalidCheckDialog
...
class TempInvalidCheck(QtGui.QDialog, Ui_TempInvalidCheckDialog, CCheck):
    def __init__(self, parent):
        ...
        self.dateEdit_1.setDate(QDate(currentDate.year(), 1, 1))
...
TempInvalid.duration, TempInvalid.state,
...
#            sex=forceInt(val('sex'))
            ...
            if checkExpert:
                state=forceInt(val('state'))
                if state == CTempInvalidState.closed:
                    if endDate > today:
