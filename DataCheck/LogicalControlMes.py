...
from DataCheck.Ui_LogicalControlMesDialog import Ui_LogicalControlMesDialog
...
#        tableMESMKB = db.table('mes.MES_mkb')
        ...
        if eventFeature == 1:
            cond.append(tableRBMesSpecification['level'].eq(2))
        elif eventFeature == 2:
            cond.append(tableRBMesSpecification['level'].ne(2))
        if begDate:
            cond.append(db.joinOr([tableEvent['execDate'].isNull(), tableEvent['execDate'].dateGe(begDate)]))
        if endDate:
        ...
        if self.chkDuration.isChecked() or (self.chkCountVisits.isChecked() and eventFeature != 2):
        ...
        if self.chkMes.isChecked():
            ...
            stmt = db.selectStmt(table, cols, condNotMES,  u'Event.id')
            self.getDataRecords(stmt, 0)
        if self.chkDuration.isChecked():
            condDuration = self.getCond(cond)
            currentDateTime = tableEvent['execDate'].formatValue(QDateTime.currentDateTime())
            ...
            stmt = db.selectStmt(tablequeryMES, cols, condDuration,  u'Event.id')
        ...
        if self.chkMKB.isChecked():
            ...
            stmt = db.selectStmt(table, cols, condMESMKB,  u'Event.id')
        ...
        if self.chkNotAlternative.isChecked():
            ...
            stmt = db.selectStmt(table, cols, condNotAlternative,  u'Event.id')
        ...
        if self.chkExecActions.isChecked():
            ...
            stmt = db.selectStmt(table, cols, condExecActions,  u'Event.id')
...
#            KSGNorm = forceInt(record.value('KSGNorm'))
...
#                        averageQnt = forceInt(recordMES.value('averageQnt'))
                        ...
                        if actionaverage is None and (not groupAlternative or (groupAlternative not in groupAlternativeList)):
                            countedActionErr = True
                            break
    ...
    def getMesAmountVisitErrors(self, eventId, mesId):
        ...
        stmt = u'''
            ...
            WHERE Visit.deleted = 0 AND (mMV.id IS NULL OR mMV.deleted = 0)
            AND (mS.id IS NULL OR mS.deleted = 0) AND (mVT.id IS NULL OR mVT.deleted = 0)
            AND Visit.event_id = %d AND mMV.master_id = %d
